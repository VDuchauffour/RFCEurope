...
class Attitude:
    ...
    def __init__(self, iThisPlayer, iTargetPlayer):
        ...
        self.bHasActiveMetBoth = CyGame().isDebugMode() or (
            pActiveTeam.isHasMet(iThisTeam) and pActiveTeam.isHasMet(iTargetTeam)
        )
            ...
            for sLine in self.sAttitudeString.split("\n"):
                ...
                if pMatch:
                    ...
                    if sString in MODIFIER_STRING_TO_KEY:
                        ...
                        BugUtil.debug(
                            u"Attitude::init() - Attitude string contains unknown modifier text: %s"
                            % (sString)
                        )
    ...
    def __str__(self):
        ...
        return (
            u"Attitude { 'iThisPlayer': %d, 'iTargetPlayer': %d, 'iAttitudeSum': %d, 'eAttitudeType': %d, 'bHasActiveMetBoth': %s,\n 'iAttitudeModifiers': %s,\n 'sAttitudeString': %s }"
            % (
                self.iThisPlayer,
                self.iTargetPlayer,
                self.iAttitudeSum,
                self.eAttitudeType,
                self.bHasActiveMetBoth,
                str(self.iAttitudeModifiers),
                self.sAttitudeString,
            )
        )
        ...
        if self.bHasActiveMetBoth:
            ...
            if bWarPeace:
                ...
                if pThisTeam.isAtWar(iTargetTeam):
                    szText += u"%c" % (
                        gc.getCommerceInfo(CommerceTypes.COMMERCE_GOLD).getChar() + 25
                    )
                    ...
                    if pThisTeam.isForcePeace(iTargetTeam):
                        ...
                        for iOwnerTeam in range(gc.getMAX_TEAMS()):
                            if pTargetTeam.isVassal(iOwnerTeam) and pThisTeam.isForcePeace(
                                iOwnerTeam
                            ):
                                bPeace = True
                                break
                    if bPeace:
                        szText += u"%c" % (
                            gc.getCommerceInfo(CommerceTypes.COMMERCE_GOLD).getChar() + 26
                        )
...
def isWorstEnemy(playerOrID, enemyOrID):
    ...
    return (
        not team.isHuman()
        and team.getID() != enemyTeam.getID()
        and getWorstEnemyTeam(player) == enemyTeam.getID()
    )
