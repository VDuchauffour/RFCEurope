    ...
    def getRequiredInheritedAttribute(self, name):
        ...
        if value is not None:
            ...
            raise BugUtil.ConfigError(
                "Element <%s> requires inherited attribute %s", self.tag, name
            )
...
class Handler:
    ...
    def addAttribute(
        self, name, required=False, inherited=False, default=None, fallback=None, exclude=False
    ):
    ...
    def flatten(self, element):
        for (name, required, inherit, default, fallback, exclude) in self.attributes:
            ...
            if value is None and required:
                raise BugUtil.ConfigError(
                    "Element <%s> requires attribute '%s'", element.tag, name
                )
    ...
    def startChild(self, parentElement, tag, attrs):
        if not self.isValidChild(tag):
            raise BugUtil.ConfigError(
                "Element <%s> does not accept child <%s>", parentElement.tag, tag
            )
    ...
    def resolveDll(self, element, dll):
        ...
        if inherited > dll:
            BugUtil.warn(
                "BugConfig - element <%s>.dll attribute %s overrides newer inherited dll attribute %s"
                % (element.tag, dll, inherited)
            )
        return dll
...
class BugHandler(Handler):
    ...
    def __init__(self):
        Handler.__init__(
            self, self.TAG, "module dll", (LoadHandler.TAG, ConfigHandler.TAG, ModHandler.TAG)
        )
    ...
    def handle(self, element, module, dll):
        ...
        if module and dll:
            BugUtil.debug(
                "BugConfig - configuration for module %s requires DLL version %s", module, dll
            )
...
class ConfigHandler(Handler):
    ...
    def handle(self, element, root, tag, key, parents, module, clazz):
        if root:
            ...
            if isRegistered(key):
                BugUtil.warn(
                    "BugConfig - <%s>.key %s handler already registered, ignoring",
                    element.tag,
                    key,
                )
            else:
...
class ModHandler(Handler):
    ...
    def __init__(self):
        Handler.__init__(
            self, self.TAG, "id module name author version build date url dll", (LoadHandler.TAG)
        )
...
class EventsHandler(HandlerWithArgs):
    ...
    def handle(self, element, module, clazz, dll):
        dll = BugDll.decode(dll)
        if self.isDllOkay(element, dll):
            BugUtil.callFunction(
                module, clazz, CvEventInterface.getEventManager(), *element.args, **element.kwargs
            )
        else:
            BugUtil.info(
