...
class BugEventManager(CvEventManager.CvEventManager):
    ...
    def setPopupHandler(self, eventType, handler):
        ...
        """
        BugUtil.debug(
            "BugEventManager - setting popup handler for event %s (%d)", handler[0], eventType
        )
    ...
    def _dispatchEvent(self, eventType, argsList):
        ...
        return EVENT_FUNCTION_MAP.get(eventType, BugEventManager._handleDefaultEvent)(
            self, eventType, argsList
        )
    ...
    def onCombatLogCollateral(self, argsList):
        ...
        BugUtil.debug(
            "%s bombards %s for %d HP", pAttacker.getName(), pDefender.getName(), iDamage
        )
