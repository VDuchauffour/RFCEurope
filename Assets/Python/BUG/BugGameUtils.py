        ...
        for name, func in clazz.__dict__.iteritems():
            if not name.startswith("_") and isinstance(func, types.FunctionType):
                self._createCallback(
                    name, BugUtil.bindFunction(self._baseUtils, name), None, self._log
                )
    ...
    def callListener(self, listener, argsList, result):
        try:
            ...
            if value is not None:
                BugUtil.warn(
                    "BugGameUtils - %s - ignoring %s listener's return value %r",
                    self.name,
                    listener.__module__,
                    value,
                )
...
class GameUtilsHandler(BugConfig.Handler):
    ...
    def __init__(self):
        BugConfig.Handler.__init__(
            self,
            GameUtilsHandler.TAG,
...
class CallbackHandler(BugConfig.Handler):
    ...
    def __init__(self):
        BugConfig.Handler.__init__(
            self, CallbackHandler.TAG, "name type default module handler listener log dll"
        )
    ...
    def handle(self, element, name, type, default, module, handler, listener, log, dll):
        ...
        if self.isDllOkay(element, dll):
            ...
            if utils:
                ...
                raise BugUtil.ConfigError(
                    "Element <%s> requires attribute module or be enclosed in <gameutils> that defines a class",
                    element.tag,
                )
            else:
            ...
            BugUtil.info(
                "BugGameUtils - ignoring <%s> %s, requires dll version %s",
                element.tag,
                name,
                self.resolveDll(element, dll),
            )
