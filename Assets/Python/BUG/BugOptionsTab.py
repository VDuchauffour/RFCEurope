...
class BugOptionsTab:
    ...
    def createMainPanel(self, screen):
        ...
        hover = BugUtil.getPlainText(
            "TXT_KEY_BUG_OPTBUTTON_EXIT_HOVER", "Exits the BUG Options screen."
        )
    ...
    def addTextEdit(self, screen, labelPanel, controlPanel, name):
        ...
        if option is not None:
            ...
            screen.attachEdit(
                controlPanel, control, value, self.callbackIFace, "handleBugTextEditChange", name
            )
            screen.setToolTip(control, option.getTooltip())
    ...
    def addDropdown(
        self, screen, labelPanel, controlPanel, name, spacer, layout, elements, index, callback
    ):
        option = self.getOption(name)
        if option is not None:
            # create label
            ...
            screen.attachDropDown(
                controlPanel, control, "", elements, self.callbackIFace, callback, name, index
            )
            screen.setToolTip(control, option.getTooltip())
    ...
    def addIntDropdown(self, screen, labelPanel, controlPanel, name, spacer=False, layout="right"):
        ...
        if option is not None:
            ...
            return self.addDropdown(
                ...
                elements,
                index,
                "handleBugIntDropdownChange",
            )
        else:
            self.addMissingOption(screen, controlPanel, name)

    def addFloatDropdown(
        self, screen, labelPanel, controlPanel, name, spacer=False, layout="right"
    ):
        option = self.getOption(name)
        if option is not None:
            index = option.getIndex()
            ...
            return self.addDropdown(
                ...
                elements,
                index,
                "handleBugFloatDropdownChange",
            )
        else:
            self.addMissingOption(screen, controlPanel, name)

    def addColorDropdown(
        self, screen, labelPanel, controlPanel, name, spacer=False, layout="left"
    ):
        option = self.getOption(name)
        if option is not None:
            index = option.getIndex()
            ...
            return self.addDropdown(
                ...
                elements,
                index,
                "handleBugColorDropdownChange",
            )
        else:
            self.addMissingOption(screen, controlPanel, name)

    def addCheckboxDropdown(
        self,
        screen,
        checkPanel,
        dropPanel,
        ...
        if checkOption is not None and dropOption is not None:
            ...
            screen.attachDropDown(
                dropPanel, dropControl, "", elements, self.callbackIFace, callback, dropName, index
            )
    ...
    def addCheckboxTextDropdown(
        self, screen, checkPanel, dropPanel, checkName, dropName, layout="left", spacer=False
    ):
        checkOption = self.getOption(checkName)
        dropOption = self.getOption(dropName)
        if checkOption is not None and dropOption is not None:
            ...
            checkControl, dropControl = self.addCheckboxDropdown(
                screen,
                checkPanel,
                dropPanel,
                checkName,
    ...
    def addCheckboxIntDropdown(
        self, screen, checkPanel, dropPanel, checkName, dropName, layout="right", spacer=False
    ):
        checkOption = self.getOption(checkName)
        dropOption = self.getOption(dropName)
        if checkOption is not None and dropOption is not None:
    ...
    def addSlider(
        ...
        if option is not None:
            ...
            value = option.getRealValue()
            if vertical:
                screen.attachVSlider(
                    controlPanel,
                    control,
                    self.callbackIFace,
                    "handleBugSliderChanged",
                    name,
                    min,
                    max,
                    value,
