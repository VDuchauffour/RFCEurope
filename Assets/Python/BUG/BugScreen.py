...
class BugScreen:
    ...
    def draw(self):
        self.screen.setDimensions(
            self.screen.centerX(0), self.screen.centerY(0), self.width, self.height
        )

        self.screen.addDDSGFC(
            self.BGwidget_Id,
            ArtFileMgr.getInterfaceArtInfo(self.BGkey).getPath(),
            0,
            0,
            self.width,
            self.height,
