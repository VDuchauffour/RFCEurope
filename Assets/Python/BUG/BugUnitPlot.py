...
class BupPanel:
    ...
    def Draw(self):
        ...
        iMaxUnits = max(len(self.BupUnits), len(self.BupUnits_Prior))

        #         iMaxUnits = min(iMaxUnits, \
        #                         iMaxRows * self._getMaxCols())

        BugUtil.debug(
            "BupPanel rows(%i), cols(%i), cells(%i)",
            self._getMaxRows(),
            self._getMaxCols(),
            iMaxRows * self._getMaxCols(),
        )
        BugUtil.debug(
            "BupPanel units_c(%i) units_p(%i), units max(%i)",
            len(self.BupUnits),
            len(self.BupUnits_Prior),
            iMaxUnits,
        )
        BugUtil.debug(
            "BupPanel max rows(%i), skipped(%i), index(%i)", iMaxRows, iSkippedCells, iIndex
        )
        BugUtil.debug(
            "BupPanel plotlistoffset, current(%i), prior(%i)",
            self.PlotListOffset,
            self.PlotListOffset_Prior,
        )
        BugUtil.debug(
            "BupPanel plotlistcolumn, current(%i), prior(%i)",
            CyInterface().getPlotListColumn(),
            self.PlotListColumn_Prior,
        )
        ...
        for iUnit in range(iMaxUnits):
            ...
            BugUtil.debug(
                "BupPanel index(%i), iUnit(%i), iUnit_Prior(%i)", iIndex, iUnit, iUnit_Prior
            )
            ...
            if iIndex == iFirstPlot:  # looking at the top left cell
                ...
                if (
                    iVisibleUnits - iIndex - CyInterface().getPlotListColumn() + iSkippedCells
                ) > 1:  # more units to show to right
                    bRightArrow = True

            # prior unit doesn't exist - just need to draw the new unit
            # this is handled below in the '_update*' functions
            if (
                len(self.BupUnits_Prior) != 0
                and iUnit_Prior >= 0
            ...
            BugUtil.debug(
                "BupPanel unit(%i), index(%i), col(%i), row(%i), x(%i), y(%i)",
                iUnit,
                iIndex,
                self._getCol(iIndex),
                self._getRow(iIndex),
    ...
    def _drawDot(self, BupUnit, szCell, iCount, x, y):
        ...
        self.screen.addDDSGFC(
            szCell + "Dot",
            getArt(BupUnit.DotStatus),
            x - 3 + xOffset,
            y - 7 + yOffset,
            xSize,
            ySize,
    ...
    def _drawHealthBar(self, BupUnit, szCell):
        if PleOpt.isShowHealthBar() and BupUnit.isShowHealth:
            ...
            self.screen.setBarPercentage(
                szStringHealth,
                InfoBarTypes.INFOBAR_STORED,
                float(BupUnit.currHitPoints) / float(BupUnit.maxHitPoints),
            )
            self.screen.setStackedBarColors(
                szStringHealth, InfoBarTypes.INFOBAR_STORED, gc.getInfoTypeForString(sColor)
            )
