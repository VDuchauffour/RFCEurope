...
def addEvents(eventManager):
    ...
    # Demand Tribute
    DiploEvent(
        "AI_DIPLOCOMMENT_DEMAND_TRIBUTE", "TributeDemanded", onTributeDemanded, sendTrade=True
    )
    DiploEvent(
        "USER_DIPLOCOMMENT_ACCEPT_DEMAND", "TributeAccepted", onTributeAccepted, sendTrade=True
    )
    DiploEvent(
        "USER_DIPLOCOMMENT_REJECT_DEMAND", "TributeRejected", onTributeRejected, sendTrade=True
    )
    ...
    DiploEvent(
        "USER_DIPLOCOMMENT_NO_REVOLUTION",
        "CivicRejected",
        onCivicRejected,
        argFunc=lambda eFromPlayer, eToPlayer, args, data: (
            PlayerUtil.getFavoriteCivic(eToPlayer),
    ...
    DiploEvent(
        "USER_DIPLOCOMMENT_STOP_TRADING", "EmbargoAccepted", onEmbargoAccepted, sendData=True
    )
    DiploEvent(
        "USER_DIPLOCOMMENT_NO_STOP_TRADING", "EmbargoRejected", onEmbargoRejected, sendData=True
    )
...
class DiploEvent:
    def __init__(
        ...
        if tradeType:
            BugUtil.debug(
                "DiplomacyUtil - mapped %s to %s with %s", comment, event, str(tradeType)
            )
        else:
    ...
    def dispatch(self, eFromPlayer, eToPlayer, args):
        data = diplo.getData()
        BugUtil.debug(
            "DiplomacyUtil - %s [%d] from %d to %d with %r",
            self.comment,
            data,
            eFromPlayer,
            eToPlayer,
            ...
            if self.sendTrade or self.tradeType:
                ...
                if self.tradeType:
                    ...
                    if trade and trades:
                        ...
                        BugUtil.debug(
                            "DiplomacyUtil - firing %s with %s %d",
                            self.event,
                            str(self.tradeType),
                            iData,
                        )
                        ...
                        BugUtil.debug(
                            "DiplomacyUtil - firing %s without %s", self.event, str(self.tradeType)
                        )
                        argList.append(-1)
            else:
                BugUtil.debug("DiplomacyUtil - firing %s", self.event)
...
def dispatchEvent(eComment, eFromPlayer, eToPlayer, args):
    ...
    if event:
        ...
        BugUtil.debug(
            "DiplomacyUtil - ignoring %s from %d to %d with %r", key, eFromPlayer, eToPlayer, args
        )
...
def getProposedTradeData(getFunc, addFunc):
    for index in range(MAX_TRADE_DATA):
        ...
        BugUtil.warn(
            "DiplomacyUtil.getProposedTradeData - reached %d items, ignoring rest", MAX_TRADE_DATA
        )
