...
def getGreatPeopleText(
    city, iGPTurns, iGPBarWidth, bGPBarTypesNone, bGPBarTypesOne, bIncludeCityName
):
    ...
    if not city:
        ...
        if iGPTurns:
            if bIncludeCityName:
                ...
                szText = BugUtil.getText(
                    "INTERFACE_GREAT_PERSON_TURNS", (sGreatPeopleChar, iGPTurns)
                )
        else:
            if bIncludeCityName:
                szText = BugUtil.getText(
                    "INTERFACE_GREAT_PERSON_CITY", (sGreatPeopleChar, city.getName())
                )
            else:
            ...
            if bGPBarTypesOne or len(lPercents) == 1:
                ...
                if iGPTurns:
                    if bIncludeCityName:
                        ...
                        szText = BugUtil.getText(
                            "INTERFACE_GREAT_PERSON_TURNS", (pInfo.getDescription(), iGPTurns)
                        )
                else:
                    if bIncludeCityName:
                        szText = BugUtil.getText(
                            "INTERFACE_GREAT_PERSON_CITY", (pInfo.getDescription(), city.getName())
                        )
                    else:
