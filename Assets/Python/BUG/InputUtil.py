...
class Keystroke:
    ...
    def __ne__(self, other):
        ...
        return (
            ...
            or self.control != other.control
            or self.shift != other.shift
        )
...
class ShortcutHandler(BugConfig.HandlerWithArgs):
    ...
    def __init__(self):
        BugConfig.HandlerWithArgs.__init__(
            self, ShortcutHandler.TAG, "key keys module function dll"
        )
    ...
    def handle(self, element, keys, module, function, dll):
        ...
        if self.isDllOkay(element, dll):
            CvEventInterface.getEventManager().addShortcutHandler(
                keys, BugUtil.getFunction(module, function, *element.args, **element.kwargs)
            )
        else:
            BugUtil.info(
