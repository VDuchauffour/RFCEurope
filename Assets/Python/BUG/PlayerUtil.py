...
def teams(alive=None, human=None, barbarian=None, minor=None, active=None):
    ...
    for eTeam in range(gc.getMAX_TEAMS()):
        ...
        if (
            not team.isNone()
            and team.isEverAlive()
            and matchPlayerOrTeam(team, alive, human, barbarian, minor)
        ):
            yield team


def matchPlayerOrTeam(
    teamOrPlayer, alive=None, human=None, barbarian=None, minor=None, active=None
):
    """
    ...
    """
    return (
        (alive is None or alive == teamOrPlayer.isAlive())
...
def isEverActive(teamOrPlayer):
    ...
    """
    return (
        (gc.getGame().isHotSeat() and teamOrPlayer.isHuman())
...
def getPossibleEmbargos(playerOrID, askingPlayerOrID):
    ...
    for player in players(alive=True, barbarian=False, minor=False):
        eTeam = player.getTeam()
        if (
            eTeam == askingPlayer.getTeam()
            or eTeam == askedPlayer.getTeam()
            or askedTeam.isAtWar(eTeam)
        ):
            # won't embargo your team, their team, or a team they are fighting
            continue
        if not (
            (askingTeam.isHasMet(eTeam) and askedTeam.isHasMet(eTeam))
