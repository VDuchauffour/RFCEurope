...
class Tracker:
    ...
    def fillTable(self, screen, table, eYield, eTileType):
        ...
        self.appendTable(
            ...
            table,
            False,
            BugUtil.getText(LABEL_KEYS[eTileType], (self.tileCounts[eTileType],)),
            eYield,
            iTotal,
        )
        ...
        for eType in (DOMESTIC_TRADE, FOREIGN_TRADE):
            ...
            if iValue != 0:
                self.appendTable(
                    ...
                    table,
                    False,
                    BugUtil.getPlainText(LABEL_KEYS[eType]),
                    eYield,
                    iValue,
                    TradeUtil.isFractionalTrade(),
                )
        ...
        for eType in (BUILDINGS, CORPORATIONS, SPECIALISTS):
            ...
            if iValue != 0:
                iTotal += iValue
                self.appendTable(
                    screen, table, False, BugUtil.getPlainText(LABEL_KEYS[eType]), eYield, iValue
                )
        ...
        if iModifier != 0:
            ...
            self.appendTable(
                ...
                table,
                False,
                BugUtil.getText("TXT_KEY_CONCEPT_BASE_MODIFIER", (iModifier,)),
                eYield,
                iValue,
            )
        else:
        ...
        if eYield == YieldTypes.YIELD_PRODUCTION and self.iProductionModifier != 0:
            ...
            self.appendTable(
                ...
                BugUtil.getText(
                    "TXT_KEY_CONCEPT_PRODUCTION_MODIFIER",
                    (self.sModifierDetail, self.iProductionModifier),
                ),
                eYield,
                iValue,
            )
    ...
    def appendTable(self, screen, table, bTotal, heading, eYield, iValue, bFraction=False):
        ...
        screen.setTableText(
            ...
            VALUE_COLUMN,
            self.iRow,
            u"<font=1>%s%c</font>" % (value, cYield),
            "",
            WidgetTypes.WIDGET_GENERAL,
            -1,
            -1,
