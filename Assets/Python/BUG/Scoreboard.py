...
class Scoreboard:
    ...
    def assignRanks(self):
        ...
        for playerScore in scores:
            if not playerScore.has(NOT_MET) or not playerScore.value(NOT_MET):
                rank += 1
                playerScore.set(
                    RANK, smallText(BugUtil.colorText(u"%d" % rank, ScoreOpt.getRankColor()))
                )
        if rank > 0:
    ...
    def draw(self, screen):
        ...
        if (
            interface.getShowInterface() == InterfaceVisibility.INTERFACE_SHOW
        ...
        format = re.findall(
            "(-?[0-9]+|[^0-9])", ScoreOpt.getDisplayOrder().replace(" ", "").upper()
        )
        format.reverse()
        for k in format:
            if k == "-":
        ...
        screen.setPanelSize(
            "ScoreBackground",
            xResolution - 21 - totalWidth,
            y - (height * self.size()) - 4,
            totalWidth + 12,
            (height * self.size()) + 8,
        )
