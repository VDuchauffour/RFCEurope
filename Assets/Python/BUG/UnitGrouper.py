...
class LevelGrouping(Grouping):
    ...
    def __init__(self):
        ...
        for i in range(self.MAX_LEVEL):
            self._addGroup(
                Group(self, i, BugUtil.getText("TXT_KEY_UNIT_GROUPER_LEVEL_GROUP", (str(i),)))
            )
        self._addGroup(
            Group(
                self,
                self.MAX_LEVEL,
                BugUtil.getText("TXT_KEY_UNIT_GROUPER_LEVEL_GROUP", ("%d+" % self.MAX_LEVEL,)),
            )
        )
            ...
            if info:
                self._addGroup(
                    Group(
                        self,
                        i + 1,
                        "<img=%s size=16></img> %s" % (info.getButton(), info.getDescription()),
                    )
                )
...
class LocationGrouping(Grouping):
    ...
    def __init__(self):
        ...
        self._addGroup(
            Group(self, self.DOMESTIC_CITY, "TXT_KEY_UNIT_GROUPER_LOCATION_GROUP_DOMESTIC_CITY")
        )
        self._addGroup(
            Group(
                self,
                self.DOMESTIC_TERRITORY,
                "TXT_KEY_UNIT_GROUPER_LOCATION_GROUP_DOMESTIC_TERRITORY",
            )
        )
        self._addGroup(
            Group(self, self.TEAM_CITY, "TXT_KEY_UNIT_GROUPER_LOCATION_GROUP_TEAM_CITY")
        )
        self._addGroup(
            Group(self, self.TEAM_TERRITORY, "TXT_KEY_UNIT_GROUPER_LOCATION_GROUP_TEAM_TERRITORY")
        )
        self._addGroup(
            Group(self, self.FRIENDLY_CITY, "TXT_KEY_UNIT_GROUPER_LOCATION_GROUP_FRIENDLY_CITY")
        )
        self._addGroup(
            Group(
                self,
                self.FRIENDLY_TERRITORY,
                "TXT_KEY_UNIT_GROUPER_LOCATION_GROUP_FRIENDLY_TERRITORY",
            )
        )
        self._addGroup(
            Group(
                self,
                self.NEUTRAL_TERRITORY,
                "TXT_KEY_UNIT_GROUPER_LOCATION_GROUP_NEUTRAL_TERRITORY",
            )
        )
        self._addGroup(
            Group(
                self, self.ENEMY_TERRITORY, "TXT_KEY_UNIT_GROUPER_LOCATION_GROUP_ENEMY_TERRITORY"
            )
        )
        self._addGroup(
            Group(
                self,
                self.BARBARIAN_TERRITORY,
                "TXT_KEY_UNIT_GROUPER_LOCATION_GROUP_BARBARIAN_TERRITORY",
            )
        )
...
class OrderGrouping(Grouping):
    ...
    def __init__(self):
        ...
        self._addGroup(
            Group(self, self.ORDER_INTERCEPT, "TXT_KEY_UNIT_GROUPER_ORDER_GROUP_INTERCEPT")
        )
        ...
        self._addGroup(
            Group(self, self.ORDER_CONSTRUCT, "TXT_KEY_UNIT_GROUPER_ORDER_GROUP_CONSTRUCT")
        )
        ...
        self._addGroup(
            Group(self, self.ORDER_AUTO_BUILD, "TXT_KEY_UNIT_GROUPER_ORDER_GROUP_AUTO_BUILD")
        )
        self._addGroup(
            Group(self, self.ORDER_AUTO_NETWORK, "TXT_KEY_UNIT_GROUPER_ORDER_GROUP_AUTO_NETWORK")
        )
        self._addGroup(
            Group(self, self.ORDER_AUTO_CITY, "TXT_KEY_UNIT_GROUPER_ORDER_GROUP_AUTO_CITY")
        )
        self._addGroup(
            Group(self, self.ORDER_AUTO_RELIGION, "TXT_KEY_UNIT_GROUPER_ORDER_GROUP_AUTO_RELIGION")
        )
