    ...
    if gDetectionNecessary:
        ...
        if (
            ...
            favorite.setFavorite(
                gc.getLeaderHeadInfo(pPlayer.getPersonalityType()).getFavoriteCivic()
            )
    return favorite
                                ...
                                if eCivic == pOtherPlayer.getCivics(eCategory):
                                    if bFoundPossibleFavorite:
                                        ...
                                        BugUtil.debug(
                                            ...
                                            % (
                                                eCategory,
                                                gc.getCivicOptionInfo(eCategory).getText(),
                                            )
                                        )
                                        for eOtherCivic in gCivicsByCategory[eCategory]:
                                            if eOtherCivic != eCivic:
...
def initHelpers():
    ...
    BugUtil.debug(
        "FavoriteCivicDetector.initHelpers() gDetectionNecessary: %s" % (str(gDetectionNecessary))
    )
    if gDetectionNecessary:
        ...
        for iPlayer in args:
            ...
            if (
                ...
                if iPlayer in gFavoriteByPlayer:
                    ...
                    BugUtil.debug(
                        "FavoriteCivicDetector.dump() No data for player %d!" % (iPlayer)
                    )
    else:
...
class FavoriteCivicDetector:
    ...
    def onBeginActivePlayerTurn(self, argsList):
        ...
        if gDetectionNecessary:
            ...
            BugUtil.debug("======================================================================")
            BugUtil.debug(
                "FavoriteCivicDetectorEvent.onBeginActivePlayerTurn() START Turn %d" % (iTurn)
            )
            doUpdate()
