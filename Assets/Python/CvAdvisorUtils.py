...
def unitBuiltFeats(pCity, pUnit):
    ...
    if not gc.getPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_ARCHER):
        ...
        if pUnit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_ARCHER"):
            ...
            gc.getPlayer(pCity.getOwner()).setFeatAccomplished(
                FeatTypes.FEAT_UNITCOMBAT_ARCHER, True
            )

            if featPopup(pCity.getOwner()) and (
                gc.getGame().getStartYear() == gc.getDefineINT("START_YEAR")
            ):
                popupInfo = CyPopupInfo()
                popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON)
    ...
    if not gc.getPlayer(pCity.getOwner()).isFeatAccomplished(
        FeatTypes.FEAT_UNITCOMBAT_LIGHT_CAVALRY
    ):

        if pUnit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_LIGHT_CAVALRY"):

            gc.getPlayer(pCity.getOwner()).setFeatAccomplished(
                FeatTypes.FEAT_UNITCOMBAT_LIGHT_CAVALRY, True
            )
    ...
    if not gc.getPlayer(pCity.getOwner()).isFeatAccomplished(
        FeatTypes.FEAT_UNITCOMBAT_HEAVY_CAVALRY
    ):

        if pUnit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_HEAVY_CAVALRY"):

            gc.getPlayer(pCity.getOwner()).setFeatAccomplished(
                FeatTypes.FEAT_UNITCOMBAT_HEAVY_CAVALRY, True
            )
    ...
    if not gc.getPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_POLEARM):
        ...
        if pUnit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_POLEARM"):

            gc.getPlayer(pCity.getOwner()).setFeatAccomplished(
                FeatTypes.FEAT_UNITCOMBAT_POLEARM, True
            )
    ...
    if not gc.getPlayer(pCity.getOwner()).isFeatAccomplished(
        FeatTypes.FEAT_UNITCOMBAT_HEAVY_INFANTRY
    ):

        if pUnit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_HEAVY_INFANTRY"):

            gc.getPlayer(pCity.getOwner()).setFeatAccomplished(
                FeatTypes.FEAT_UNITCOMBAT_HEAVY_INFANTRY, True
            )
    ...
    if not gc.getPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_SIEGE):
        ...
        if pUnit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_SIEGE"):
            ...
            gc.getPlayer(pCity.getOwner()).setFeatAccomplished(
                FeatTypes.FEAT_UNITCOMBAT_SIEGE, True
            )
    ...
    if not gc.getPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_NAVAL):
        ...
        if pUnit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_NAVAL"):
            ...
            gc.getPlayer(pCity.getOwner()).setFeatAccomplished(
                FeatTypes.FEAT_UNITCOMBAT_NAVAL, True
            )
...
def buildingBuiltFeats(pCity, iBuildingType):
    ...
    if not gc.getPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_NATIONAL_WONDER):
        ...
        if isNationalWonderClass(gc.getBuildingInfo(iBuildingType).getBuildingClassType()):
            ...
            gc.getPlayer(pCity.getOwner()).setFeatAccomplished(
                FeatTypes.FEAT_NATIONAL_WONDER, True
            )
...
def endTurnFeats(iPlayer):
    ...
    if lRealPopulation > 100000:
        populationFeat(
            iPlayer, FeatTypes.FEAT_POPULATION_100_THOUSAND, "TXT_KEY_FEAT_100_THOUSAND"
        )
    if lRealPopulation > 200000:
        populationFeat(
            iPlayer, FeatTypes.FEAT_POPULATION_200_THOUSAND, "TXT_KEY_FEAT_200_THOUSAND"
        )
    if lRealPopulation > 500000:
        populationFeat(
            iPlayer, FeatTypes.FEAT_POPULATION_500_THOUSAND, "TXT_KEY_FEAT_500_THOUSAND"
        )
        ...
        if not gc.getPlayer(iPlayer).isFeatAccomplished(FeatTypes.FEAT_COPPER_CONNECTED):
            ...
            if iBonus != BonusTypes.NO_BONUS:
                if pCapitalCity.hasBonus(iBonus):
                    gc.getPlayer(iPlayer).setFeatAccomplished(
                        FeatTypes.FEAT_COPPER_CONNECTED, True
                    )
            ...
            for iI in range(gc.getNumBonusInfos()):
                if gc.getBonusInfo(iI).getHappiness() > 0:
                    if pCapitalCity.hasBonus(iI):
                        gc.getPlayer(iPlayer).setFeatAccomplished(
                            FeatTypes.FEAT_LUXURY_CONNECTED, True
                        )
        ...
        if not gc.getPlayer(iPlayer).isFeatAccomplished(FeatTypes.FEAT_FOOD_CONNECTED):
            ...
            for iI in range(gc.getNumBonusInfos()):
                if gc.getBonusInfo(iI).getHealth() > 0:
                    if pCapitalCity.hasBonus(iI):
                        gc.getPlayer(iPlayer).setFeatAccomplished(
                            FeatTypes.FEAT_FOOD_CONNECTED, True
                        )
...
def cityAdvise(pCity, iPlayer):
    ...
    if (
        gc.getPlayer(iPlayer).isOption(PlayerOptionTypes.PLAYEROPTION_ADVISOR_POPUPS)
        and gc.getPlayer(iPlayer).isHuman()
        ...
        if turn() % 40 == pCity.getGameTurnFounded() % 40:
            if pCity.getID() not in g_listNoLiberateCities:
                ...
                if eLiberationPlayer != -1:
                    ...
                    gc.getPlayer(iPlayer).canSplitEmpire()
                    and gc.getPlayer(iPlayer).canSplitArea(pCity.area().getID())
                    and pCity.AI_cityValue() < 0
                ):
                    popupInfo = CyPopupInfo()
                    popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON)
                    popupInfo.setData1(pCity.getID())
        ...
        if pCity.isProduction():
            ...
            if not pCity.isProductionUnit() and (pCity.getOrderQueueLength() <= 1):
                ...
                if (turn() + 1) % 40 == pCity.getGameTurnFounded() % 40:
                    ...
                    if (
                        ...
                        and (pCity.getPopulation() > 2)
                        and (
                            gc.getPlayer(iPlayer).AI_totalAreaUnitAIs(
                                pCity.area(), UnitAITypes.UNITAI_SETTLE
                            )
                            == 0
                        )
                        and not gc.getPlayer(iPlayer).AI_isFinancialTrouble()
                        and (pCity.area().getBestFoundValue(iPlayer) > 0)
                    ):
                        ...
                        for iI in range(gc.getNumUnitClassInfos()):
                            ...
                            if not isLimitedUnitClass(iI):
                                ...
                                if eLoopUnit != UnitTypes.NO_UNIT:
                                    ...
                                    if (
                                        gc.getUnitInfo(eLoopUnit).getDomainType()
                                        ...
                                        if pCity.canTrain(eLoopUnit, False, False):
                                            ...
                                            if pCity.getFirstUnitOrder(eLoopUnit) == -1:
                                                ...
                                                iValue = gc.getPlayer(iPlayer).AI_unitValue(
                                                    eLoopUnit,
                                                    UnitAITypes.UNITAI_SETTLE,
                                                    pCity.area(),
                                                )
                ...
                if (turn() + 5) % 40 == pCity.getGameTurnFounded() % 40:
                    ...
                    if (
                        ...
                        and (pCity.countNumImprovedPlots() == 0)
                        and (pCity.AI_countBestBuilds(pCity.area()) > 3)
                    ):
                                ...
                                if eLoopUnit != UnitTypes.NO_UNIT:
                                    ...
                                    if (
                                        ...
                                        if pCity.canTrain(eLoopUnit, False, False):
                                            ...
                                            iValue = (
                                                gc.getPlayer(iPlayer).AI_unitValue(
                                                    eLoopUnit,
                                            ...
                                            iValue += gc.getPlayer(iPlayer).AI_unitValue(
                                                eLoopUnit, UnitAITypes.UNITAI_ATTACK, pCity.area()
                                            )
                ...
                if (turn() + 12) % 40 == pCity.getGameTurnFounded() % 40:
                    ...
                    if (
                        gc.getPlayer(iPlayer).AI_totalAreaUnitAIs(
                            pCity.area(), UnitAITypes.UNITAI_MISSIONARY
                        )
                        == 0
                    ) and (gc.getTeam(gc.getPlayer(iPlayer).getTeam()).getAtWarCount(True) == 0):

                        eStateReligion = gc.getPlayer(iPlayer).getStateReligion()

                        if eStateReligion != ReligionTypes.NO_RELIGION:

                            if gc.getPlayer(iPlayer).getHasReligionCount(eStateReligion) < (
                                gc.getPlayer(iPlayer).getNumCities() / 2
                            ):
                                ...
                                for iI in range(gc.getNumUnitClassInfos()):
                                    ...
                                    if eLoopUnit != UnitTypes.NO_UNIT:
                                        ...
                                        if (
                                            ...
                                            if gc.getUnitInfo(eLoopUnit).getReligionSpreads(
                                                eStateReligion
                                            ):
                                ...
                                if eBestUnit != UnitTypes.NO_UNIT:
                                    ...
                                    popupInfo.setOnFocusPythonCallback(
            ...
            if not pCity.isProductionBuilding() and (pCity.getOrderQueueLength() <= 1):
                ...
                if (
                    (gc.getGame().getGameTurn < 100)
                    and (gc.getTeam(gc.getPlayer(iPlayer).getTeam()).getHasMetCivCount(True) > 0)
                        ...
                        for iI in range(gc.getNumBuildingClassInfos()):
                            ...
                            if not isLimitedWonderClass(iI):
                                ...
                                if eLoopBuilding != BuildingTypes.NO_BUILDING:
                                    ...
                                    if (
                                        gc.getBuildingInfo(eLoopBuilding).getDefenseModifier()
                                        > pCity.getNaturalDefense()
                                    ):
                        ...
                        for iI in range(gc.getNumBuildingClassInfos()):
                            ...
                            if not isLimitedWonderClass(iI):
                                ...
                                if eLoopBuilding != BuildingTypes.NO_BUILDING:
                                    ...
                                    if (
                                        gc.getBuildingInfo(eLoopBuilding).getMaintenanceModifier()
                                        < 0
                                    ):
                ...
                if (
                    pCity.getCommerceRate(CommerceTypes.COMMERCE_CULTURE) == 0
                        ...
                        for iI in range(gc.getNumBuildingClassInfos()):
                            ...
                            if not isLimitedWonderClass(iI):
                                ...
                                if eLoopBuilding != BuildingTypes.NO_BUILDING:
                                    ...
                                    if (
                                        gc.getBuildingInfo(eLoopBuilding).getCommerceModifier(
                                        ...
                                        if pCity.canConstruct(eLoopBuilding, False, False, False):
                                            ...
                                            iValue = gc.getBuildingInfo(
                                                eLoopBuilding
                                            ).getCommerceModifier(CommerceTypes.COMMERCE_GOLD)
                                ...
                                if eLoopBuilding != BuildingTypes.NO_BUILDING:
                                    ...
                                    if (
                                        ...
                                        if pCity.canConstruct(eLoopBuilding, False, False, False):
                                            ...
                                            iValue = gc.getBuildingInfo(
                                                eLoopBuilding
                                            ).getCommerceModifier(CommerceTypes.COMMERCE_RESEARCH)
                        ...
                        for iI in range(gc.getNumBuildingClassInfos()):
                            ...
                            if not isLimitedWonderClass(iI):
                                ...
                                if eLoopBuilding != BuildingTypes.NO_BUILDING:
                                    ...
                                    if (
                                        gc.getBuildingInfo(eLoopBuilding).getSeaPlotYieldChange(
                                        ...
                                        if pCity.canConstruct(eLoopBuilding, False, False, False):
                                            ...
                                            iValue = gc.getBuildingInfo(
                                                eLoopBuilding
                                            ).getSeaPlotYieldChange(YieldTypes.YIELD_FOOD)
