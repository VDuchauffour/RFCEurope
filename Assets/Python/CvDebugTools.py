...
class CvDebugTools:
    ...
    def setDebugMode(self, bVal):
        ...
        CyInterface().addImmediateMessage(
            "CvDebugTools.setDebugMode set to %s" % self.bDebugMode, ""
        )
    ...
    def initUnitPicker(self, argsList):
        ...
        popup.addPullDownString(
            gc.getPlayer(iOwner).getCivilizationShortDescription(0), iOwner
        )  # Rhye
        for i in range(gc.getMAX_PLAYERS()):
            if gc.getPlayer(i).isEverAlive():
                if i != iOwner:
                    ...
                    popup.addPullDownString(
                        gc.getPlayer(i).getCivilizationShortDescription(0), i
                    )  # Rhye
        ...
        if iNImprovements < map.getGridWidth() * map.getGridHeight():
            for x in range(map.getGridWidth()):
                for y in range((iNImprovements / map.getGridWidth()) + 1):
                    map.plot(x, y).setImprovementType(
                        (x + y * map.getGridWidth()) % iNImprovements
                    )
