...
class CvPediaMain(CvPediaScreen.CvPediaScreen):
    ...
    def createDictionaries(self):
        ...
        self.linkListGraphics = {
            ...
            "PROJECTS": u"%c  " % (CyGame().getSymbolID(FontSymbols.MAP_CHAR)),
            ...
            "CORPS": u"%c  " % (gc.getCommerceInfo(CommerceTypes.COMMERCE_GOLD).getChar()),
    ...
    def setPediaCommonWidgets(self):
        ...
        screen.setText(
            ...
            0,
            FontTypes.TITLE_FONT,
            WidgetTypes.WIDGET_PEDIA_FORWARD,
            1,
            -1,
        )
    ...
    def placeImprovements(self):
        ...
        self.displayTopics(
            tList, WidgetTypes.WIDGET_PEDIA_JUMP_TO_IMPROVEMENT, gc.getImprovementInfo
        )
    ...
    def placeUnitGroups(self):
        ...
        self.displayTopics(
            tList, WidgetTypes.WIDGET_PEDIA_JUMP_TO_UNIT_COMBAT, gc.getUnitCombatInfo
        )
    ...
    def placeCorporations(self):
        ...
        self.displayTopics(
            tList, WidgetTypes.WIDGET_PEDIA_JUMP_TO_CORPORATION, gc.getCorporationInfo
        )
    ...
    def placeSpecialists(self):
        ...
        self.displayTopics(
            tList, WidgetTypes.WIDGET_PEDIA_JUMP_TO_SPECIALIST, gc.getSpecialistInfo
        )
            ...
            if widgeyType == WidgetTypes.WIDGET_PEDIA_JUMP_TO_CIV:
                if gc.getCivilizationInfo(item[1]).isPlayable():
                    ...
                    screen.setTableText(
                        ...
                        gc.getCivilizationInfo(item[1]).getButton(),
                        WidgetTypes.WIDGET_PEDIA_JUMP_TO_CIV,
                        item[1],
                        1,
                        CvUtil.FONT_LEFT_JUSTIFY,
                    )
    ...
    def placeConcepts(self):
        ...
        for topic in tList:
            ...
            screen.appendListBoxString(
                ...
                CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT,
                topic[1],
                CvUtil.FONT_LEFT_JUSTIFY,
            )
    ...
    def placeHints(self):
        ...
        screen.addListBoxGFC(
            self.szHints,
            "",
            self.X_ITEMS_PANE + 20,
            self.Y_ITEMS_PANE,
            self.W_ITEMS_PANE + self.W_PEDIA_PAGE - 40,
            self.H_ITEMS_PANE,
            TableStyles.TABLE_STYLE_STANDARD,
        )
    ...
    def placeLinks(self):
        ...
        for item in self.linkList:
            ...
            if item[0] == -1:
                screen.appendListBoxString(
                    self.LIST_ID, "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY
                )
            else:
                lChar = self.linkListGraphics[item[2]]
                screen.appendListBoxString(
                    self.LIST_ID,
                    lChar + self.listCategories[item[1]],
    ...
    def link(self, szLink):
        if szLink == "PEDIA_MAIN_TECH":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_TECH), True
            )
        if szLink == "PEDIA_MAIN_UNIT":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_UNIT), True
            )
        if szLink == "PEDIA_MAIN_BUILDING":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_BUILDING), True
            )
        if szLink == "PEDIA_MAIN_TERRAIN":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_TERRAIN), True
            )
        if szLink == "PEDIA_MAIN_FEATURE":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_FEATURE), True
            )
        if szLink == "PEDIA_MAIN_BONUS":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_BONUS), True
            )
        if szLink == "PEDIA_MAIN_IMPROVEMENT":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN,
                int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_IMPROVEMENT),
                True,
            )
        if szLink == "PEDIA_MAIN_SPECIALIST":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN,
                int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_SPECIALIST),
                True,
            )
        if szLink == "PEDIA_MAIN_PROMOTION":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN,
                int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_PROMOTION),
                True,
            )
        if szLink == "PEDIA_MAIN_UNIT_GROUP":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN,
                int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_UNIT_GROUP),
                True,
            )
        if szLink == "PEDIA_MAIN_CIV":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_CIV), True
            )
        if szLink == "PEDIA_MAIN_LEADER":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_LEADER), True
            )
        if szLink == "PEDIA_MAIN_RELIGION":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_RELIGION), True
            )
        if szLink == "PEDIA_MAIN_CORPORATION":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN,
                int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_CORPORATION),
                True,
            )
        if szLink == "PEDIA_MAIN_CIVIC":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_CIVIC), True
            )
        if szLink == "PEDIA_MAIN_PROJECT":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_PROJECT), True
            )
        if szLink == "PEDIA_MAIN_CONCEPT":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT), True
            )
        if szLink == "PEDIA_MAIN_HINTS":
            return self.pediaJump(
                CvScreenEnums.PEDIA_MAIN, int(CivilopediaPageTypes.CIVILOPEDIA_PAGE_HINTS), True
            )

        for i in range(gc.getNumConceptInfos()):
            if gc.getConceptInfo(i).isMatchForLink(szLink, False):
                iEntryId = self.pediaHistorical.getIdFromEntryInfo(
                    CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT, i
                )
