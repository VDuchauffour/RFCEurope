...
#####################################
	...
	def checkTurn(self, iGameTurn):
		...
		if (iGameTurn > xml.i700AD and iGameTurn < xml.i800AD and iGameTurn % 2 == 1):
			if ( gc.getGame().getSorenRandNum(100, 'Spread chance') < 28 ):
				tCity = self.selectRandomCityArea(tMaghrebAndalusia, True)
				if (tCity != 0):
					self.spreadReligion(tCity,xml.iIslam)
		...
		if (iGameTurn > xml.i700AD and iGameTurn < xml.i800AD and iGameTurn % 3 == 1):
			if ( gc.getGame().getSorenRandNum(100, 'Spread chance') < 25 ):
				tCity = self.selectRandomCityArea(tBulgariaBalkans, True)
				if tCity:
		...
		if (iGameTurn > xml.i852AD and iGameTurn < xml.i1300AD and iGameTurn % 4 == 3):
			if ( gc.getGame().getSorenRandNum(100, 'Spread chance') < 25 ):
				tCity = self.selectRandomCityArea(tOldRus, True)
				if tCity:
					self.spreadReligion(tCity,xml.iOrthodoxy)
	...
	def selectRandomCityArea(self, tProvinces, bNoReligion = False):
		cityList = []
		for iPlayer in range(con.iNumTotalPlayersB):
				...
				if tProvinceMap[city.getY()][city.getX()] in tProvinces:
					...
					for iReligion in range(xml.iNumReligions):
						if city.isHasReligion(iReligion):
							bHasReligion = bHasReligion
							break
					if not (bNoReligion and bHasReligion):
...
##REFORMATION
	...
	def reformationchoice(self, iCiv):
		if iCiv == con.iPope: return # Absinthe: totally exclude the Pope from the Reformation
		...
		for city in utils.getCityList(iCiv):
			if city.isHasReligion(xml.iCatholicism) and not city.isHasReligion(xml.iProtestantism):
				...
				if rndnum <= 25 + (lReformationMatrix[iCiv] / 2): # only add the religion, chance between 30% and 70%, based on lReformationMatrix
					city.setHasReligion(xml.iProtestantism, True, False, False)
	...
	def reformationReformCity(self, pCity, iCiv):
		...
		if gc.getGame().getSorenRandNum(100, 'Religion spread to City') < 50 + iCivRef + iPopBonus:
			...
			if pCity.hasBuilding(xml.iCatholicChapel) and gc.getGame().getSorenRandNum(100, 'Reformation of a City') < iChance:
				pCity.setHasRealBuilding(xml.iCatholicChapel, False)
				pCity.setHasRealBuilding(xml.iProtestantChapel, True)
			if pCity.hasBuilding(xml.iCatholicTemple) and gc.getGame().getSorenRandNum(100, 'Reformation of a City') < iChance:
				pCity.setHasRealBuilding(xml.iCatholicTemple, False)
				pCity.setHasRealBuilding(xml.iProtestantTemple, True)
				iFaith += 1
			if pCity.hasBuilding(xml.iCatholicMonastery) and gc.getGame().getSorenRandNum(100, 'Reformation of a City') < iChance:
		...
		if gc.getGame().getSorenRandNum(100, 'Religion spread to City') < 20 + iCivRef + iPopBonus:
			...
			if gc.getGame().getSorenRandNum(100, 'Remove Religion') < 50 + ((lReformationMatrix[iCiv] / 5) * 2) - (iPopBonus / 2): # range goes from 39-54% to 71-86%, based on lReformationMatrix
				...
				if not bCathBuildings:
					...
					if pPlayer.isHuman(): # message for the human player
						CityName = pCity.getNameKey()
						if pPlayer.getStateReligion() == xml.iPlayerslam:
			...
			if pNbr.isAlive() and pNbr.getStateReligion() == xml.iProtestantism:
				...
				if ( iX == -1 or iY == -1 ):
					if pNbr.getNumCities() > 0:
						pNCapital = utils.getRandomEntry(utils.getCityList(lReformationNeighbours[iPlayer][iNbr]))
						iX = pNCapital.getX()
						iY = pNCapital.getY()
					else:
						return
...
### End Reformation ###
	...
	def resettleRefugies( self ):
		...
		for iPlayer in range( con.iNumTotalPlayersB ):
			...
			if pPlayer.isAlive():
				if iPlayer < con.iPope:
					...
					intolerance[iPlayer] += gc.getGame().getSorenRandNum(100, 'roll to randomize the migration of refugies')
					intolerance[iPlayer] += 10*pPlayer.getProsecutionCount()
					if pPlayer.getProsecutionCount() == 0:
						intolerance[iPlayer] = max( 0, intolerance[iPlayer] - 30 ) # if this player doesn't prosecute, decrease intolerance
		...
		if iCandidate2 == iCandidate1:
			iCandidate2 = 1
		for iPlayer in range( con.iNumTotalPlayersB ):
			if intolerance[iPlayer] > -1 and iPlayer != iCandidate1 and intolerance[iPlayer] < intolerance[iCandidate1]:
				iCandidate2 = iPlayer
	...
	def migrateJews( self, iPlayer ):
		...
		lCityList = [city for city in utils.getCityList(iPlayer) if not city.isHasReligion(xml.iJudaism)]
