...
class Resources:
	...
	def createResource(self, iX, iY, iBonus, textKey="TXT_KEY_RESOURCE_DISCOVERED"):
		...
		if gc.getMap().plot(iX, iY).getBonusType(-1) == -1 or iBonus == -1: # Absinthe: only proceed if there isn't any bonus resources on the plot, or if we're removing the bonus
			if iBonus == -1:
			...
			if iOwner >= 0 and textKey != -1: # Absinthe: only show alert to the tile owner
				...
				if not city.isNone():
					...
					CyInterface().addMessage(iOwner, False, con.iDuration, szText, "AS2D_DISCOVERBONUS", InterfaceMessageTypes.MESSAGE_TYPE_MINOR_EVENT, gc.getBonusInfo(iBonus).getButton(), ColorTypes(con.iLime), iX, iY, True, True)
	...
	def removeResource(self, iX, iY, textKey="TXT_KEY_RESOURCE_EXHAUSTED"):
		...
		if gc.getMap().plot(iX, iY).getBonusType(-1) != -1: # only proceed if there is a bonus resource on the plot
			...
			elif iBonusType == xml.iNorthAccess and iImprovementType == xml.iImprovementColonialTrade:
				gc.getMap().plot(iX, iY).setImprovementType(-1)
	...
	def checkTurn(self, iGameTurn):
		...
		if iGameTurn == xml.i552AD:
			...
			self.createResource(55, 35, xml.iRice) #Rice in Italy - represents trade of the merchant republics
