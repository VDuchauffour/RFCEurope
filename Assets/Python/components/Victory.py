...
class Victory:
    def __init__(self):
        self.switchConditionsPerCiv = {
            Civ.BYZANTIUM: self.checkByzantium,
            Civ.FRANCE: self.checkFrankia,
            Civ.ARABIA: self.checkArabia,
            Civ.BULGARIA: self.checkBulgaria,
            Civ.CORDOBA: self.checkCordoba,
            Civ.VENECIA: self.checkVenecia,
            Civ.BURGUNDY: self.checkBurgundy,
            Civ.GERMANY: self.checkGermany,
            Civ.NOVGOROD: self.checkNovgorod,
            Civ.NORWAY: self.checkNorway,
            Civ.KIEV: self.checkKiev,
            Civ.HUNGARY: self.checkHungary,
            Civ.CASTILE: self.checkSpain,
            Civ.DENMARK: self.checkDenmark,
            Civ.SCOTLAND: self.checkScotland,
            Civ.POLAND: self.checkPoland,
            Civ.GENOA: self.checkGenoa,
            Civ.MOROCCO: self.checkMorocco,
            Civ.ENGLAND: self.checkEngland,
            Civ.PORTUGAL: self.checkPortugal,
            Civ.ARAGON: self.checkAragon,
            Civ.SWEDEN: self.checkSweden,
            Civ.PRUSSIA: self.checkPrussia,
            Civ.LITHUANIA: self.checkLithuania,
            Civ.AUSTRIA: self.checkAustria,
            Civ.OTTOMAN: self.checkTurkey,
            Civ.MOSCOW: self.checkMoscow,
            Civ.DUTCH: self.checkDutch,
        }
    ...
    def onReligionFounded(self, iReligion, iFounder):
        ...
        if iReligion == Religion.PROTESTANTISM:
            if iFounder == Civ.GERMANY:
                self.wonUHV(Civ.GERMANY, 1)
            else:
                self.lostUHV(Civ.GERMANY, 1)
    ...
    def onTechAcquired(self, iTech, iPlayer):
        ...
        if iTech == Technology.INDUSTRIAL_TECH:
            if self.isPossibleUHV(Civ.ENGLAND, 2, False):
                if iPlayer == Civ.ENGLAND:
                    self.wonUHV(Civ.ENGLAND, 2)
                else:
                    self.lostUHV(Civ.ENGLAND, 2)
    ...
    def onBuildingBuilt(self, iPlayer, iBuilding):
        ...
        if iBuilding in tCordobaWonders:
            if self.isPossibleUHV(Civ.CORDOBA, 1, False):
                if iPlayer == Civ.CORDOBA:
                    ...
                    if iWondersBuilt == 2:  # so we already had 2 wonders, and this is the 3rd one
                        self.wonUHV(Civ.CORDOBA, 1)
                else:
                    self.lostUHV(Civ.CORDOBA, 1)
        ...
        if iBuilding in tOttomanWonders:
            if self.isPossibleUHV(Civ.OTTOMAN, 1, False):
                if iPlayer == Civ.OTTOMAN:
                    ...
                    if iWondersBuilt == 3:  # so we already had 3 wonders, and this is the 4th one
                        self.wonUHV(Civ.OTTOMAN, 1)
                else:
                    self.lostUHV(Civ.OTTOMAN, 1)
    ...
    def onProjectBuilt(self, iPlayer, iProject):
        ...
        if self.isPossibleUHV(Civ.VENECIA, 2, False):
            if iProject != Colony.VINLAND:
                if bColony:
                    if iPlayer == Civ.VENECIA:
                        self.wonUHV(Civ.VENECIA, 2)
                    else:
                        self.lostUHV(Civ.VENECIA, 2)
    ...
    def checkByzantium(self, iGameTurn):
        ...
        if iGameTurn == year(632):
            if self.isPossibleUHV(Civ.BYZANTIUM, 0, True):
                iNumCities = 0
                for iProv in tByzantiumControl:
                    iNumCities += player(Civ.BYZANTIUM).getProvinceCityCount(iProv)
                if iNumCities >= 6:
                    self.wonUHV(Civ.BYZANTIUM, 0)
                else:
                    self.lostUHV(Civ.BYZANTIUM, 0)
        ...
        # UHV 2: Control Constantinople, Thrace, Thessaloniki, Moesia, Macedonia, Serbia, Arberia, Epirus, Thessaly, Morea, Colonea, Antiochia, Charsianon, Cilicia, Armeniakon, Anatolikon, Paphlagonia, Thrakesion and Opsikion in 1282
            ...
            if self.isPossibleUHV(Civ.BYZANTIUM, 1, True):
                if self.checkProvincesStates(Civ.BYZANTIUM, tByzantiumControlII):
                    self.wonUHV(Civ.BYZANTIUM, 1)
                else:
                    self.lostUHV(Civ.BYZANTIUM, 1)
        ...
        # UHV 3: Make Constantinople the largest and most cultured city while being the richest empire in the world in 1453
            ...
            if self.isPossibleUHV(Civ.BYZANTIUM, 2, True):
                ...
                if (
                    ...
                    self.wonUHV(Civ.BYZANTIUM, 2)
                else:
                    self.lostUHV(Civ.BYZANTIUM, 2)

    def checkFrankia(self, iGameTurn):

        # UHV 1: Achieve Charlemagne's Empire by 840
        if self.isPossibleUHV(Civ.FRANCE, 0, True):
            if self.checkProvincesStates(Civ.FRANCE, tFrankControl):
                self.wonUHV(Civ.FRANCE, 0)
        if iGameTurn == year(840):
            self.expireUHV(Civ.FRANCE, 0)

        # UHV 2: Control Jerusalem in 1291
        elif iGameTurn == year(1291):
            if self.isPossibleUHV(Civ.FRANCE, 1, True):
                pJPlot = gc.getMap().plot(*CITIES[City.JERUSALEM])
                if pJPlot.isCity():
                    if pJPlot.getPlotCity().getOwner() == Civ.FRANCE:
                        self.wonUHV(Civ.FRANCE, 1)
                    else:
                        self.lostUHV(Civ.FRANCE, 1)
                else:
                    self.lostUHV(Civ.FRANCE, 1)
    ...
    def checkArabia(self, iGameTurn):
        ...
        if iGameTurn == year(850):
            if self.isPossibleUHV(Civ.ARABIA, 0, True):
                if self.checkProvincesStates(Civ.ARABIA, tArabiaControlI):
                    self.wonUHV(Civ.ARABIA, 0)
                else:
                    self.lostUHV(Civ.ARABIA, 0)

        # UHV 2: Control the Levant and Egypt in 1291AD while being the most advanced civilization
        elif iGameTurn == year(1291):
            if self.isPossibleUHV(Civ.ARABIA, 1, True):
                iMostAdvancedCiv = getMostAdvancedCiv()
                if (
                    self.checkProvincesStates(Civ.ARABIA, tArabiaControlII)
                    and iMostAdvancedCiv == Civ.ARABIA
                ):
                    self.wonUHV(Civ.ARABIA, 1)
                else:
                    self.lostUHV(Civ.ARABIA, 1)

        # UHV 3: Spread Islam to at least 35% of the population of Europe
        if self.isPossibleUHV(Civ.ARABIA, 2, True):
            iPerc = gc.getGame().calculateReligionPercent(Religion.ISLAM)
            if iPerc >= 35:
                self.wonUHV(Civ.ARABIA, 2)
    ...
    def checkBulgaria(self, iGameTurn):
        ...
        if self.isPossibleUHV(Civ.BULGARIA, 0, True):
            if self.checkProvincesStates(Civ.BULGARIA, tBulgariaControl):
                self.wonUHV(Civ.BULGARIA, 0)
        if iGameTurn == year(917):
            self.expireUHV(Civ.BULGARIA, 0)

        # UHV 2: Accumulate at least 100 Orthodox Faith Points by 1259
        if self.isPossibleUHV(Civ.BULGARIA, 1, True):
            if (
                civilization(Civ.BULGARIA).has_state_religion(Religion.ORTHODOXY)
                and player(Civ.BULGARIA).getFaith() >= 100
            ):
                self.wonUHV(Civ.BULGARIA, 1)
        if iGameTurn == year(1259):
            self.expireUHV(Civ.BULGARIA, 1)
            ...
            if self.isPossibleUHV(Civ.CORDOBA, 2, True):
                ...
                for iProv in tCordobaIslamize:
                    if not player(Civ.CORDOBA).provinceIsSpreadReligion(iProv, Religion.ISLAM):
                        ...
                        break
                if bIslamized:
                    self.wonUHV(Civ.CORDOBA, 2)
                else:
                    self.lostUHV(Civ.CORDOBA, 2)
            ...
            if self.isPossibleUHV(Civ.NORWAY, 2, True):
                ...
                for iTestPlayer in tNorwayOutrank:
                    if gc.getGame().getTeamRank(iTestPlayer) < iNorwayRank:
                        ...
                        break
                if bIsOnTop:
                    self.wonUHV(Civ.NORWAY, 2)
                else:
                    self.lostUHV(Civ.NORWAY, 2)
    ...
    def checkDenmark(self, iGameTurn):
        ...
        if iGameTurn == year(1050):
            if self.isPossibleUHV(Civ.DENMARK, 0, True):
                if self.checkProvincesStates(Civ.DENMARK, tDenmarkControlI):
                    self.wonUHV(Civ.DENMARK, 0)
                else:
                    self.lostUHV(Civ.DENMARK, 0)
        ...
        # UHV 2: Control Denmark, Norway, Vestfold, Skaneland, G�taland, Svealand, Norrland, Gotland, �sterland, Estonia and Iceland in 1523
            ...
            if self.isPossibleUHV(Civ.DENMARK, 1, True):
                if self.checkProvincesStates(Civ.DENMARK, tDenmarkControlIII):
                    self.wonUHV(Civ.DENMARK, 1)
                else:
                    self.lostUHV(Civ.DENMARK, 1)
    ...
    def checkVenecia(self, iGameTurn):
        ...
        if self.isPossibleUHV(Civ.VENECIA, 0, True):
            if self.checkProvincesStates(Civ.VENECIA, tVenetianControl):
                self.wonUHV(Civ.VENECIA, 0)
        if iGameTurn == year(1004):
            self.expireUHV(Civ.VENECIA, 0)

        # UHV 2: Conquer Constantinople, Thessaly, Morea, Crete and Cyprus by 1204
        if self.isPossibleUHV(Civ.VENECIA, 1, True):
            if (
                player(Civ.VENECIA).getProvinceCurrentState(Province.CONSTANTINOPLE)
                >= ProvinceStatus.CONQUER
            ):
                if self.checkProvincesStates(Civ.VENECIA, tVenetianControlII):
                    self.wonUHV(Civ.VENECIA, 1)
        if iGameTurn == year(1204):
            self.expireUHV(Civ.VENECIA, 1)
    ...
    def checkBurgundy(self, iGameTurn):
        ...
        if iGameTurn < year(1336) + 2:
            ...
            if self.isPossibleUHV(Civ.BURGUNDY, 0, True):
                if iCulture >= 12000:
                    self.wonUHV(Civ.BURGUNDY, 0)
        if iGameTurn == year(1336):
            self.expireUHV(Civ.BURGUNDY, 0)

        # UHV 2: Control Burgundy, Provence, Picardy, Flanders, Champagne and Lorraine in 1376
        elif iGameTurn == year(1376):
            if self.isPossibleUHV(Civ.BURGUNDY, 1, True):
                if self.checkProvincesStates(Civ.BURGUNDY, tBurgundyControl):
                    self.wonUHV(Civ.BURGUNDY, 1)
                else:
                    self.lostUHV(Civ.BURGUNDY, 1)

        # UHV 3: Have a higher score than France, England and Germany in 1473
        elif iGameTurn == year(1473):
            if self.isPossibleUHV(Civ.BURGUNDY, 2, True):
                ...
                for iTestPlayer in tBurgundyOutrank:
                    if gc.getGame().getTeamRank(iTestPlayer) < iBurgundyRank:
                        ...
                        break
                if bIsOnTop:
                    self.wonUHV(Civ.BURGUNDY, 2)
                else:
                    self.lostUHV(Civ.BURGUNDY, 2)
    ...
    def checkGermany(self, iGameTurn):
        ...
        if iGameTurn == year(1167):
            if self.isPossibleUHV(Civ.GERMANY, 0, True):
                if self.checkProvincesStates(Civ.GERMANY, tGermanyControl):
                    self.wonUHV(Civ.GERMANY, 0)
                else:
                    self.lostUHV(Civ.GERMANY, 0)
        ...
        # UHV 3: Control Swabia, Saxony, Bavaria, Franconia, Brandenburg, Holstein, Flanders, Pomerania, Silesia, Bohemia, Moravia and Austria in 1648
            ...
            if self.isPossibleUHV(Civ.GERMANY, 2, True):
                if self.checkProvincesStates(Civ.GERMANY, tGermanyControlII):
                    self.wonUHV(Civ.GERMANY, 2)
                else:
                    self.lostUHV(Civ.GERMANY, 2)
    ...
    def checkKiev(self, iGameTurn):
        ...
        if iGameTurn < year(1300) + 2:
            ...
            if self.isPossibleUHV(Civ.KIEV, 2, True):
                if iFood > 25000:
                    self.wonUHV(Civ.KIEV, 2)
        if iGameTurn == year(1300):
            self.expireUHV(Civ.KIEV, 2)
    ...
    def checkHungary(self, iGameTurn):
        ...
        if iGameTurn == year(1490):
            if self.isPossibleUHV(Civ.HUNGARY, 0, True):
                if self.checkProvincesStates(Civ.HUNGARY, tHungaryControl):
                    self.wonUHV(Civ.HUNGARY, 0)
                else:
                    self.lostUHV(Civ.HUNGARY, 0)

        # UHV 2: Allow no Ottoman cities in Europe in 1541
        elif iGameTurn == year(1541):
            if self.isPossibleUHV(Civ.HUNGARY, 1, True):
                bClean = True
    ...
    def checkSpain(self, iGameTurn):
        ...
        if iGameTurn == year(1492):
            if self.isPossibleUHV(Civ.CASTILE, 0, True):
                bConverted = True
                for iProv in tSpainConvert:
                    if not player(Civ.CASTILE).provinceIsConvertReligion(
                        iProv, Religion.CATHOLICISM
                    ):
                        bConverted = False
                        break
                if bConverted:
                    self.wonUHV(Civ.CASTILE, 0)
                else:
                    self.lostUHV(Civ.CASTILE, 0)

        # UHV 2: Have more Colonies than any other nation in 1588, while having at least 3
        elif iGameTurn == year(1588):
            if self.isPossibleUHV(Civ.CASTILE, 1, True):
                bMost = True
                iSpainColonies = self.getNumRealColonies(Civ.CASTILE)
                for iPlayer in civilizations().majors().ids():
    ...
    def checkScotland(self, iGameTurn):
        ...
        if self.isPossibleUHV(Civ.SCOTLAND, 0, True):
        ...
        if self.isPossibleUHV(Civ.SCOTLAND, 1, True):
            if civilization(Civ.FRANCE).is_alive():
                ...
                if iNewScore >= 1500:
                    self.wonUHV(Civ.SCOTLAND, 1)
        if iGameTurn == year(1560):
            self.expireUHV(Civ.SCOTLAND, 1)

        # UHV 3: Control Scotland, The Isles, Ireland, Wales, Brittany and Galicia in 1700
        elif iGameTurn == year(1700):
            if self.isPossibleUHV(Civ.SCOTLAND, 2, True):
                if self.checkProvincesStates(Civ.SCOTLAND, tScotlandControl):
                    self.wonUHV(Civ.SCOTLAND, 2)
                else:
                    self.lostUHV(Civ.SCOTLAND, 2)
    ...
    def checkPoland(self, iGameTurn):
        ...
        if year(1500) <= iGameTurn <= year(1520):
            if self.isPossibleUHV(Civ.POLAND, 0, True):
    ...
    def checkGenoa(self, iGameTurn):
        ...
        if iGameTurn == year(1400):
            if self.isPossibleUHV(Civ.GENOA, 0, True):
                if self.checkProvincesStates(Civ.GENOA, tGenoaControl):
                    self.wonUHV(Civ.GENOA, 0)
                else:
                    self.lostUHV(Civ.GENOA, 0)
        ...
        # UHV 2: Export is based on your cities' trade routes with foreign cities, import is based on foreign cities' trade routes with your cities
            ...
            if self.isPossibleUHV(Civ.GENOA, 1, True):
    ...
    def checkMorocco(self, iGameTurn):
        ...
        if iGameTurn == year(1248):
            if self.isPossibleUHV(Civ.MOROCCO, 0, True):
                if self.checkProvincesStates(Civ.MOROCCO, tMoroccoControl):
                    self.wonUHV(Civ.MOROCCO, 0)
                else:
                    self.lostUHV(Civ.MOROCCO, 0)

        # UHV 2: Have 5000 culture in each of three cities in 1465
        elif iGameTurn == year(1465):
            if self.isPossibleUHV(Civ.MOROCCO, 1, True):
                iGoodCities = 0
                for city in cities().owner(Civ.MOROCCO).entities():
                    if city.getCulture(Civ.MOROCCO) >= 5000:
                        iGoodCities += 1
                if iGoodCities >= 3:
                    self.wonUHV(Civ.MOROCCO, 1)
                else:
                    self.lostUHV(Civ.MOROCCO, 1)

        # UHV 3: Destroy or vassalize Portugal, Spain, and Aragon by 1578
        if year(1164) <= iGameTurn <= year(1578):
            if self.isPossibleUHV(Civ.MOROCCO, 2, True):
                bConq = True
                if (
                    (
                ...
                if bConq:
                    self.wonUHV(Civ.MOROCCO, 2)
        if iGameTurn == year(1578) + 1:
            self.expireUHV(Civ.MOROCCO, 2)
    ...
    def checkEngland(self, iGameTurn):
        ...
        if iGameTurn == year(1452):
            if self.isPossibleUHV(Civ.ENGLAND, 0, True):
                if self.checkProvincesStates(Civ.ENGLAND, tEnglandControl):
                    self.wonUHV(Civ.ENGLAND, 0)
                else:
                    self.lostUHV(Civ.ENGLAND, 0)
    ...
    def checkPortugal(self, iGameTurn):
        ...
        if iGameTurn == year(1640):
            if self.isPossibleUHV(Civ.PORTUGAL, 1, True):
                self.wonUHV(Civ.PORTUGAL, 1)
    ...
    def checkAragon(self, iGameTurn):
        ...
        if iGameTurn == year(1282):
            if self.isPossibleUHV(Civ.ARAGON, 0, True):
                if self.checkProvincesStates(Civ.ARAGON, tAragonControlI):
                    self.wonUHV(Civ.ARAGON, 0)
                else:
                    self.lostUHV(Civ.ARAGON, 0)
        ...
        # UHV 2: Ships with at least one cargo space count as Trade Ships
            ...
            if self.isPossibleUHV(Civ.ARAGON, 1, True):
                ...
                if iPorts >= 12 and iCargoShips >= 30:
                    self.wonUHV(Civ.ARAGON, 1)
                else:
                    self.lostUHV(Civ.ARAGON, 1)
        ...
        # UHV 3: Control Catalonia, Valencia, Aragon, Balears, Corsica, Sardinia, Sicily, Calabria, Apulia, Provence and Thessaly in 1474
            ...
            if self.isPossibleUHV(Civ.ARAGON, 2, True):
                if self.checkProvincesStates(Civ.ARAGON, tAragonControlII):
                    self.wonUHV(Civ.ARAGON, 2)
                else:
                    self.lostUHV(Civ.ARAGON, 2)
    ...
    def checkPrussia(self, iGameTurn):
        ...
        if iGameTurn == year(1410):
            if self.isPossibleUHV(Civ.PRUSSIA, 0, True):
                if self.checkProvincesStates(Civ.PRUSSIA, tPrussiaControlI):
                    self.wonUHV(Civ.PRUSSIA, 0)
                else:
                    self.lostUHV(Civ.PRUSSIA, 0)
        ...
        if self.isPossibleUHV(Civ.PRUSSIA, 2, True):
    ...
    def checkLithuania(self, iGameTurn):
        ...
        if iGameTurn < year(1386) + 2:
            ...
            if self.isPossibleUHV(Civ.LITHUANIA, 0, True):
                if civilization(Civ.LITHUANIA).has_a_state_religion():
                    self.lostUHV(Civ.LITHUANIA, 0)
                else:
                    if iCulture >= 2500:
                        self.wonUHV(Civ.LITHUANIA, 0)
        if iGameTurn == year(1386):
            self.expireUHV(Civ.LITHUANIA, 0)

        # UHV 2: Control the most territory in Europe in 1430
        elif iGameTurn == year(1430):
            if self.isPossibleUHV(Civ.LITHUANIA, 1, True):
                bMost = True
                ...
                for iOtherPlayer in civilizations().majors().ids():
                    ...
                    if iOtherCount >= iCount:
                        ...
                        break
                if bMost:
                    self.wonUHV(Civ.LITHUANIA, 1)
                else:
                    self.lostUHV(Civ.LITHUANIA, 1)

        # UHV 3: Destroy or Vassalize Muscovy, Novgorod and Prussia by 1795
        if year(1380) <= iGameTurn <= year(1795):
            if self.isPossibleUHV(Civ.LITHUANIA, 2, True):
                bConq = True
                if (
                    (
                ...
                if bConq:
                    self.wonUHV(Civ.LITHUANIA, 2)
        if iGameTurn == year(1795) + 1:
            self.expireUHV(Civ.LITHUANIA, 2)

    def checkAustria(self, iGameTurn):

        # UHV 1: Control all of medieval Austria, Hungary and Bohemia in 1617
        if iGameTurn == year(1617):
            if self.isPossibleUHV(Civ.AUSTRIA, 0, True):
                if self.checkProvincesStates(Civ.AUSTRIA, tAustriaControl):
                    self.wonUHV(Civ.AUSTRIA, 0)
                else:
                    self.lostUHV(Civ.AUSTRIA, 0)

        # UHV 2: Have 3 vassals in 1700
        elif iGameTurn == year(1700):
            if self.isPossibleUHV(Civ.AUSTRIA, 1, True):
                iCount = 0
                for iPlayer in civilizations().majors().ids():
                    ...
                    if pPlayer.isAlive():
                        if gc.getTeam(pPlayer.getTeam()).isVassal(team(Civ.AUSTRIA).getID()):
                            iCount += 1
                if iCount >= 3:
                    self.wonUHV(Civ.AUSTRIA, 1)
                else:
                    self.lostUHV(Civ.AUSTRIA, 1)

        # UHV 3: Have the highest score in 1780
        elif iGameTurn == year(1780):
            if self.isPossibleUHV(Civ.AUSTRIA, 2, True):
                if gc.getGame().getTeamRank(Civ.AUSTRIA) == 0:
                    self.wonUHV(Civ.AUSTRIA, 2)
                else:
                    self.lostUHV(Civ.AUSTRIA, 2)
    ...
    def checkTurkey(self, iGameTurn):
        ...
        if iGameTurn == year(1517):
            if self.isPossibleUHV(Civ.OTTOMAN, 0, True):
                if self.checkProvincesStates(Civ.OTTOMAN, tOttomanControlI):
                    self.wonUHV(Civ.OTTOMAN, 0)
                else:
                    self.lostUHV(Civ.OTTOMAN, 0)
        ...
        # Controlled in the onBuildingBuilt function
            ...
            self.expireUHV(Civ.OTTOMAN, 1)

        # UHV 3: Conquer Austria, Pannonia and Lesser Poland by 1683
        if self.isPossibleUHV(Civ.OTTOMAN, 2, True):
            if self.checkProvincesStates(Civ.OTTOMAN, tOttomanControlII):
                self.wonUHV(Civ.OTTOMAN, 2)
        if iGameTurn == year(1683):
            self.expireUHV(Civ.OTTOMAN, 2)
    ...
    def checkMoscow(self, iGameTurn):
        ...
        if iGameTurn == year(1482):
            if self.isPossibleUHV(Civ.MOSCOW, 0, True):
                bClean = True
                for iProv in tMoscowControl:
                    if civilizations().barbarian().unwrap().player.getProvinceCityCount(iProv) > 0:
                        ...
                        break
                if bClean:
                    self.wonUHV(Civ.MOSCOW, 0)
                else:
                    self.lostUHV(Civ.MOSCOW, 0)

        # UHV 2: Control at least 25% of Europe
        if self.isPossibleUHV(Civ.MOSCOW, 1, True):
            totalLand = gc.getMap().getLandPlots()
    ...
    def checkSweden(self, iGameTurn):
        ...
        if iGameTurn == year(1323):
            if self.isPossibleUHV(Civ.SWEDEN, 0, True):
                iNumCities = 0
                for iProv in tSwedenControl:
                    iNumCities += player(Civ.SWEDEN).getProvinceCityCount(iProv)
                if iNumCities >= 6:
                    self.wonUHV(Civ.SWEDEN, 0)
                else:
                    self.lostUHV(Civ.SWEDEN, 0)
    ...
    def checkDutch(self, iGameTurn):
        ...
        if self.isPossibleUHV(Civ.DUTCH, 0, True):
