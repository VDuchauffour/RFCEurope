...
class CvCivicsScreen:
    ...
    def __init__(self):
        ...
        self.W_CIVIC_CATEGORY = (
            (self.W_SCREEN - self.X_CIVIC_CATEGORY) / self.nColumns
        ) - self.MARGIN
        ...
        self.X_MAINTENANCE = (self.W_BOTTOM_PANEL / 3) * 2
    ...
    def interfaceScreen(self):
        ...
        screen.setDimensions(
            (screen.getXResolution() / 2) - (self.W_SCREEN / 2),
            screen.centerY(0),
            self.W_SCREEN,
            self.H_SCREEN,
        )
        ...
        screen.addDDSGFC(
            "CivicsBackground",
            CyArtFileMgr().getInterfaceArtInfo("MAINMENU_SLIDESHOW_LOAD").getPath(),
            0,
            0,
            self.W_SCREEN,
            self.H_SCREEN,
        ...
        screen.setText(
            "RevolutionButton",
            "Background",
            u"<font=4>REVOLUTION</font>",
            CvUtil.FONT_RIGHT_JUSTIFY,
            self.X_REVOLUTION,
            self.Y_REVOLUTION,
            0,
            FontTypes.TITLE_FONT,
            WidgetTypes.WIDGET_REVOLUTION,
            1,
            0,
        )
        screen.setText(
    ...
    def placeContents(self):
        ...
        for i in xrange(len(self.Categories)):
            ...
            screen.addDDSGFC(
                sName,
                CyArtFileMgr().getInterfaceArtInfo("SCREEN_BG").getPath(),
                iX,
                iY,
                self.W_CIVIC_CATEGORY,
                self.H_CIVIC_CATEGORY,
                WidgetTypes.WIDGET_GENERAL,
                -1,
                -1,
            )
            ...
            screen.addPanel(
                ...
                True,
                True,
                iX + self.BUTTON_LARGE + (iSpacing * 2),
                iY + self.MARGIN,
                self.W_CIVIC_TEXT,
                self.H_CIVIC_TEXT,
    ...
    def showCivic(self, iCategory):
        ...
        if gc.getCivicInfo(iCivic).getUpkeep() != -1 and not player.isNoCivicUpkeep(
            gc.getCivicInfo(iCivic).getCivicOptionType()
        ):
        ...
        screen.addMultilineText(
            ...
            sText,
            iX + iSpacing,
            iY + self.BUTTON_LARGE + 20,
            self.BUTTON_LARGE + iSpacing,
            self.H_CIVIC_CATEGORY - self.BUTTON_LARGE - (iSpacing * 2),
            WidgetTypes.WIDGET_GENERAL,
            -1,
            -1,
        ...
        sText += (
            u"<font=2>" + CyGameTextMgr().parseCivicInfo(iCivic, False, True, True) + "</font>"
        )
        screen.addMultilineText(
            sName,
            sText,
    ...
    def updateCivicOptions(self, iCategory):
        ...
        for iCivic in xrange(gc.getNumCivicInfos()):
            if gc.getCivicInfo(iCivic).getCivicOptionType() == iCategory:
                ...
                screen.setState(
                    "CivicButton" + str(iCivic), self.SelectedCivics[iCategory] == iCivic
                )
    ...
    def colorCivicTexts(self, iHoverCivic, bHoverOn):
        ...
        for iCivic in range(gc.getNumCivicInfos()):
            ...
            iX, iY = self.getPosition(iCategory)
            xPos = iX + self.W_CIVIC_CATEGORY - self.BUTTON_SMALL - self.MARGIN
            iLine = iY + self.MARGIN + (iCivic % 5) * self.LINE
    ...
    def handleInput(self, inputClass):
        ...
        elif inputClass.getFunctionName().startswith(
            "CivicButton"
        ) or inputClass.getFunctionName().startswith("CivicName"):
