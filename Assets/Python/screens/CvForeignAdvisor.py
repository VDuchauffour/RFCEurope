...
class CvForeignAdvisor:
    ...
    def getScreen(self):
        return CyGInterfaceScreen(
            self.SCREEN_NAME + str(self.iScreen), CvScreenEnums.FOREIGN_ADVISOR
        )
            ...
            for i in range(gc.getGame().getIndexAfterLastDeal()):
                ...
                if (
                    ...
                    screen.appendListBoxString(
                        dealPanelName,
                        CyGameTextMgr().getDealString(deal, iLoopPlayer),
                ...
                if self.iSelectedLeader == self.iActiveLeader:
                    ...
                    for iLoopPlayer in range(gc.getMAX_PLAYERS()):
                        if (
                            ...
                            and not gc.getPlayer(iLoopPlayer).isBarbarian()
                            and not gc.getPlayer(iLoopPlayer).isMinorCiv()
                            and gc.getTeam(gc.getPlayer(iLoopPlayer).getTeam()).isHasMet(
                                gc.getPlayer(self.iActiveLeader).getTeam()
                            )
                        ):
                            if iLoopPlayer != self.iActiveLeader and gc.getPlayer(
                                self.iActiveLeader
                            ).canTradeItem(iLoopPlayer, tradeData, False):
                                bTradeable = True
                                iLoopPlayer = gc.getMAX_PLAYERS()  # exit for loop
                else:
                    # display resources that you can trade to the selected leader
                    bTradeable = gc.getPlayer(self.iActiveLeader).canTradeItem(
                        self.iSelectedLeader, tradeData, False
                    )
            ...
            if (
                ...
                if self.iScreen == FOREIGN_BONUS_SCREEN:
                    if (
                        ...
                        screen.attachMultiListControlGFC(
                            currentPlayerPanelName,
                            "ChildTrade" + currentPlayerPanelName,
                            "",
                            1,
                            self.BUTTON_SIZE,
                            self.BUTTON_SIZE,
                        ...
                        for iLoopBonus in range(gc.getNumBonusInfos()):
                            tradeData.iData = iLoopBonus
                            if gc.getPlayer(iLoopPlayer).canTradeItem(
                                self.iActiveLeader, tradeData, False
                            ):
                                if (
                                    gc.getPlayer(iLoopPlayer).getTradeDenial(
                        ...
                        screen.attachMultiListControlGFC(
                            currentPlayerPanelName,
                            "ChildNoTrade" + currentPlayerPanelName,
                            "",
                            1,
                            self.BUTTON_SIZE,
                            self.BUTTON_SIZE,
                    ...
                    if (
                        ...
                        for iLoopTech in range(gc.getNumTechInfos()):
                            ...
                            elif gc.getTeam(gc.getPlayer(iLoopPlayer).getTeam()).isHasTech(
                                iLoopTech
                            ) and playerActive.canResearch(iLoopTech, False):
                        ...
                        screen.attachMultiListControlGFC(
                            currentPlayerPanelName,
                            "ChildCantTrade" + currentPlayerPanelName,
                            "",
                            1,
                            self.BUTTON_SIZE,
                            self.BUTTON_SIZE,
    ...
    def drawRelations(self, bInitial):
        ...
        for iPlayer in range(gc.getMAX_PLAYERS()):
            ...
            if (
                player.isAlive()
                and iPlayer != self.iActiveLeader
                and (
                    gc.getTeam(player.getTeam()).isHasMet(
                        gc.getPlayer(self.iActiveLeader).getTeam()
                    )
                    or gc.getGame().isDebugMode()
                )
        ...
        screen.addLineGFC(
            self.BACKGROUND_ID,
            self.getNextLineName(),
            x,
            y,
            x + self.W_LEGEND - 2 * self.MARGIN_LEGEND,
            y,
        ...
        screen.addLineGFC(
            ...
            x + self.W_LEGEND - 2 * self.MARGIN_LEGEND,
            y,
            gc.getInfoTypeForString("COLOR_BLUE"),
        )
        y += 2 * self.MARGIN_LEGEND
        screen.setLabel(
            self.getNextWidgetName(),
        ...
        screen.addLineGFC(
            ...
            x + self.W_LEGEND - 2 * self.MARGIN_LEGEND,
            y,
            gc.getInfoTypeForString("COLOR_CYAN"),
        )
        ...
        screen.addCheckBoxGFC(
            ...
            gc.getCivilizationInfo(
                gc.getPlayer(self.iActiveLeader).getCivilizationType()
            ).getButton(),
            ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
        ...
        screen.setLabel(
            ...
            szLeaderName,
            CvUtil.FONT_CENTER_JUSTIFY,
            self.X_LEADER_CIRCLE_TOP,
            fLeaderTop + iLeaderHeight + iDist,
            0,
            FontTypes.GAME_FONT,
            WidgetTypes.WIDGET_GENERAL,
            -1,
        ...
        for iPlayer in leaderMap.keys():
            ...
            fX = int(
                self.X_LEADER_CIRCLE_TOP
                - fRadius * math.cos(deltaTheta * leaderMap[iPlayer])
                - iLeaderWidth / 2
            )
            fY = int(
                fLeaderArcTop
                + fRadius * math.sin(deltaTheta * leaderMap[iPlayer])
                - iLeaderHeight / 2
            )
            ...
            screen.addCheckBoxGFC(
                szLeaderHead,
                gc.getCivilizationInfo(player.getCivilizationType()).getButton(),
                ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                int(fX),
                int(fY),
                iLeaderWidth,
            ...
            szName = self.getNextWidgetName()
            if (
                gc.getTeam(player.getTeam()).isHasMet(playerBase.getTeam())
                and iBaseLeader != iPlayer
            ):
                # Rhye - start
                ...
                if len(leaderMap.keys()) >= 16:
                    szText = (
                        ...
                        + " ("
                        + gc.getAttitudeInfo(
                            gc.getPlayer(iPlayer).AI_getAttitude(iBaseLeader)
                        ).getDescription()
                        + u"</font>"
                    )
                    if iBaseLeader != iPlayer:
        ...
        for iSelectedLeader in range(gc.getMAX_PLAYERS()):
            bDisplayed = (
                not gc.getPlayer(iSelectedLeader).isBarbarian()
                and not gc.getPlayer(iSelectedLeader).isMinorCiv()
                and gc.getPlayer(iSelectedLeader).isAlive()
                and (
                    gc.getGame().isDebugMode()
                    or gc.getTeam(playerActive.getTeam()).isHasMet(
                        gc.getPlayer(iSelectedLeader).getTeam()
                    )
                )
            )
            if iSelectedLeader in self.listSelectedLeaders or (bNoLeadersSelected and bDisplayed):
                ...
                for iPlayer in leaderMap.keys():
                    ...
                    fX = self.X_LEADER_CIRCLE_TOP - fRadius * math.cos(
                        deltaTheta * leaderMap[iPlayer]
                    )
                    ...
                    if iSelectedLeader != iPlayer:
                        ...
                        elif gc.getTeam(player.getTeam()).isVassal(
                            gc.getPlayer(iSelectedLeader).getTeam()
                        ) or gc.getTeam(gc.getPlayer(iSelectedLeader).getTeam()).isVassal(
                            player.getTeam()
                        ):
                            szName = self.getNextLineName()
                            screen.addLineGFC(
                        ...
                        elif gc.getTeam(player.getTeam()).isHasMet(
                            gc.getPlayer(iSelectedLeader).getTeam()
                        ):
                            if gc.getTeam(player.getTeam()).isAtWar(
                                gc.getPlayer(iSelectedLeader).getTeam()
                            ):
                                szName = self.getNextLineName()
                                screen.addLineGFC(
                                ...
                                bJustPeace = True
                                if gc.getTeam(player.getTeam()).isOpenBorders(
                                    gc.getPlayer(iSelectedLeader).getTeam()
                                ):
                                    fDy = fYSelected - fY
                                    fDx = fXSelected - fX
                                    ...
                                    screen.addLineGFC(
                                        self.BACKGROUND_ID,
                                        szName,
                                        int(fXSelected + fSecondLineOffsetX),
                                        int(fYSelected + fSecondLineOffsetY),
                                        int(fX + fSecondLineOffsetX),
                                        int(fY + fSecondLineOffsetY),
                                        gc.getInfoTypeForString("COLOR_CITY_GREEN"),
                                    )
                                    bJustPeace = False
                                if gc.getTeam(player.getTeam()).isDefensivePact(
                                    gc.getPlayer(iSelectedLeader).getTeam()
                                ):
                                    szName = self.getNextLineName()
                                    screen.addLineGFC(
                    ...
                    if gc.getTeam(player.getTeam()).isAtWar(
                        ...
                        if gc.getTeam(player.getTeam()).isOpenBorders(
                            ...
                            screen.addLineGFC(
                                ...
                                int(fXSelected + fSecondLineOffsetX),
                                int(fYSelected + fSecondLineOffsetY),
                                int(self.X_LEADER_CIRCLE_TOP + fSecondLineOffsetX),
                                int(fLeaderTop + iLeaderHeight / 2 + fSecondLineOffsetY),
                                gc.getInfoTypeForString("COLOR_CITY_GREEN"),
                            )
                            bJustPeace = False
                        if gc.getTeam(player.getTeam()).isDefensivePact(
                            ...
                            screen.addLineGFC(
                                ...
                                int(fXSelected),
                                int(fYSelected),
                                int(self.X_LEADER_CIRCLE_TOP),
                                int(fLeaderTop + iLeaderHeight / 2),
                                gc.getInfoTypeForString("COLOR_BLUE"),
                            )
                            bJustPeace = False
                        if bJustPeace:
    ...
    def handleInput(self, inputClass):
        if inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED:
            ...
            if inputClass.getFunctionName() + str(inputClass.getID()) == self.getWidgetName(
            ...
            if inputClass.getData() == int(InputTypes.KB_LSHIFT) or inputClass.getData() == int(
