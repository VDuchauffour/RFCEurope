...
class CvMilitaryAdvisor:
    ...
    def __init__(self, screenId):
        ...
        self.ATTACHED_WIDGET_ID = (
            "MilitaryAdvisorAttachedWidget"  # no need to explicitly delete these
        )
    ...
    def drawCombatExperience(self):
        ...
        if gc.getPlayer(self.iActivePlayer).greatPeopleThreshold(True) > 0:
            ...
            screen.addStackedBarGFC(
                ...
                self.X_GREAT_GENERAL_BAR,
                self.Y_GREAT_GENERAL_BAR,
                self.W_GREAT_GENERAL_BAR,
                self.H_GREAT_GENERAL_BAR,
            ...
            screen.setBarPercentage(
                ...
                InfoBarTypes.INFOBAR_STORED,
                float(iExperience)
                / float(gc.getPlayer(self.iActivePlayer).greatPeopleThreshold(True)),
            )
            screen.setLabel(
    ...
    def handleInput(self, inputClass):
        if (
            ...
            and inputClass.getFunctionName() == self.UNIT_BUTTON_ID
        ):
        ...
        for iUnit in range(gc.getNumUnitInfos()):
            if len(self.unitsList[iUnit][2]) > 0:
                if iPrevUnitCombat != self.unitsList[iUnit][0] and self.unitsList[iUnit][0] != -1:
                    ...
                    szDescription = (
                        gc.getUnitCombatInfo(self.unitsList[iUnit][0]).getDescription().upper()
                    )
                ...
                for loopUnit in self.unitsList[iUnit][2]:
                    ...
                    iColor = gc.getPlayerColorInfo(
                        gc.getPlayer(iPlayer).getPlayerColor()
                    ).getColorTypePrimary()
                    screen.setMinimapColor(
                        MinimapModeTypes.MINIMAPMODE_MILITARY,
                        loopUnit.getX(),
                        loopUnit.getY(),
                        iColor,
                        0.6,
                    )
                    if self.isSelectedUnit(loopUnit.getOwner(), loopUnit.getID(), True) and (
                        iPlayer in self.selectedPlayerList
                    ):

                        if player.getTeam().isAtWar(gc.getPlayer(self.iActivePlayer).getTeam()):
                            iColor = gc.getInfoTypeForString("COLOR_RED")
                        elif (
                            gc.getPlayer(iPlayer).getTeam()
    ...
    def refresh(self, bReload):
        ...
        for iLoopPlayer in range(gc.getMAX_PLAYERS()):
            ...
            if player.isAlive() and (
                ...
                # if (player.isAlive() and player.getID() != Consts.iIndependent and player.getID() != Consts.iIndependent2 and (gc.getTeam(player.getTeam()).isHasMet(gc.getPlayer(self.iActivePlayer).getTeam()) or gc.getGame().isDebugMode())):
        ...
        for iIndex in range(iNumLeaders):
            ...
            x = (
                self.X_LEADERS
                + self.LEADER_MARGIN
                + (iIndex % iColumns) * (iButtonSize + self.LEADER_MARGIN)
            )
            y = (
                self.Y_LEADERS
                + self.LEADER_MARGIN
            ...
            if bReload:
                if player.isBarbarian():
                    ...
                    szButton = gc.getCivilizationInfo(
                        gc.getPlayer(iLoopPlayer).getCivilizationType()
                    ).getButton()  # Rhye
                screen.addCheckBoxGFC(
                    self.getLeaderButton(iLoopPlayer),
                    szButton,
                ...
                screen.setState(
                    self.getLeaderButton(iLoopPlayer), (iLoopPlayer in self.selectedPlayerList)
                )
