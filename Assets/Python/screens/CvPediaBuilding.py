...
class CvPediaBuilding:
    ...
    def interfaceScreen(self, iBuilding):
        ...
        # Header...
        szHeader = (
            u"<font=4b>" + gc.getBuildingInfo(self.iBuilding).getDescription().upper() + u"</font>"
        )
        ...
        screen.setLabel(
            ...
            CivilopediaPageTypes.CIVILOPEDIA_PAGE_BUILDING,
            iBuilding,
        )
        ...
        screen.addPanel(
            ...
            False,
            False,
            self.X_BUILDING_PANE,
            self.Y_BUILDING_PANE,
            self.W_BUILDING_PANE,
            self.H_BUILDING_PANE,
        ...
        screen.addDDSGFC(
            self.top.getNextWidgetName(),
            gc.getBuildingInfo(self.iBuilding).getButton(),
        ...
        screen.addBuildingGraphicGFC(
            ...
            -1,
            -1,
            self.X_ROTATION_BUILDING_ANIMATION,
            self.Z_ROTATION_BUILDING_ANIMATION,
    ...
    def placeStats(self):
        ...
        for k in range(CommerceTypes.NUM_COMMERCE_TYPES):
            ...
            iTotalCommerce = buildingInfo.getObsoleteSafeCommerceChange(
                k
            ) + buildingInfo.getCommerceChange(k)
            if iTotalCommerce != 0:
                if iTotalCommerce > 0:
                    szSign = "+"
                else:
        ...
        if self.top.iActivePlayer != -1:
            if self.iBuilding == gc.getCivilizationInfo(
                ...
                iHappiness += gc.getPlayer(self.top.iActivePlayer).getExtraBuildingHappiness(
                    self.iBuilding
                )
        ...
        if self.top.iActivePlayer != -1:
            if self.iBuilding == gc.getCivilizationInfo(
                ...
                iHealth += gc.getPlayer(self.top.iActivePlayer).getExtraBuildingHealth(
                    self.iBuilding
                )
    ...
    def placeRequires(self):
        ...
        if iPrereq >= 0:
            screen.attachImageButton(
                ...
                GenericButtonSizes.BUTTON_SIZE_CUSTOM,
                WidgetTypes.WIDGET_PEDIA_JUMP_TO_RELIGION,
                iPrereq,
                -1,
                False,
            )
    ...
    def getBuildingType(self, iBuilding):
        ...
        ):  # Rhye - should be changed to move embassies to regular buildings
            return True
    ...
    def getBuildingSortedList(self, bWonder):
        ...
        for iBuilding in range(gc.getNumBuildingInfos()):  # Rhye
            if (
                self.getBuildingType(iBuilding) == bWonder
