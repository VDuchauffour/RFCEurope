...
class CvPediaCivilization:
    ...
    def interfaceScreen(self, iCivilization):
        ...
        szHeader = (
            u"<font=4b>"
            + gc.getCivilizationInfo(self.iCivilization).getDescription().upper()
            + u"</font>"
        )
        ...
        screen.setText(
            ...
            CivilopediaPageTypes.CIVILOPEDIA_PAGE_CIV,
            -1,
        )
        ...
        screen.addDDSGFC(
            self.top.getNextWidgetName(),
            ArtFileMgr.getCivilizationArtInfo(
                gc.getCivilizationInfo(self.iCivilization).getArtDefineTag()
            ).getButton(),
    ...
    def placeTech(self):
        ...
        screen.addPanel(
            ...
            self.Y_TECH - 4,
            self.W_LEADER,
            self.Y_TEXT - self.Y_TECH + 4,
            PanelStyles.PANEL_STYLE_BLUE50,
        )
        szText = CyGameTextMgr().parseCivInfos(self.iCivilization, True)
        screen.attachMultilineText(
            panelName, "", szText, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY
        )
        # Rhye - end
