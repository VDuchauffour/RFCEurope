...
class CvPediaImprovement:
    ...
    def interfaceScreen(self, iImprovement):
        ...
        szHeader = (
            u"<font=4b>"
            + gc.getImprovementInfo(self.iImprovement).getDescription().upper()
            + u"</font>"
        )
        ...
        screen.setText(
            ...
            CivilopediaPageTypes.CIVILOPEDIA_PAGE_IMPROVEMENT,
            -1,
        )
        ...
        screen.addDDSGFC(
            self.top.getNextWidgetName(),
            gc.getImprovementInfo(self.iImprovement).getButton(),
        ...
        if self.top.animations:
            screen.addImprovementGraphicGFC(
                self.top.getNextWidgetName(),
                self.iImprovement,
                self.X_IMPROVENEMT_ANIMATION,
                self.Y_IMPROVENEMT_ANIMATION,
                self.W_IMPROVENEMT_ANIMATION,
                self.H_IMPROVENEMT_ANIMATION,
                WidgetTypes.WIDGET_GENERAL,
                -1,
                -1,
                self.X_ROTATION_IMPROVENEMT_ANIMATION,
                self.Z_ROTATION_IMPROVENEMT_ANIMATION,
    ...
    def placeYield(self):
        ...
        for k in range(YieldTypes.NUM_YIELD_TYPES):
            ...
            if iYieldChange != 0:
                ...
                szYield += u"%s: %s%i%c" % (
                    gc.getYieldInfo(k).getDescription(),
                    sign,
                    iYieldChange,
        ...
        for iTech in range(gc.getNumTechInfos()):
            for k in range(YieldTypes.NUM_YIELD_TYPES):
                szYield = u""
                iYieldChange = gc.getImprovementInfo(self.iImprovement).getTechYieldChanges(
                    iTech, k
                )
                if iYieldChange != 0:
                    szYield = localText.getText(
        ...
        for iCivic in range(gc.getNumCivicInfos()):
            for k in range(YieldTypes.NUM_YIELD_TYPES):
                szYield = u""
                iYieldChange = gc.getCivicInfo(iCivic).getImprovementYieldChanges(
                    self.iImprovement, k
                )
                if iYieldChange != 0:
                    szYield = localText.getText(
    ...
    def placeBonusYield(self):
        ...
        screen.addPanel(
            ...
            True,
            True,
            self.X_BONUS_YIELDS_PANE,
            self.Y_BONUS_YIELDS_PANE,
            self.W_BONUS_YIELDS_PANE,
            self.H_BONUS_YIELDS_PANE,
            PanelStyles.PANEL_STYLE_BLUE50,
        )
        info = gc.getImprovementInfo(self.iImprovement)
