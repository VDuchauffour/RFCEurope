...
class CvPediaTech(CvPediaScreen.CvPediaScreen):
    ...
    def interfaceScreen(self, iTech):
        ...
        screen.setText(
            ...
            CivilopediaPageTypes.CIVILOPEDIA_PAGE_TECH,
            iTech,
        )
        ...
        screen.addDDSGFC(
            self.top.getNextWidgetName(),
            gc.getTechInfo(self.iTech).getButton(),
        ...
        screen.setLabel(
            szCostId,
            "Background",
            u"<font=3>" + szCostText.upper() + u"</font>",
            CvUtil.FONT_LEFT_JUSTIFY,
            self.X_COST + 25,
            self.Y_COST,
            0,
            ...
            for k in range(gc.getNUM_OR_TECH_PREREQS()):
                ...
                if iPrereq == self.iTech:
                    screen.attachImageButton(
                        panelName,
                        "",
                        gc.getTechInfo(j).getButton(),
                        GenericButtonSizes.BUTTON_SIZE_CUSTOM,
                        WidgetTypes.WIDGET_PEDIA_JUMP_TO_DERIVED_TECH,
                        j,
                        self.iTech,
                        False,
                    )
        ...
        for j in range(gc.getNUM_AND_TECH_PREREQS()):
            ...
            if eTech > -1:
                ...
                screen.attachImageButton(
                    ...
                    GenericButtonSizes.BUTTON_SIZE_CUSTOM,
                    WidgetTypes.WIDGET_PEDIA_JUMP_TO_REQUIRED_TECH,
                    eTech,
                    j,
                    False,
                )
        ...
        for eLoopProject in range(gc.getNumProjectInfos()):
            if isTechRequiredForProject(self.iTech, eLoopProject):
                screen.attachImageButton(
                    panelName,
                    "",
                    gc.getProjectInfo(eLoopProject).getButton(),
                    GenericButtonSizes.BUTTON_SIZE_CUSTOM,
    ...
    def placeSpecial(self):
        ...
        screen.addMultilineText(
            ...
            self.W_SPECIAL_PANE - 35,
            self.H_SPECIAL_PANE - 10,
            WidgetTypes.WIDGET_GENERAL,
            -1,
            -1,
            CvUtil.FONT_LEFT_JUSTIFY,
    ...
    def placeQuote(self):
        ...
        screen.addMultilineText(
            szQuoteTextWidget,
            szQuote,
            self.X_QUOTE_PANE + 15,
            self.Y_QUOTE_PANE + 15,
            self.W_QUOTE_PANE - (15 * 2),
            self.H_QUOTE_PANE - (15 * 2),
            WidgetTypes.WIDGET_GENERAL,
            -1,
            -1,
            CvUtil.FONT_LEFT_JUSTIFY,
