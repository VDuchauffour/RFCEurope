...
class CvPediaUnitChart:
    ...
    def interfaceScreen(self, iGroup):
        ...
        screen.setText(
            ...
            CivilopediaPageTypes.CIVILOPEDIA_PAGE_UNIT_GROUP,
            -1,
        )
    ...
    def placeUnitTable(self):
        ...
        screen.addPanel(
            ...
            True,
            True,
            self.X_UNITS + iMargin,
            self.Y_UNITS + iMargin,
        ...
        screen.setTableColumnHeader(
            szTable, 1, u"%c" % CyGame().getSymbolID(FontSymbols.STRENGTH_CHAR), iColWidth
        )
        screen.setTableColumnHeader(
            szTable, 2, u"%c" % CyGame().getSymbolID(FontSymbols.MOVES_CHAR), iColWidth
        )
        screen.setTableColumnHeader(
            szTable, 3, u"%c" % gc.getYieldInfo(YieldTypes.YIELD_PRODUCTION).getChar(), iColWidth
        )
        ...
        for j in range(gc.getNumUnitInfos()):
            if (
                self.iGroup == gc.getUnitInfo(j).getUnitCombatType()
        ...
        for j in range(gc.getNumUnitInfos()):
            if (
                ...
                unitsList[i] = (
                    gc.getUnitInfo(j).getCombat(),
                    gc.getUnitInfo(j).getMoves(),
                    szCost,
                    gc.getUnitInfo(j).getDescription(),
                    j,
                )
                i += 1
        # unitsList.sort()
