...
class CvReligionScreen:
        ...
        for iRel in self.RELIGIONS:
            ...
            if gc.getGame().getReligionGameTurnFounded(iRel) >= 0:
                screen.addCheckBoxGFCAt(
                    ...
                    self.X_SCROLLABLE_RELIGION_AREA + xLoop - 25,
                    self.Y_SCROLLABLE_RELIGION_AREA + 5,
                    self.BUTTON_SIZE,
                    self.BUTTON_SIZE,
                    WidgetTypes.WIDGET_GENERAL,
                    -1,
                ...
                screen.setImageButtonAt(
                    szButtonName,
                    szArea,
                    gc.getReligionInfo(iRel).getButtonDisabled(),
                    self.X_SCROLLABLE_RELIGION_AREA + xLoop - 25,
                    self.Y_SCROLLABLE_RELIGION_AREA + 5,
                    self.BUTTON_SIZE,
                    self.BUTTON_SIZE,
            ...
            screen.setLabelAt(
                ...
                szLabel,
                CvUtil.FONT_CENTER_JUSTIFY,
                self.X_SCROLLABLE_RELIGION_AREA + xLoop,
                self.Y_RELIGION_NAME,
                self.DZ,
                FontTypes.GAME_FONT,
                WidgetTypes.WIDGET_GENERAL,
                -1,
    ...
    def drawHelpInfo(self):
        ...
        # Holy City...
        screen.setLabelAt("", szArea, text("TXT_KEY_RELIGION_SCREEN_HOLY_CITY"), CvUtil.FONT_LEFT_JUSTIFY, self.LEFT_EDGE_TEXT, self.Y_HOLY_CITY, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    #                screen.setLabelAt("", szArea, szFounded, CvUtil.FONT_CENTER_JUSTIFY, xLoop, self.Y_HOLY_CITY, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
        ...
        # Influence...
        screen.setLabelAt("", szArea, text("TXT_KEY_RELIGION_SCREEN_INFLUENCE"), CvUtil.FONT_LEFT_JUSTIFY, self.LEFT_EDGE_TEXT, self.Y_INFLUENCE, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
...
# BUG - start
    ...
    def BUGConstants(self):
        ...
        self.szCities = text("TXT_KEY_BUG_RELIGIOUS_CITY")
        self.szTemples = text("TXT_KEY_BUG_RELIGIOUS_TEMPLE")
        self.szMonastaries = text("TXT_KEY_BUG_RELIGIOUS_MONASTARY")
        self.szMissionaries = text("TXT_KEY_BUG_RELIGIOUS_MISSIONARY")
...
# end of BUG indent of original code
    ...
    def drawFaithInfo(self):
        ...
        screen.addPanel(
            self.FAITH_EFFECT_AREA_ID,
            "",
            "",
            True,
            True,
            self.X_FAITH_EFFECT_AREA,
            self.Y_FAITH_EFFECT_AREA,
            self.W_FAITH_EFFECT_AREA,
            self.H_FAITH_EFFECT_AREA,
            PanelStyles.PANEL_STYLE_MAIN,
        )
        ...
        screen.addMultilineText(
            "Child" + self.FAITH_EFFECT_AREA_ID,
            szFaithBenefits,
            self.X_FAITH_EFFECT_AREA + 12,
            self.Y_FAITH_EFFECT_AREA + 12,
            self.W_FAITH_EFFECT_AREA - 20,
            self.H_FAITH_EFFECT_AREA - 20,
            WidgetTypes.WIDGET_GENERAL,
            -1,
            -1,
            CvUtil.FONT_LEFT_JUSTIFY,
    ...
    def canConvert(self, iReligion):
        ...
        return iConvertReligion != iCurrentReligion and gc.getPlayer(
            self.iActivePlayer
        ).canConvert(iConvertReligion)
...
#                CyCamera().JustLookAtPlot(pCity.plot())
    ...
    def ReligionScreenButton(self, inputClass):
        if inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED:
            if (
                inputClass.getID() == gc.getNumReligionInfos()
                or gc.getGame().getReligionGameTurnFounded(inputClass.getID()) >= 0
            ):
