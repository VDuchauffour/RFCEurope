...
class CvReligionScreen:
    ...
    def drawReligionInfo(self):
        ...
        screen.addCheckBoxGFC(
            ...
            ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
            self.X_RELIGION_AREA + xLoop - self.BUTTON_SIZE / 2,
            self.Y_RELIGION_AREA + self.Y_RELIGION - self.BUTTON_SIZE / 2,
            self.BUTTON_SIZE,
            self.BUTTON_SIZE,
            WidgetTypes.WIDGET_GENERAL,
            -1,
        ...
        screen.setText(
            ...
            szLabel,
            CvUtil.FONT_CENTER_JUSTIFY,
            xLoop + self.X_RELIGION_AREA,
            self.Y_RELIGION_AREA + self.Y_RELIGION_NAME,
            2 * self.DZ,
            FontTypes.GAME_FONT,
            WidgetTypes.WIDGET_GENERAL,
            -1,
    ...
    def drawHelpInfo(self):
        ...
        for iRel in self.RELIGIONS:
            if gc.getGame().getReligionGameTurnFounded(iRel) >= 0:
                szFounded = CyGameTextMgr().getTimeStr(
                    gc.getGame().getReligionGameTurnFounded(iRel), False
                )
                screen.setLabelAt(
                    "",
                    szArea,
        ...
        if AdvisorOpt.isReligious():
            ...
            for iUnit in PlayerUtil.playerUnits(self.iActivePlayer):
                for iRel in self.RELIGIONS:
                    if iUnit.getUnitType() == ReligionUtil.getUnit(
                        iRel, ReligionUtil.UNIT_MISSIONARY
                    ):
            ...
            for iRel in self.RELIGIONS:
                if gc.getGame().getReligionGameTurnFounded(iRel) >= 0:
                    szFounded = "%i [%i]" % (
                        iMissionaries_Active[iRel],
                        iMissionaries_Construct[iRel],
                    )
                    screen.setLabelAt(
                        "",
                        szArea,
    ...
    def drawCityInfo(self, iReligion):
        ...
        if AdvisorOpt.isReligious():
            ...
            for iRel in range(self.NUM_RELIGIONS):  # columns for religious icons
                if gc.getGame().getReligionGameTurnFounded(iRel) >= 0:
                    ...
                    screen.setTableColumnHeader(
                        self.TABLE_ID, self.COL_FIRST_RELIGION + iRel, szReligionIcon, 25
                    )
            ...
            for type in ReligionUtil.getUnitTypes():
                screen.setTableColumnHeader(
                    self.TABLE_ID,
                    self.COL_FIRST_UNIT + type.index,
                    u"<font=2>%s</font>" % type.icon,
                    30,
                )
            ...
            for type in ReligionUtil.getBuildingTypes():
                screen.setTableColumnHeader(
                    self.TABLE_ID,
                    self.COL_FIRST_BUILDING + type.index,
                    u"<font=2>%s</font>" % type.icon,
                    30,
                )
                ...
                for iRel in range(self.NUM_RELIGIONS):
                    if gc.getGame().getReligionGameTurnFounded(iRel) >= 0:
                        ...
                        if iRel in lHolyCity:
                            szReligionIcon = u"<font=2>%c</font>" % (
                                gc.getReligionInfo(iRel).getHolyCityChar()
                            )
                        elif iRel in lReligions:
                            szReligionIcon = u"<font=2>%c</font>" % (
                                gc.getReligionInfo(iRel).getChar()
                            )
    ...
    def handleInput(self, inputClass):
        ...
        if (
            inputClass.getNotifyCode() == NotifyCode.NOTIFY_LISTBOX_ITEM_SELECTED
