...
class WBCityDataScreen:
    ...
    def interfaceScreen(self, pCityX):
        ...
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_WB_CITY_DATA", ()), 0, 0, False
        )
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_WB_CITY_DATA2", ()), 1, 1, True
        )
        screen.addPullDownString(
            "CurrentPage",
        ...
        for i in xrange(gc.getNumSpecialistInfos()):
            ...
            if iGPClass == -1:
                continue
            iGP = gc.getCivilizationInfo(pCity.getCivilizationType()).getCivilizationUnits(
                iGPClass
            )
            if iGP == -1:
                continue
    ...
    def placeModify(self):
        ...
        screen.setLabel(
            "ModifyHeader",
            "Background",
            "<font=3b>" + CyTranslator().getText("TXT_KEY_WB_MODIFY", (sText,)) + "</font>",
        ...
        iY += 30
        screen.addDropDownBoxGFC(
            "BuildingType", iX, iY, 150, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT
        )
        screen.addPullDownString(
            "BuildingType",
            CyTranslator().getText("TXT_KEY_CONCEPT_BUILDINGS", ()),
            0,
            0,
            not bWonder,
        )
        screen.addPullDownString(
            "BuildingType", CyTranslator().getText("TXT_KEY_CONCEPT_WONDERS", ()), 1, 1, bWonder
        )
        ...
        for i in xrange(CommerceTypes.NUM_COMMERCE_TYPES):
            screen.addPullDownString(
                ...
                i + YieldTypes.NUM_YIELD_TYPES,
                i + YieldTypes.NUM_YIELD_TYPES,
                iSelectedYield == i + YieldTypes.NUM_YIELD_TYPES,
            )
    ...
    def placeSpecialist(self):
        ...
        sText = (
            "<font=3b>"
            + CyTranslator().getText("TXT_KEY_WB_BASE_RATE", (pCity.getBaseGreatPeopleRate(),))
            + "</font>"
        )
        screen.setButtonGFC(
            "GreatPeopleRatePlus",
        ...
        screen.setLabel(
            "SpecialistHeader",
            "Background",
            u"<font=4b>" + CyTranslator().getText("TXT_KEY_WB_FREE_SPECIALISTS", ()) + "</font>",
            CvUtil.FONT_CENTER_JUSTIFY,
            iX + iWidth / 2,
            iY,
            -0.1,
        ...
        for item in lSpecialist:
            ...
            screen.setTableText(
                ...
                0,
                iRow,
                "<font=3>" + sColor + sItem + "</font></color>",
                ItemInfo.getButton(),
                WidgetTypes.WIDGET_PYTHON,
                7879,
                item[1],
    ...
    def placeGreatPeople(self):
        ...
        sText = (
            "<font=3b>"
            + CyTranslator().getText("TXT_KEY_WB_PROGRESS", (pCity.getGreatPeopleProgress(),))
            + "/"
            + str(gc.getPlayer(iPlayer).greatPeopleThreshold(False))
            + "</font>"
        )
        screen.setButtonGFC(
            "GreatPeopleFlatPlus",
        ...
        screen.setLabel(
            "GreatPeopleHeader",
            "Background",
            u"<font=4b>" + CyTranslator().getText("TXT_KEY_CONCEPT_GREAT_PEOPLE", ()) + "</font>",
            CvUtil.FONT_CENTER_JUSTIFY,
            iX + iWidth / 2,
            iY,
            -0.1,
    ...
    def placeBonus(self):
        ...
        screen.setLabel(
            "BonusHeader",
            "Background",
            u"<font=4b>" + CyTranslator().getText("TXT_KEY_PEDIA_CATEGORY_BONUS", ()) + "</font>",
            CvUtil.FONT_CENTER_JUSTIFY,
            iX + iWidth / 2,
            iY - 60,
            -0.1,
    ...
    def modifyBuilding(self, iBuilding):
        ...
        if iType < YieldTypes.NUM_YIELD_TYPES:
            pCity.setBuildingYieldChange(
                iClass, iType, pCity.getBuildingYieldChange(iClass, iType) + iCount
            )
        else:
            iType -= YieldTypes.NUM_YIELD_TYPES
            if iType < CommerceTypes.NUM_COMMERCE_TYPES:
                pCity.setBuildingCommerceChange(
                    iClass, iType, pCity.getBuildingCommerceChange(iClass, iType) + iCount
                )
            else:
                iType -= CommerceTypes.NUM_COMMERCE_TYPES
                if iType == 0:
                    pCity.setBuildingHappyChange(
                        iClass, pCity.getBuildingHappyChange(iClass) + iCount
                    )
                else:
                    pCity.setBuildingHealthChange(
                        iClass, pCity.getBuildingHealthChange(iClass) + iCount
                    )
    ...
    def editGreatPeopleFlat(self, iCount):
        ...
        iCount = min(
            iCount,
            gc.getPlayer(iPlayer).greatPeopleThreshold(False) - pCity.getGreatPeopleProgress(),
        )
    ...
    def editGreatPeopleProgress(self, item):
        ...
        pCity.changeGreatPeopleUnitProgress(
            item,
            min(
                iCount,
                gc.getPlayer(iPlayer).greatPeopleThreshold(False)
                - pCity.getGreatPeopleUnitProgress(item),
            ),
        )
