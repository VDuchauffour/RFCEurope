...
class WBCityEditScreen:
    ...
    def interfaceScreen(self, pCityX):
        ...
        sText = "<font=3b>%s, X: %d, Y: %d</font>" % (
            CyTranslator().getText("TXT_KEY_WB_LATITUDE", (pCity.plot().getLatitude(),)),
            pCity.getX(),
            pCity.getY(),
        )

        sText = u"<font=3b>%s ID: %d, %s: %d</font>" % (
            CyTranslator().getText("TXT_KEY_WB_CITY", ()),
            pCity.getID(),
            CyTranslator().getText("TXT_KEY_WB_AREA_ID", ()),
            pPlot.getArea(),
        )
        screen.setLabel(
            "PlotScreenHeaderB",
        ...
        screen.addPullDownString(
            "Commands", CyTranslator().getText("TXT_KEY_WB_MOVE_CITY", ()), 1, 1, False
        )
        screen.addPullDownString(
            "Commands", CyTranslator().getText("TXT_KEY_WB_DUPLICATE_CITY", ()), 2, 2, False
        )
        screen.addPullDownString(
            "Commands",
            CyTranslator().getText("TXT_KEY_WB_MOVE_CITY", ())
            + " + "
        ...
        screen.addPullDownString(
            "Commands",
            CyTranslator().getText("TXT_KEY_WB_DUPLICATE_CITY", ())
            + " + "
            + CyTranslator().getText("TXT_KEY_PEDIA_CATEGORY_UNIT", ()),
            4,
            4,
        ...
        screen.addPullDownString(
            "Commands", CyTranslator().getText("TXT_KEY_WB_RAZE_CITY", ()), 5, 5, False
        )
        ...
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_WB_CITY_DATA", ()), 0, 0, True
        )
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_WB_CITY_DATA2", ()), 1, 1, False
        )
        screen.addPullDownString(
            "CurrentPage",
        ...
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_WB_PLAYER_DATA", ()), 3, 3, False
        )
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_WB_TEAM_DATA", ()), 4, 4, False
        )
        screen.addPullDownString(
            "CurrentPage",
            CyTranslator().getText("TXT_KEY_PEDIA_CATEGORY_RELIGION", ()),
            8,
            8,
            False,
        )
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_CONCEPT_CORPORATIONS", ()), 9, 9, False
        )
        screen.addPullDownString(
            "CurrentPage",
            CyTranslator().getText("TXT_KEY_PEDIA_CATEGORY_UNIT", ())
        ...
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_WB_PLOT_DATA", ()), 6, 6, False
        )
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_CONCEPT_EVENTS", ()), 7, 7, False
        )
        screen.addPullDownString(
    ...
    def sortCities(self):
        ...
        for iPlayerX in xrange(gc.getMAX_PLAYERS()):  # type: ignore
            ...
            if iOwnerType == 1 and iPlayerX != iPlayer:
                continue
            if iOwnerType == 2 and pPlayerX.getTeam() != pCity.getTeam():
                continue
            (loopCity, iter) = pPlayerX.firstCity(False)
            while loopCity:
                if iPlotType == 2 or (
                    iPlotType == 1 and loopCity.plot().getArea() == pCity.plot().getArea()
                ):
    ...
    def placeCityTable(self):
        ...
        for (loopCity, iPlayerX, sColor) in self.lCities:
            ...
            screen.setTableText(
                ...
                2,
                iRow,
                "<font=3>" + sColor + loopCity.getName() + "</font></color>",
                "",
                WidgetTypes.WIDGET_PYTHON,
                7200 + iPlayerX,
                loopCity.getID(),
    ...
    def placeMap(self):
        ...
        screen.addPlotGraphicGFC(
            "PlotView",
            screen.getXResolution() * 3 / 4 - iMapWidth / 2,
            iY,
            iMapWidth,
            iMapHeight,
            pCity.plot(),
    ...
    def placeStats(self):
        ...
        for i in xrange(gc.getNumCultureLevelInfos()):  # type: ignore
            screen.addPullDownString(
                ...
                i,
                i,
                pCity.getCultureLevel() == i,
            )

        iY += 30
        screen.setButtonGFC(
            "CityChangeCulturePlus",
            "",
            "",
            iX,
            iY,
        ...
        sText = u"<font=3>%s %s/%s%c</font>" % (
            CyTranslator().getText("TXT_KEY_WB_CULTURE", ()),
            CvPlatyBuilderScreen.CvWorldBuilderScreen().addComma(pCity.getCulture(iPlayer)),
            CvPlatyBuilderScreen.CvWorldBuilderScreen().addComma(pCity.getCultureThreshold()),
            gc.getCommerceInfo(CommerceTypes.COMMERCE_CULTURE).getChar(),
        )
        screen.setLabel(
            "CityChangeCultureText",
            "Background",
            sText,
            CvUtil.FONT_LEFT_JUSTIFY,
            iX + 50,
        ...
        for i in xrange(YieldTypes.NUM_YIELD_TYPES):  # type: ignore
            ...
            sText = CyTranslator().getText(
                "TXT_KEY_WB_BASE_YIELD", (gc.getYieldInfo(i).getDescription(), iYield)
            )
        ...
        sText = u"<font=3>%s: %d/%d%c</font>" % (
            CyTranslator().getText("TXT_KEY_CONCEPT_FOOD", ()),
            pCity.getFood(),
        ...
        sText = (
            "<font=3>"
            + CyTranslator().getText("TXT_KEY_WB_POPULATION", ())
            + " "
            + str(pCity.getPopulation())
            + CyTranslator().getText("[ICON_ANGRYPOP]", ())
            + "</font>"
        )
        screen.setLabel(
            "CityPopulationText",
        ...
        sText = (
            "<font=3>"
            + CyTranslator().getText("TXT_KEY_WB_DEFENSE", (pCity.getDefenseModifier(False),))
            + "</font>"
        )
        screen.setLabel(
            "CityDefenseValueText",
            "Background",
            sText,
            CvUtil.FONT_LEFT_JUSTIFY,
            iX + 50,
        ...
        sText = (
            "<font=3>"
            + CyTranslator().getText("TXT_KEY_WB_DAMAGE", ())
            + ": "
            + str(pCity.getDefenseDamage())
            + "</font>"
        )
        screen.setLabel(
            "CityDefenseDamageText",
            "Background",
            sText,
            CvUtil.FONT_LEFT_JUSTIFY,
            iX + 50,
        ...
        sText = u"<font=3>%s: %d%s</font>" % (
            CyTranslator().getText("TXT_KEY_CONCEPT_TRADE", ()),
            pCity.getTradeRoutes(),
            CyTranslator().getText("[ICON_TRADE]", ()),
        )
        screen.setLabel(
            "CityCTradeRouteText",
            "Background",
        ...
        sText = "<font=3>%s: %d%s, %d%s</font>" % (
            CyTranslator().getText("TXT_KEY_CONCEPT_HAPPINESS", ()),
            pCity.happyLevel(),
            CyTranslator().getText("[ICON_HAPPY]", ()),
            pCity.unhappyLevel(0),
            CyTranslator().getText("[ICON_UNHAPPY]", ()),
        )
        screen.setLabel(
            "CityChangeHappyText",
            "Background",
        ...
        sText = "<font=3>%s: %d%s, %d%s</font>" % (
            CyTranslator().getText("TXT_KEY_CONCEPT_HEALTH", ()),
            pCity.goodHealth(),
            CyTranslator().getText("[ICON_HEALTHY]", ()),
            pCity.badHealth(False),
            CyTranslator().getText("[ICON_UNHEALTHY]", ()),
        )
        screen.setLabel(
            "CityChangeHealthText",
            "Background",
            sText,
            CvUtil.FONT_LEFT_JUSTIFY,
            iX + 50,
        ...
        sText = (
            "<font=3>"
            + CyTranslator().getText("TXT_KEY_CONCEPT_RESISTANCE", ())
            + ": "
            + str(pCity.getOccupationTimer())
            + CyTranslator().getText("[ICON_OCCUPATION]", ())
            + "</font>"
        )
        screen.setLabel(
            "CityOccupationTurnText",
            "Background",
            sText,
            CvUtil.FONT_LEFT_JUSTIFY,
            iX + 50,
        ...
        sText = (
            "<font=3>"
            + CyTranslator().getText("TXT_KEY_CONCEPT_DRAFT", ())
            + CyTranslator().getText(" [ICON_UNHAPPY]: ", ())
            + str(pCity.getConscriptAngerTimer())
            + "</font>"
        )
        screen.setLabel(
            "CityDraftAngerText",
        ...
        sText = (
            "<font=3>"
            + CyTranslator().getText("TXT_KEY_WB_HURRY_ANGER", (pCity.getHurryAngerTimer(),))
            + "</font>"
        )
        screen.setLabel(
            "CityHurryAngerText",
        ...
        sText = (
            ...
            + CyTranslator().getText(
                "TXT_KEY_WB_DEFY_RESOLUTION", (pCity.getDefyResolutionAngerTimer(),)
            )
            + "</font>"
        )
        screen.setLabel(
            "CityDefyResolutionText",
            "Background",
            sText,
            CvUtil.FONT_LEFT_JUSTIFY,
            iX + 50,
        ...
        sText = u"<font=3>%s %s: %d</font>" % (
            CyTranslator().getText("TXT_KEY_ESPIONAGE_CULTURE", ()),
            CyTranslator().getText("[ICON_UNHEALTHY]", ()),
            pCity.getEspionageHealthCounter(),
        )
        screen.setLabel(
            "CityEspionageHealthText",
            "Background",
            sText,
            CvUtil.FONT_LEFT_JUSTIFY,
            iX + 50,
        ...
        sText = (
            "<font=3>"
            + CyTranslator().getText("TXT_KEY_WB_TEMP_HAPPY", (pCity.getHappinessTimer(),))
            + "</font>"
        )
        screen.setLabel(
            "CityTemporaryHappyText",
            "Background",
            sText,
            CvUtil.FONT_LEFT_JUSTIFY,
            iX + 50,
    ...
    def placeProduction(self):
        ...
        if pCity.isProductionProcess():
            ...
            screen.setButtonGFC(
                "CurrentProductionPlus",
                "",
                "",
                screen.getXResolution() - 70,
                iY,
                24,
                24,
                WidgetTypes.WIDGET_PYTHON,
            ...
            screen.setButtonGFC(
                "CurrentProductionMinus",
                "",
                "",
                screen.getXResolution() - 45,
                iY,
                24,
                24,
                WidgetTypes.WIDGET_PYTHON,
        ...
        for i in xrange(gc.getNumUnitInfos()):  # type: ignore
            if pCity.canTrain(i, True, False):
                ...
                screen.setTableText(
                    ...
                    0,
                    iRow,
                    "<font=3>" + sColor + ItemInfo.getDescription() + "</font></color>",
                    ItemInfo.getButton(),
                    WidgetTypes.WIDGET_PYTHON,
                    8202,
                    i,
        ...
        screen.setTableText(
            ...
            "<font=3>"
            + sColor
            + CyTranslator().getText("TXT_KEY_CULTURELEVEL_NONE", ())
            + "</font></color>",
    ...
    def handleInput(self, inputClass):
        ...
        if inputClass.getFunctionName() == "ChangeBy":
            ...
            gc.getPlayer(screen.getPullDownData("CityOwner", iIndex)).acquireCity(
                pCity, False, True
            )
            ...
            if inputClass.getData1() == 1030:
                pCity.changeDefenseDamage(
                    min(iChange, gc.getMAX_CITY_DEFENSE_DAMAGE() - pCity.getDefenseDamage())
                )
            elif inputClass.getData1() == 1031:
            ...
            if inputClass.getData1() == 1030:
                pCity.changeExtraTradeRoutes(
                    min(iChange, gc.getDefineINT("MAX_TRADE_ROUTES") - pCity.getTradeRoutes())
                )
            elif inputClass.getData1() == 1031:
            ...
            if iIndex == 0:
                ...
                pCity.setCulture(
                    iPlayer,
                    gc.getCultureLevelInfo(iIndex).getSpeedThreshold(CyGame().getGameSpeedType()),
                    True,
                )
            self.placeStats()
            ...
            if inputClass.getData1() == 1030:
                pCity.changeProduction(
                    min(iChange, pCity.getProductionNeeded() - pCity.getProduction())
                )
            elif inputClass.getData1() == 1031:
    ...
    def handlePlatyChooseProduction(self, inputClass):
        if inputClass.getButtonType() == WidgetTypes.WIDGET_HELP_BUILDING:
            for j in xrange(pCity.getOrderQueueLength()):  # type: ignore
                ...
                if (
                    iOrderData.eOrderType == OrderTypes.ORDER_CONSTRUCT
                    and iOrderData.iData1 == inputClass.getData1()
                ):
            ...
            for j in xrange(pCity.getOrderQueueLength()):  # type: ignore
                ...
                if (
                    ...
                    and iOrderData.iData1 == inputClass.getData2()
                ):
                    pCity.popOrder(j, False, False)
                    break
            pCity.pushOrder(
            ...
            for j in xrange(pCity.getOrderQueueLength()):  # type: ignore
                ...
                if (
                    ...
                    break
            pCity.pushOrder(
                OrderTypes.ORDER_MAINTAIN, inputClass.getData2(), -1, False, False, False, True
            )
        else:
