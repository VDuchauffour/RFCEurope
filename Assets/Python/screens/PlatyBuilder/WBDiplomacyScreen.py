...
class WBDiplomacyScreen:
    ...
    def interfaceScreen(self, iPlayerX, bPage):
        ...
        screen.setText(
            ...
            screen.getXResolution() - 25,
            screen.getYResolution() - 40,
            -0.1,
        ...
        screen.addDropDownBoxGFC(
            "ChangeType", 20, 50, iWidth, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT
        )
        screen.addPullDownString(
            "ChangeType", CyTranslator().getText("TXT_KEY_WB_CITY_ADD", ()), 1, 1, not bRemove
        )
        screen.addPullDownString(
            "ChangeType", CyTranslator().getText("TXT_KEY_WB_CITY_REMOVE", ()), 0, 0, bRemove
        )

        screen.addDropDownBoxGFC(
            "ChangeBy", 20, 80, iWidth, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT
        )
        i = 1
        while i < 1000001:
        ...
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_CONCEPT_TRADE", ()), 2, 2, False
        )
    ...
    def setPlayerList(self, iPlayer):
        ...
        for iPlayerX in xrange(gc.getMAX_PLAYERS()):  # type: ignore
            ...
            if pPlayerX.isEverAlive():
                ...
                if pPlayerX.isTurnActive():
                    sText = "[" + sText + "]"
                screen.addPullDownString(
                    "CurrentPlayer", sText, iPlayerX, iPlayerX, iPlayerX == iSelectedPlayer
                )
        ...
        sText = u"%s >>> %s" % (
            gc.getPlayer(iSelectedPlayer).getName(),
    ...
    def setGeneralPage(self):
        ...
        sText = "%s: %s\t" % (
            ...
            CyTranslator().getText("TXT_KEY_FOREIGN_ADVISOR_CONTACT", ()),
        )
        sText += "%s: %s\t" % (
            CyTranslator().getText("[ICON_OPENBORDERS]", ()),
            CyTranslator().getText("TXT_KEY_MISC_OPEN_BORDERS", ()),
        )
        sText += "%s: %s\t" % (
            CyTranslator().getText("[ICON_DEFENSIVEPACT]", ()),
            CyTranslator().getText("TXT_KEY_MISC_DEFENSIVE_PACT", ()),
        )
        sText += "%s: %s" % (
            CyTranslator().getText("[ICON_OCCUPATION]", ()),
            CyTranslator().getText("TXT_KEY_CONCEPT_WAR", ()),
        )
        screen.setLabel(
            "LegendText",
            "Background",
        ...
        screen.setTableColumnHeader(
            "WBDiplomacy", 0, CyTranslator().getText("TXT_KEY_PEDIA_CATEGORY_CIV", ()), iWidth2
        )
        screen.setTableColumnHeader(
            "WBDiplomacy", 1, CyTranslator().getText("TXT_KEY_PEDIA_CATEGORY_LEADER", ()), iWidth2
        )
        screen.setTableColumnHeader(
            "WBDiplomacy", 2, CyTranslator().getText("TXT_KEY_PITBOSS_TEAM", ()), 50
        )
        screen.setTableColumnHeader(
            "WBDiplomacy", 3, CyTranslator().getText("TXT_KEY_WB_ATTITUDE", ()), iWidth1
        )
        screen.setTableColumnHeader(
            "WBDiplomacy",
            4,
            CyTranslator().getText("TXT_KEY_FOREIGN_ADVISOR_RELATIONS", ()),
            iWidth1,
        )
        screen.setTableColumnHeader(
            "WBDiplomacy", 5, CyTranslator().getText("[ICON_ANGRYPOP]", ()), 25
        )
        screen.setTableColumnHeader(
            "WBDiplomacy", 6, CyTranslator().getText("[ICON_OPENBORDERS]", ()), 25
        )
        screen.setTableColumnHeader(
            "WBDiplomacy", 7, CyTranslator().getText("[ICON_DEFENSIVEPACT]", ()), 25
        )
        screen.setTableColumnHeader(
            "WBDiplomacy", 8, CyTranslator().getText("[ICON_OCCUPATION]", ()), 25
        )
        screen.setTableColumnHeader(
            "WBDiplomacy", 9, CyTranslator().getText("TXT_KEY_CONCEPT_WAR_WEARINESS", ()), iWidth1
        )
        ...
        screen.setTableText(
            ...
            0,
            0,
            "<font=4>" + CyTranslator().getText("[ICON_ANGRYPOP]", ()) + "<\font>",
            "",
        ...
        screen.setTableText(
            ...
            1,
            0,
            "<font=4>" + CyTranslator().getText("[ICON_OPENBORDERS]", ()) + "<\font>",
            "",
            WidgetTypes.WIDGET_PYTHON,
            1030,
            1,
        ...
        screen.setTableText(
            ...
            2,
            0,
            "<font=4>" + CyTranslator().getText("[ICON_DEFENSIVEPACT]", ()) + "<\font>",
            "",
            WidgetTypes.WIDGET_PYTHON,
            1030,
            2,
        ...
        screen.setTableText(
            ...
            3,
            0,
            "<font=4>" + CyTranslator().getText("[ICON_OCCUPATION]", ()) + "<\font>",
            "",
            WidgetTypes.WIDGET_PYTHON,
            1030,
            3,
        ...
        for iPlayer in lPlayers:
            ...
            screen.setTableInt(
                ...
                2,
                iRow,
                "<font=3>" + sColor + str(iTeam) + "</font></color>",
                "",
                WidgetTypes.WIDGET_GENERAL,
                -1,
                -1,
            ...
            sText = CyTranslator().changeTextColor(
                gc.getAttitudeInfo(iAttitude).getDescription(),
                gc.getInfoTypeForString(self.lAttitude[iAttitude]),
            )
            screen.setTableText(
                "WBDiplomacy",
                3,
    ...
    def setEspionagePage(self):
        ...
        iWidth2 = iWidth * 3 / 16
        screen.setTableColumnHeader(
            "WBEspionage", 0, CyTranslator().getText("TXT_KEY_PEDIA_CATEGORY_CIV", ()), iWidth2
        )
        screen.setTableColumnHeader(
            "WBEspionage", 1, CyTranslator().getText("TXT_KEY_PEDIA_CATEGORY_LEADER", ()), iWidth2
        )
        screen.setTableColumnHeader(
            "WBEspionage", 2, CyTranslator().getText("TXT_KEY_ESPIONAGE_CULTURE", ()), iWidth1
        )
        screen.setTableColumnHeader(
            "WBEspionage",
            3,
            CyTranslator().getText("TXT_KEY_ESPIONAGE_SCREEN_SPENDING_WEIGHT", ()),
            iWidth1,
        )
        screen.setTableColumnHeader(
            "WBEspionage", 4, CyTranslator().getText("TXT_KEY_REPLAY_SCREEN_TURNS", ()), iWidth1
        )
        screen.setTableColumnHeader(
            "WBEspionage", 5, CyTranslator().getText("TXT_KEY_WB_MODIFIER", ()), iWidth1
        )
        screen.setTableColumnHeader(
            "WBEspionage", 6, CyTranslator().getText("TXT_KEY_WB_MEMORY", ()), iWidth1
        )
        ...
        screen.setLabel(
            "CounterEspionageHeader",
            "Background",
            u"<font=4b>" + CyTranslator().getText("TXT_KEY_WB_COUNTER_ESPIONAGE", ()) + "</font>",
            CvUtil.FONT_CENTER_JUSTIFY,
            iX,
            self.iTable_Y - 60,
            -0.1,
        ...
        screen.setLabel(
            "EspionageHeader",
            "Background",
            u"<font=4b>" + CyTranslator().getText("TXT_KEY_ESPIONAGE_CULTURE", ()) + "</font>",
            CvUtil.FONT_CENTER_JUSTIFY,
            iX,
            self.iTable_Y - 60,
            -0.1,
        ...
        for iPlayer in lPlayers:
            ...
            if bTowardsPlayer:
                ...
                sMemory = str(
                    gc.getPlayer(iSelectedPlayer).AI_getMemoryCount(iPlayer, iSelectedMemory)
                )
                sEspionage = str(gc.getTeam(iSelectedTeam).getEspionagePointsAgainstTeam(iTeam))
                sWeight = str(
                    gc.getPlayer(iSelectedPlayer).getEspionageSpendingWeightAgainstTeam(iTeam)
                )
                sTurns = str(
                    gc.getTeam(iSelectedTeam).getCounterespionageTurnsLeftAgainstTeam(iTeam)
                )
        ...
        if bDiplomacyPage:
            ...
            if inputClass.getFunctionName() == "AttitudeAll":
                ...
                self.editOpenBorders(
                    inputClass.getData2(), pSelectedTeam.isOpenBorders(inputClass.getData2())
                )
            elif inputClass.getData1() == 1034:
                self.editDefensivePact(
                    inputClass.getData2(), pSelectedTeam.isDefensivePact(inputClass.getData2())
                )
            elif inputClass.getData1() == 1035:
                self.editWarStatus(
                    inputClass.getData2(), pSelectedTeam.isAtWar(inputClass.getData2())
                )
    ...
    def editWarStatus(self, iTeam, bCancel):
        if not pSelectedTeam.isHasMet(iTeam):
            return
        if gc.getTeam(iTeam).isVassal(iSelectedTeam):
            return
        if pSelectedTeam.isVassal(iTeam):
            return
        if bCancel:
    ...
    def editCETurns(self, iTeam):
        ...
        if bRemove:
            iCount = -iChange
            iCount = max(
                iCount, -gc.getTeam(iTeam1).getCounterespionageTurnsLeftAgainstTeam(iTeam2)
            )
    ...
    def editAttitude(self, iPlayer):
        ...
        if self.RelationshipStatus(pPlayer1.getTeam(), gc.getPlayer(iPlayer2).getTeam()) == 1:
            return
        iCount = 1
        iNewAttitude = min(
            pPlayer1.AI_getAttitude(iPlayer2) + 1, AttitudeTypes.NUM_ATTITUDE_TYPES - 1
        )
        if bRemove:
            iCount = -1
