...
class WBEventScreen:
    ...
    def interfaceScreen(self, pPlotX):
        ...
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_WB_PLOT_DATA", ()), 0, 0, False
        )
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_CONCEPT_EVENTS", ()), 1, 1, True
        )
        if pPlot.isOwned():
            screen.addPullDownString(
    ...
    def placeEventPlayers(self):
        ...
        screen.setLabel(
            "EventPlayerText",
            "Background",
            "<font=3b>" + sHeader + "</font>",
            CvUtil.FONT_CENTER_JUSTIFY,
            screen.getXResolution() * 3 / 10,
            self.iTable_Y - 30,
            -0.1,
            FontTypes.TITLE_FONT,
            WidgetTypes.WIDGET_GENERAL,
    ...
    def placeCorporations(self):
        ...
        for item in lCorporations:
            ...
            screen.setTableText(
                ...
                gc.getCorporationInfo(item[1]).getButton(),
                WidgetTypes.WIDGET_PYTHON,
                8201,
                item[1],
                CvUtil.FONT_LEFT_JUSTIFY,
    ...
    def placeReligions(self):
        ...
        for item in lReligions:
            ...
            screen.setTableText(
                ...
                iRow,
                "<font=3>" + sColor + item[0] + "</font></color>",
                gc.getReligionInfo(item[1]).getButton(),
    ...
    def placeEvents(self):
        ...
        if iSelectedEvent > -1:
            sText = (
                "<font=4b>"
                + CyTranslator().getText("[COLOR_BLACK]", ())
                + CyTranslator().getText("TXT_KEY_WB_TRIGGER_EVENT", (sHeader,)).upper()
                + "</color></font>"
            )
            screen.setButtonGFC(
                "TriggerEvent",
