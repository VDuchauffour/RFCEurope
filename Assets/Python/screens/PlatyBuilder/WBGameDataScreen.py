...
class WBGameDataScreen:
    ...
    def interfaceScreen(self):
        ...
        screen.setLabel(
            "GameDataHeader",
            "Background",
            "<font=4b>" + CyTranslator().getText("TXT_KEY_PITBOSS_GAME_OPTIONS", ()) + "</font>",
        ...
        screen.addDropDownBoxGFC(
            "ChangeBy", 20, 50, iWidth, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT
        )
        i = 1
        while i < 100001:
        ...
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_PITBOSS_GAME_OPTIONS", ()), 10, 10, True
        )
        screen.addPullDownString(
    ...
    def placeStats(self):
        ...
        screen.setButtonGFC(
            "MaxCityEliminationPlus",
            "",
            "",
            screen.getXResolution() / 4 + 20,
            iY,
            24,
            24,
            WidgetTypes.WIDGET_PYTHON,
        ...
        screen.setButtonGFC(
            "MaxCityEliminationMinus",
            "",
            "",
            screen.getXResolution() / 4 + 45,
            iY,
            24,
            24,
            WidgetTypes.WIDGET_PYTHON,
        ...
        sText = CyTranslator().getText(
            "TXT_KEY_WB_MAX_CITY_ELIMINATION", (CyGame().getMaxCityElimination(),)
        )
        screen.setLabel(
            "MaxCityEliminationText",
            "Background",
            "<font=3>" + sText + "</font>",
            CvUtil.FONT_LEFT_JUSTIFY,
            screen.getXResolution() / 4 + 75,
            iY + 1,
            -0.1,
            FontTypes.TITLE_FONT,
            WidgetTypes.WIDGET_GENERAL,
        ...
        sText = CyTranslator().getText(
            "TXT_KEY_WB_ESTIMATED_END_TURN", (CyGame().getEstimateEndTurn(),)
        )
        screen.setLabel(
            "EstimateEndTurnText",
            "Background",
        ...
        sText = (
            CyTranslator().getText("TXT_KEY_HEADING_TRADEROUTE_LIST", ())
            + ": "
            + str(gc.getDefineINT("INITIAL_TRADE_ROUTES") + CyGame().getTradeRoutes())
        )
        screen.setLabel(
            "TradeRoutesText",
            "Background",
        ...
        screen.setLabel(
            "AutoPlayStop",
            "Background",
            "<font=3>" + CyTranslator().getText("TXT_KEY_WB_AI_AUTOPLAY_WARNING", ()) + "</font>",
            CvUtil.FONT_CENTER_JUSTIFY,
    ...
    def placeNewPlayer(self):
        ...
        screen.setText(
            ...
            "<font=3b>"
            + sColor
            + CyTranslator().getText("TXT_KEY_MAIN_MENU_REPEAT_PASSWORD", ())
            + "</color></font>",
            CvUtil.FONT_RIGHT_JUSTIFY,
            screen.getXResolution() - 20,
            self.iNewPlayer_Y - 30,
            -0.1,
            FontTypes.TITLE_FONT,
            WidgetTypes.WIDGET_PYTHON,
            1029,
        ...
        if iSelectedCiv > -1:
            ...
            for item in xrange(gc.getNumLeaderHeadInfos()):  # type: ignore
                ...
                if not CyGame().isLeaderEverActive(item) or bRepeat:
                    if CyGame().isOption(
                        GameOptionTypes.GAMEOPTION_LEAD_ANY_CIV
                    ) or gc.getCivilizationInfo(iSelectedCiv).isLeaders(item):
            ...
            if iSelectedLeader > -1:
                ...
                sText = (
                    ...
                    + "<font=4b>"
                    + CyTranslator().getText("TXT_KEY_MAIN_MENU_LOADSAVE_CREATE", ())
                    + "</color></font>"
                )
                screen.setText(
                    "CreatePlayer",
    ...
    def placeGameOptions(self):
        ...
        for i in xrange(len(lList)):  # type: ignore
            ...
            sText = (
                "<font=3>"
                + sColor
                + CyTranslator().getText("TXT_KEY_WB_DEFAULT", ())
                + "</font></color>"
            )
            screen.setTableText(
                "WBGameOptions",
    ...
    def checkOptions(self, iGameOption):
        if iGameOption == GameOptionTypes.GAMEOPTION_LEAD_ANY_CIV:
            ...
            iSelectedLeader = -1
            self.interfaceScreen()
        elif iGameOption == GameOptionTypes.GAMEOPTION_NO_GOODY_HUTS and CyGame().isOption(
            iGameOption
        ):
