...
class WBPlayerScreen:
    ...
    def interfaceScreen(self, iPlayerX):
        ...
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_WB_PLAYER_DATA", ()), 0, 0, True
        )
        screen.addPullDownString(
    ...
    def placeStats(self):
        ...
        screen.addDDSGFC(
            ...
            screen.getXResolution() * 3 / 8 - self.iIconSize / 2,
            80,
            self.iIconSize,
            self.iIconSize,
        ...
        screen.addDDSGFC(
            ...
            screen.getXResolution() * 5 / 8 - self.iIconSize / 2,
            80,
            self.iIconSize,
            self.iIconSize,
            WidgetTypes.WIDGET_PYTHON,
        ...
        screen.setLabel(
            "CivilizationName",
            "Background",
            "<font=4b>" + pPlayer.getCivilizationDescription(0) + "</font>",
            CvUtil.FONT_CENTER_JUSTIFY,
            screen.getXResolution() / 2,
            50,
            -0.1,
        ...
        sText = u"%s %s%c" % (
            CyTranslator().getText("TXT_KEY_WB_GOLD", ()),
            CvPlatyBuilderScreen.CvWorldBuilderScreen().addComma(pPlayer.getGold()),
        ...
        sText = u"%s: %d/%d" % (
            CyTranslator().getText("TXT_KEY_MISC_COMBAT_EXPERIENCE", ()),
            pPlayer.getCombatExperience(),
            pPlayer.greatPeopleThreshold(True),
        )
        screen.setLabel(
            "CombatXPText",
            "Background",
        ...
        sText = u"%s: %d" % (
            CyTranslator().getText("TXT_KEY_CONCEPT_GOLDEN_AGE", ()),
        ...
        sText = CyTranslator().getText(
            "TXT_KEY_WB_GOLDEN_AGE_UNITS", (pPlayer.unitsRequiredForGoldenAge(),)
        )
        screen.setLabel(
            "GPRequiredText",
            "Background",
        ...
        sText = CyTranslator().getText(
            "TXT_KEY_WB_COASTAL_TRADE", (pPlayer.getCoastalTradeRoutes(),)
        )
        screen.setLabel(
            "CoastalTradeText",
            "Background",
        ...
        for i in xrange(CommerceTypes.NUM_COMMERCE_TYPES):  # type: ignore
            ...
            sText = sColor + u"<font=3>%c: %d%% %s</color></font>" % (
                gc.getCommerceInfo(i).getChar(),
                pPlayer.getCommercePercent(i),
                CyTranslator().getText(
                    "TXT_KEY_MISC_POS_GOLD_PER_TURN", (pPlayer.getCommerceRate(CommerceTypes(i)),)
                ),
            )
            screen.setText(
                "AdjustCommerceFlexible" + gc.getCommerceInfo(i).getType(),
                "Background",
                "<font=3>" + sText + "</font>",
                CvUtil.FONT_LEFT_JUSTIFY,
                iX + 50,
    ...
    def placeResearch(self):
        ...
        screen.setTableText(
            ...
            0,
            0,
            "<font=3>" + sColor + sCurrentTech + "</font></color>",
        ...
        for iTech in xrange(gc.getNumTechInfos()):  # type: ignore
            if pPlayer.canResearch(iTech, False):
                ...
                sText = u"%s (%d/%d)%c" % (
                    ...
                    pTeam.getResearchProgress(iTech),
                    pTeam.getResearchCost(iTech),
        ...
        if iCurrentTech > -1:
            sText = u"%s: %d/%d%c" % (
                gc.getTechInfo(iCurrentTech).getDescription(),
                pTeam.getResearchProgress(iCurrentTech),
                pTeam.getResearchCost(iCurrentTech),
                gc.getCommerceInfo(CommerceTypes.COMMERCE_RESEARCH).getChar(),
            )
            screen.setButtonGFC(
                "CurrentResearchPlus",
    ...
    def placeReligions(self):
        ...
        screen.setTableText(
            ...
            + "</font></color>",
            CyArtFileMgr().getInterfaceArtInfo("INTERFACE_BUTTONS_CANCEL").getPath(),
            WidgetTypes.WIDGET_HELP_RELIGION,
            -1,
            -1,
            CvUtil.FONT_LEFT_JUSTIFY,
        )
        ...
        sText = (
            "<font=3>"
            + CyTranslator().getText(
                "TXT_KEY_WB_STATE_RELIGION_UNIT",
                (pPlayer.getStateReligionUnitProductionModifier(),),
            )
            + "</font>"
        )
        screen.setLabel(
            "StateReligionUnitText",
            "Background",
            sText,
            CvUtil.FONT_LEFT_JUSTIFY,
            iX + 50,
        ...
        sText = (
            "<font=3>"
            + CyTranslator().getText(
                "TXT_KEY_WB_STATE_RELIGION_BUILDING",
                (pPlayer.getStateReligionBuildingProductionModifier(),),
            )
            + "</font>"
        )
        screen.setLabel(
            "StateReligionBuildingText",
            "Background",
            sText,
            CvUtil.FONT_LEFT_JUSTIFY,
            iX + 50,
    ...
    def placeCivics(self):
        ...
        screen.setLabel(
            "CivicsHeader",
            "Background",
            "<font=3b>" + CyTranslator().getText("TXT_KEY_PEDIA_CATEGORY_CIVIC", ()) + "</font>",
            CvUtil.FONT_CENTER_JUSTIFY,
            iX + iWidth / 2,
            iY,
            -0.1,
        ...
        for iCivicOption in xrange(gc.getNumCivicOptionInfos()):  # type: ignore
            ...
            sText = (
                ...
                + gc.getCivicOptionInfo(iCivicOption).getDescription()
                + "</font></color>"
            )
            screen.setTableText(
                "WBPlayerCivics",
            ...
            if (
                iCivicOption % iColumns == iColumns - 1
                and iCivicOption < gc.getNumCivicOptionInfos() - 1
            ):
                screen.appendTableRow("WBPlayerCivics")
    ...
    def handleInput(self, inputClass):
        ...
        if inputClass.getFunctionName() == "ChangeBy":
            ...
            pPlayer.setCurrentEra(
                screen.getPullDownData("CurrentEra", screen.getSelectedPullDownID("CurrentEra"))
            )
            ...
            if inputClass.getData1() == 1030:
                pPlayer.changeCombatExperience(
                    min(
                        iChange, pPlayer.greatPeopleThreshold(True) - pPlayer.getCombatExperience()
                    )
                )
            elif inputClass.getData1() == 1031:
            ...
            if inputClass.getData1() == 1030:
                ...
                pPlayer.changeNumUnitGoldenAges(
                    -min(iChange, pPlayer.unitsRequiredForGoldenAge() - 1)
                )
            self.placeStats()
            ...
            if inputClass.getData1() == 1030:
                pPlayer.changeCoastalTradeRoutes(
                    min(
                        iChange,
                        gc.getDefineINT("MAX_TRADE_ROUTES") - pPlayer.getCoastalTradeRoutes(),
                    )
                )
            elif inputClass.getData1() == 1031:
            ...
            if inputClass.getData1() == 1030:
                ...
                if pPlayer.isCommerceFlexible(iCommerce):
                    pPlayer.changeCommercePercent(
                        iCommerce, -min(iChange, pPlayer.getCommercePercent(iCommerce))
                    )
            elif inputClass.getData1() == 7881:
                if pPlayer.isCommerceFlexible(iCommerce):
                    pTeam.changeCommerceFlexibleCount(
                        iCommerce, -pTeam.getCommerceFlexibleCount(iCommerce)
                    )
                else:
            ...
            if iTech > -1:
                if inputClass.getData1() == 1030:
                    pTeam.changeResearchProgress(
                        ...
                        min(
                            iChange,
                            pTeam.getResearchCost(iTech) - pTeam.getResearchProgress(iTech),
                        ),
                        iPlayer,
                    )
                elif inputClass.getData1() == 1031:
                    pTeam.changeResearchProgress(
                        pPlayer.getCurrentResearch(),
                        -min(iChange, pTeam.getResearchProgress(iTech)),
                        iPlayer,
                    )
                self.placeResearch()
            ...
            if inputClass.getData1() == 1030:
                ...
                pPlayer.changeStateReligionUnitProductionModifier(
                    -min(iChange, pPlayer.getStateReligionUnitProductionModifier())
                )
            self.placeReligions()
            ...
            if inputClass.getData1() == 1030:
                ...
                pPlayer.changeStateReligionBuildingProductionModifier(
                    -min(iChange, pPlayer.getStateReligionBuildingProductionModifier())
                )
            self.placeReligions()
