...
class WBPlotScreen:
    ...
    def interfaceScreen(self, pPlotX):
        ...
        for i in xrange(gc.getMAX_PLAYERS()):  # type: ignore
            ...
            if pPlayerX.isAlive():
                screen.addPullDownString(
                    "CurrentPlayer", pPlayerX.getName(), i, i, i == pPlot.getOwner()
                )

        iY += 30
        screen.addDropDownBoxGFC(
            "ChangeType", iX, iY, iWidth, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT
        )
        screen.addPullDownString(
            "ChangeType", CyTranslator().getText("TXT_KEY_WB_CITY_ADD", ()), 1, 1, bAdd
        )
        screen.addPullDownString(
            "ChangeType", CyTranslator().getText("TXT_KEY_WB_CITY_REMOVE", ()), 0, 0, not bAdd
        )
        ...
        screen.addPullDownString(
            "EditType", CyTranslator().getText("TXT_KEY_WB_SINGLE_PLOT", ()), 0, 0, iEditType == 0
        )
        screen.addPullDownString(
            "EditType", CyTranslator().getText("TXT_KEY_WB_AREA_PLOTS", ()), 1, 1, iEditType == 1
        )
        screen.addPullDownString(
            "EditType", CyTranslator().getText("TXT_KEY_WB_ALL_PLOTS", ()), 2, 2, iEditType == 2
        )
        ...
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_WB_PLOT_DATA", ()), 0, 0, True
        )
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_CONCEPT_EVENTS", ()), 1, 1, False
        )
        if pPlot.isOwned():
            screen.addPullDownString(
            ...
            if pPlot.isCity():
                screen.addPullDownString(
                    "CurrentPage", CyTranslator().getText("TXT_KEY_WB_CITY_DATA", ()), 4, 4, False
                )
        if pPlot.getNumUnits() > 0:
            screen.addPullDownString(
                "CurrentPage", CyTranslator().getText("TXT_KEY_WB_UNIT_DATA", ()), 5, 5, False
            )
        screen.addPullDownString(
        ...
        sText = u"<font=3b>%s ID: %d, %s: %d</font>" % (
            CyTranslator().getText("TXT_KEY_WB_PLOT", ()),
            iIndex,
        ...
        screen.addPullDownString(
            "BonusClass", CyTranslator().getText("TXT_KEY_WB_CITY_ALL", ()), -1, -1, True
        )
        screen.addPullDownString(
            "BonusClass",
        ...
        while gc.getBonusClassInfo(iBonusClass) is not None:
            ...
            screen.addPullDownString(
                "BonusClass", sText, iBonusClass, iBonusClass, iBonusClass == iSelectedClass
            )
            iBonusClass += 1

        screen.setImageButton(
            "NextPlotUpButton",
            CyArtFileMgr().getInterfaceArtInfo("INTERFACE_GENERAL_UPARROW").getPath(),
            screen.getXResolution() / 2 - 12,
            self.iTable_Y,
            24,
            24,
            WidgetTypes.WIDGET_GENERAL,
        ...
        screen.setImageButton(
            "NextPlotDownButton",
            CyArtFileMgr().getInterfaceArtInfo("INTERFACE_GENERAL_DOWNARROW").getPath(),
            screen.getXResolution() / 2 - 12,
            self.iTable_Y + 48,
            24,
            24,
        ...
        screen.setImageButton(
            "NextPlotLeftButton",
            CyArtFileMgr().getInterfaceArtInfo("INTERFACE_BUTTONS_LEFT").getPath(),
            screen.getXResolution() / 2 - 36,
            self.iTable_Y + 24,
            24,
            24,
            WidgetTypes.WIDGET_GENERAL,
        ...
        screen.setImageButton(
            "NextPlotRightButton",
            CyArtFileMgr().getInterfaceArtInfo("INTERFACE_BUTTONS_RIGHT").getPath(),
            screen.getXResolution() / 2 + 12,
            self.iTable_Y + 24,
            24,
            24,
    ...
    def placeSigns(self):
        ...
        sText = (
            CyTranslator().getText("[ICON_CULTURE]", ()) + gc.getPlayer(iCulturePlayer).getName()
        )
        screen.setLabel(
            "PlotCultureText",
            "Background",
        ...
        for iPlayerX in xrange(gc.getMAX_PLAYERS()):  # type: ignore
            ...
            if pPlayerX.isEverAlive():
                ...
                screen.setTableText(
                    ...
                    iRow,
                    "",
                    gc.getCivilizationInfo(iCivilization).getButton(),
                    WidgetTypes.WIDGET_PYTHON,
                    7872,
                    iPlayerX * 10000 + iCivilization,
                    CvUtil.FONT_LEFT_JUSTIFY,
                )
                screen.setTableText(
                    "WBSigns",
                    ...
                    WidgetTypes.WIDGET_PYTHON,
                    7876,
                    iPlayerX * 10000 + iLeader,
                    CvUtil.FONT_LEFT_JUSTIFY,
                )
                sText = (
                    "<font=3>"
                    + CvPlatyBuilderScreen.CvWorldBuilderScreen().addComma(
                        pPlot.getCulture(iPlayerX)
                    )
    ...
    def placeMap(self):
        screen = CyGInterfaceScreen("WBPlotScreen", CvScreenEnums.WB_PLOT)
        iMapHeight = min(
            (screen.getYResolution() / 2 - 30 - (self.iTable_Y + 48 + 24)), iWidth * 2 / 3
        )
        iMapWidth = iMapHeight * 3 / 2
        screen.addPlotGraphicGFC(
            "PlotView",
            screen.getXResolution() / 2 - iMapWidth / 2,
            screen.getYResolution() / 2 - 30 - iMapHeight,
            iMapWidth,
            iMapHeight,
            pPlot,
            350,
    ...
    def placeStats(self):
        ...
        screen.setLabel(
            "YieldHeader",
            "Background",
            "<font=3b>" + CyTranslator().getText("TXT_KEY_PEDIA_YIELDS", ()) + "</font>",
            CvUtil.FONT_CENTER_JUSTIFY,
            screen.getXResolution() / 10,
            iY,
            -0.1,
            FontTypes.GAME_FONT,
            WidgetTypes.WIDGET_GENERAL,
        ...
        for i in xrange(YieldTypes.NUM_YIELD_TYPES):  # type: ignore
            ...
            if iImprovement > -1:
                iYield -= pPlot.calculateImprovementYieldChange(
                    iImprovement, YieldTypes(i), pPlot.getOwner(), False
                )
            screen.setButtonGFC(
                "BaseYieldPlus" + str(i),
                "",
    ...
    def placeRivers(self):
        ...
        screen.setLabel(
            "PlotRiverHeader",
            "Background",
            "<font=3b>" + CyTranslator().getText("TXT_KEY_MISC_RIVERS", ()) + "</font>",
            CvUtil.FONT_CENTER_JUSTIFY,
            iX,
            iY - 30,
            -0.1,
        ...
        if not pNorthPlot.isNone():
            ...
            screen.setState(
                "RiverNorthAButton",
                pNorthPlot.getRiverWEDirection() == CardinalDirectionTypes.CARDINALDIRECTION_WEST,
            )
            screen.setState(
                "RiverNorthBButton",
                pNorthPlot.getRiverWEDirection() == CardinalDirectionTypes.NO_CARDINALDIRECTION,
            )
            screen.setState(
                "RiverNorthCButton",
                pNorthPlot.getRiverWEDirection() == CardinalDirectionTypes.CARDINALDIRECTION_EAST,
            )
        ...
        if not pWestPlot.isNone():
            screen.setState(
                "RiverWestAButton",
                pWestPlot.getRiverNSDirection() == CardinalDirectionTypes.CARDINALDIRECTION_NORTH,
            )
            screen.setState(
                "RiverWestBButton",
                pWestPlot.getRiverNSDirection() == CardinalDirectionTypes.NO_CARDINALDIRECTION,
            )
            screen.setState(
                "RiverWestCButton",
                pWestPlot.getRiverNSDirection() == CardinalDirectionTypes.CARDINALDIRECTION_SOUTH,
            )

        screen.setState(
            "RiverEastAButton",
            pPlot.getRiverNSDirection() == CardinalDirectionTypes.CARDINALDIRECTION_NORTH,
        )
        screen.setState(
            "RiverEastBButton",
            pPlot.getRiverNSDirection() == CardinalDirectionTypes.NO_CARDINALDIRECTION,
        )
        screen.setState(
            "RiverEastCButton",
            pPlot.getRiverNSDirection() == CardinalDirectionTypes.CARDINALDIRECTION_SOUTH,
        )
        ...
        screen.setState(
            "RiverSouthAButton",
            pPlot.getRiverWEDirection() == CardinalDirectionTypes.CARDINALDIRECTION_WEST,
        )
        screen.setState(
            "RiverSouthBButton",
            pPlot.getRiverWEDirection() == CardinalDirectionTypes.NO_CARDINALDIRECTION,
        )
        screen.setState(
            "RiverSouthCButton",
            pPlot.getRiverWEDirection() == CardinalDirectionTypes.CARDINALDIRECTION_EAST,
        )
        global iPlotType_Y
        iPlotType_Y = iY + 30
    ...
    def placeRoutes(self):
        ...
        screen.setLabel(
            ...
            "<font=3b>" + sText + "</font>",
            CvUtil.FONT_CENTER_JUSTIFY,
            iX + screen.getXResolution() / 10 - 10,
            iY - 30,
            -0.1,
            FontTypes.GAME_FONT,
            WidgetTypes.WIDGET_GENERAL,
    ...
    def placeFeature(self):
        ...
        for item in lFeatures:
            ...
            screen.setTableText(
                ...
                iRow,
                "<font=3>" + sColor + item[0] + "</font></color>",
                gc.getFeatureInfo(iType).getButton(),
                WidgetTypes.WIDGET_PYTHON,
                7874,
                item[1],
                CvUtil.FONT_LEFT_JUSTIFY,
    ...
    def placeImprovements(self):
        ...
        if iImprovement > -1:
            if gc.getImprovementInfo(iImprovement).getUpgradeTime():
                ...
                sText = CyTranslator().getText(
                    "TXT_KEY_WB_UPGRADE_PROGRESS",
                    (pPlot.getUpgradeTimeLeft(iImprovement, pPlot.getOwner()),),
                )
                screen.setLabel(
                    "UpgradeTimeText",
                    "Background",
    ...
    def handleInput(self, inputClass):
        ...
        if inputClass.getFunctionName() == "ChangeBy":
            ...
            iEditType = screen.getPullDownData(
                "EditType", screen.getSelectedPullDownID("EditType")
            )
            ...
            if iIndex == 1:
                ...
                if pPlot.isCity():
                    ...
                    ).interfaceScreen(pPlot.getPlotCity())
            elif iIndex == 5:
            ...
            if inputClass.getData1() == 1030:
                ...
                if iImprovement > -1:
                    iYield -= pPlot.calculateImprovementYieldChange(
                        iImprovement, i, pPlot.getOwner(), False
                    )
            ...
            if inputClass.getData1() == 1030:
                ...
                pPlot.changeCulture(
                    iCulturePlayer, -min(iChange, pPlot.getCulture(iCulturePlayer)), True
                )
            ...
            iSelectedClass = screen.getPullDownData(
                "BonusClass", screen.getSelectedPullDownID("BonusClass")
            )
            self.createBonusList()
                ...
                for i in xrange(CyMap().numPlots()):  # type: ignore
                    ...
                    if bAdd:
                        ...
                        if (
                            iBonus > -1
                            and bSensibility
                            and not pLoopPlot.canHaveBonus(iBonus, False)
                        ):
                ...
                for i in xrange(CyMap().numPlots()):  # type: ignore
                    ...
                    if bAdd:
                        if (
                            iImprovement > -1
                            and bSensibility
                            and not pLoopPlot.canHaveImprovement(iImprovement, -1, True)
                        ):
                            continue
            ...
            if inputClass.getData1() == 1030:
                ...
                pPlot.changeUpgradeProgress(
                    min(
                        iChange,
                        pPlot.getUpgradeTimeLeft(pPlot.getImprovementType(), pPlot.getOwner()) - 1,
                    )
                )
            self.placeImprovements()
                ...
                for i in xrange(CyMap().numPlots()):  # type: ignore
                    ...
                    if bAdd:
                        ...
                        if (
                            iFeature > -1
                            and bSensibility
                            and not pLoopPlot.canHaveFeature(iFeature)
                        ):
