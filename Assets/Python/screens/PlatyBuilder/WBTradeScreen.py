...
class WBTradeScreen:
    ...
    def interfaceScreen(self):
        ...
        screen.setLabel(
            "TradeHeader",
            "Background",
            "<font=4b>" + CyTranslator().getText("TXT_KEY_CONCEPT_TRADE", ()) + "</font>",
        ...
        screen.setText(
            "TradeCancel",
            "Background",
            "<font=4>" + CyTranslator().getText("TXT_KEY_SCREEN_CANCEL", ()).upper() + "</font>",
            CvUtil.FONT_CENTER_JUSTIFY,
            screen.getXResolution() / 2,
            screen.getYResolution() - 42,
            -0.1,
        ...
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_ESPIONAGE_CULTURE", ()), 1, 1, False
        )
        screen.addPullDownString(
            "CurrentPage", CyTranslator().getText("TXT_KEY_CONCEPT_TRADE", ()), 2, 2, True
        )
    ...
    def placeDeals(self):
        ...
        screen.addListBoxGFC(
            "TradeTable", "", iX, iY, iWidth, iHeight, TableStyles.TABLE_STYLE_STANDARD
        )
        ...
        for i in xrange(CyGame().getIndexAfterLastDeal()):  # type: ignore
            ...
            if iPlayer1 > -1:
                ...
                sColor = u"<color=%d,%d,%d,%d>" % (
                    pPlayer1.getPlayerTextColorR(),
                    pPlayer1.getPlayerTextColorG(),
                    pPlayer1.getPlayerTextColorB(),
            ...
            if iPlayer2 > -1:
                ...
                sColor = u"<color=%d,%d,%d,%d>" % (
                    pPlayer2.getPlayerTextColorR(),
                    pPlayer2.getPlayerTextColorG(),
                    pPlayer2.getPlayerTextColorB(),
            ...
            screen.appendListBoxString(
                "TradeTable", sText, WidgetTypes.WIDGET_GENERAL, -1, i, CvUtil.FONT_LEFT_JUSTIFY
            )
    ...
    def getTradeData(self, iType, iData, iTurn):
        if iType == TradeableItems.TRADE_OPEN_BORDERS:
            return CyTranslator().getText("[ICON_OPENBORDERS]", ()) + CyTranslator().getText(
                "TXT_KEY_MISC_OPEN_BORDERS", ()
            )
        if iType == TradeableItems.TRADE_DEFENSIVE_PACT:
            return CyTranslator().getText("[ICON_DEFENSIVEPACT]", ()) + CyTranslator().getText(
                "TXT_KEY_MISC_DEFENSIVE_PACT", ()
            )
        ...
        if iType == TradeableItems.TRADE_GOLD_PER_TURN:
            return CyTranslator().getText("[ICON_GOLD]", ()) + CyTranslator().getText(
                "TXT_KEY_MISC_GOLD_PER_TURN", (iData,)
            )
        if iType == TradeableItems.TRADE_RESOURCES:
            return u"%c%s" % (
                gc.getBonusInfo(iData).getChar(),
                gc.getBonusInfo(iData).getDescription(),
            )
        return ""
