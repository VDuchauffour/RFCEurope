...
void CvCityAI::AI_reset()
{
  ...
  FAssertMsg(
      GC.getNumEmphasizeInfos() > 0,
      "GC.getNumEmphasizeInfos() is not greater than zero but an array is being allocated in CvCityAI::AI_reset");
    ...
    for (iI = 0; iI < GC.getNumSpecialistInfos(); iI++)
    {
      if (isSpecialistValid((SpecialistTypes)iI, 1))
      {
        ...
        if (iForcedSpecialistCount > 0)
        {
          ...
          int iSpecialistsToAdd =
              ((iInitialExtraSpecialists * iForcedSpecialistCount) + (iTotalForcedSpecialists / 2)) /
...
bool CvCityAI::AI_avoidGrowth()
{
  ...
  if (!AI_isEmphasizeYield(YIELD_FOOD) && !AI_isEmphasizeGreatPeople())
  {
    ...
    {
      iHappinessLevel +=
          ((GC.getDefineINT("NO_MILITARY_PERCENT_ANGER") * (getPopulation() + 1)) / GC.getPERCENT_ANGER_DIVISOR());
    }
  ...
  if (iGreatPeopleRate != 0)
  {
    ...
    if (!isHuman() || AI_isEmphasizeGreatPeople())
    {
      ...
      if (iProgress > 0)
      {
        ...
        iTempValue +=
            ((iGreatPeopleRate * (isHuman() ? 1 : 4) * iGPPValue * iProgress * iProgress) / (iThreshold * iThreshold));
      }
    }
      ...
      if (eGreatPeopleUnit != NO_UNIT)
      {
        ...
        if (kUnitInfo.getGreatWorkCulture() > 0)
        {
          iTempValue += kUnitInfo.getGreatWorkCulture() /
                        ((GET_PLAYER(getOwnerINLINE()).AI_isDoStrategy(AI_STRATEGY_CULTURE3)) ? 200 : 350);
        }
      }
    }
...
void CvCityAI::AI_chooseProduction()
{
  ...
  bLandWar =
      ((pArea->getAreaAIType(getTeam()) == AREAAI_OFFENSIVE) || (pArea->getAreaAIType(getTeam()) == AREAAI_DEFENSIVE) ||
  ...
  bAssault = bAssaultAssist || (pArea->getAreaAIType(getTeam()) == AREAAI_ASSAULT) ||
  ...
  int iNumWaterAreaCitySites =
      (pWaterArea == NULL)
          ? 0
          : kPlayer.AI_getNumAdjacentAreaCitySites(pWaterArea->getID(), getArea(), iWaterAreaBestFoundValue);
  ...
  if (!bFinancialTrouble)
  {
    ...
    iMaxSettlers = std::min((GET_PLAYER(getOwnerINLINE()).getNumCities() + 2) / 3,
                            iNumAreaCitySites + iNumWaterAreaCitySites); //Rhye
  ...
  if (iNumCitiesInArea > 2)
  {
    if (kPlayer.AI_isDoStrategy(AI_STRATEGY_CULTURE2))
    {
      if (iCultureRateRank <= iCulturalVictoryNumCultureCities + 1)
      {
        ...
        {
          if ((((iNumAreaCitySites + iNumWaterAreaCitySites) > 0) && (kPlayer.getNumCities() < 6)) &&
              (GC.getGameINLINE().getSorenRandNum(2, "AI Less Culture More Expand") == 0))
          {
  ...
  if (isBarbarian())
  {
    if (!AI_isDefended(plot()->plotCount(PUF_isUnitAIType, UNITAI_ATTACK, -1,
                                         getOwnerINLINE()))) // XXX check for other team's units?
    {
...
*/
  ...
  if ((iExistingWorkers == 0) && (!bDanger) &&
      ((AI_getWorkersNeeded() > 0) ||
       ((isCapital() &&
         (GC.getGame().getElapsedGameTurns() <
          ((30 * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getTrainPercent()) / 100))))))
  {
    int iLandBonuses = AI_countNumBonuses(NO_BONUS, /*bIncludeOurs*/ true, /*bIncludeNeutral*/ true, -1, /*bLand*/ true,
                                          /*bWater*/ false);
  ...
  for (int iI = 0; iI < GC.getNumVictoryInfos(); iI++)
  {
    if (GC.getVictoryInfo((VictoryTypes)iI).getPopulationPercentLead() > 0)
    {
      iPercentOfDomination =
          100 * iOurPopPercent / std::max(1, GC.getGameINLINE().getAdjustedPopulationPercent((VictoryTypes)iI));
    }
  }
  ...
  if (bMaybeWaterArea)
  {
    if (kPlayer.AI_getNumTrainAIUnits(UNITAI_ATTACK_SEA) + kPlayer.AI_getNumTrainAIUnits(UNITAI_PIRATE_SEA) +
            kPlayer.AI_getNumTrainAIUnits(UNITAI_RESERVE_SEA) <
        3)
    {
  ...
  int iPlotSettlerCount =
      (iNumSettlers == 0) ? 0 : plot()->plotCount(PUF_isUnitAIType, UNITAI_SETTLE, -1, getOwnerINLINE());
  ...
  if (!bDanger && ((kPlayer.getCurrentEra() > (GC.getGame().getStartEra() + iProductionRank / 2))) ||
      (kPlayer.getCurrentEra() > (GC.getNumEraInfos() / 2)))
  {
  ...
  int iNeededFloatingDefenders =
      (isBarbarian() || bCrushStrategy) ? 0 : kPlayer.AI_getTotalFloatingDefendersNeeded(pArea);
  ...
  {
    if ((AI_countNumBonuses(NO_BONUS, /*bIncludeOurs*/ true, /*bIncludeNeutral*/ true, -1, /*bLand*/ true,
                            /*bWater*/ false) > 0) ||
  ...
  {
    int iWonderTime = GC.getGameINLINE().getSorenRandNum(
        GC.getLeaderHeadInfo(getPersonalityType()).getWonderConstructRand(), "Wonder Construction Rand");
    iWonderTime /= 5;
    iWonderTime += 7;
  ...
  //minimal defense.
  if (plot()->plotCount(PUF_isUnitAIType, UNITAI_CITY_DEFENSE, -1, getOwnerINLINE()) <
      (AI_minDefenders() + iPlotSettlerCount))
  {
    if (AI_chooseUnit(UNITAI_CITY_DEFENSE))
    {
      return;
  ...
  if ((iAreaBestFoundValue > iMinFoundValue) || (iWaterAreaBestFoundValue > iMinFoundValue))
  {
    ...
    if (iPlotSettlerCount == 0)
    {
      if ((iNumSettlers < iMaxSettlers) &&
          (!bLandWar || (GC.getGameINLINE().getSorenRandNum(2, "AI War Settler") == 0)))
      {
  ...
  if (bImportantCity && (iCultureRateRank <= iCulturalVictoryNumCultureCities))
  {
    ...
    else if (GC.getGameINLINE().getSorenRandNum(
                 ((iCultureRateRank == 1) ? 4 : 1) + iCulturalVictoryNumCultureCities * 2, "AI Build up Culture") <
             iCultureRateRank)
    {
      if (AI_chooseBuilding(BUILDINGFOCUS_BIGCULTURE | BUILDINGFOCUS_CULTURE | BUILDINGFOCUS_WONDEROK, 40))
      {
...
*/
    ...
    if (NULL != pWaterArea && (bAssault))
    {
      ...
      if (eBestAttackSeaUnit != NO_UNIT)
      {
        if (GC.getUnitInfo(eBestAttackSeaUnit).getBombardRate() > 0)
        {
          ...
          if ((iAttackSea < ((1 + 2 * iTransports) / 2)) &&
              (GC.getGame().getSorenRandNum(2, "AI train attack sea") == 0))
          {
            if (AI_chooseUnit(UNITAI_ATTACK_SEA))
            {
              AI_chooseBuilding(BUILDINGFOCUS_DOMAINSEA);
  ...
  if (iUnitCostPercentage < (iMaxUnitSpending + 4))
  {
    ...
    {
      iAircraftHave = kPlayer.AI_getNumAIUnits(UNITAI_ATTACK_AIR) + kPlayer.AI_getNumAIUnits(UNITAI_DEFENSE_AIR) +
                      kPlayer.AI_getNumAIUnits(UNITAI_MISSILE_AIR);
      if (NO_UNIT != eBestAttackAircraft)
      {
        iAircraftNeed = (2 + kPlayer.getNumCities() * (3 * GC.getUnitInfo(eBestAttackAircraft).getAirCombat())) /
                        (2 * std::max(1, GC.getGame().getBestLandUnitCombat()));
  ...
  if (!bAlwaysPeace && !(bLandWar || bAssault) &&
      (kPlayer.AI_isDoStrategy(AI_STRATEGY_OWABWNW) ||
       (GC.getGame().getSorenRandNum(4, "AI consider Nuke") == 0))) //Rhye
  {
    ...
    if ((iTotalNukes < iNukesWanted) && (GC.getGame().getSorenRandNum(100, "AI train nuke MWAHAHAH") <
                                         (100 - (80 * iTotalNukes) / iNukesWanted))) //Rhye
    {
  ...
  {
    if (!bFinancialTrouble && !bGetBetterUnits &&
        (bLandWar || ((bAssault || kPlayer.AI_isDoStrategy(AI_STRATEGY_DAGGER)) && !bAssaultAssist)))
    {
      ...
      if ((pArea->getAreaAIType(getTeam()) == AREAAI_MASSING) ||
          (pArea->getAreaAIType(getTeam()) == AREAAI_ASSAULT_MASSING))
      {
      ...
      {
        {
          ...
          invaderTypes.push_back(std::make_pair(
              UNITAI_PARADROP, (kPlayer.AI_isDoStrategy(AI_STRATEGY_AIR_BLITZ) ? 30 : 20) / (bAssault ? 2 : 1)));
          if (!bAssault)
          {
  ...
  if ((iProductionRank <= ((kPlayer.getNumCities() > 8) ? 3 : 2)) && (getPopulation() > 3))
  {
    int iWonderRand =
        8 + GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getWonderConstructRand(),
                                               "Wonder Construction Rand");

    // increase chance of going for an early wonder
    if (GC.getGameINLINE().getElapsedGameTurns() <
            (100 * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getConstructPercent() / 100) &&
        iNumCitiesInArea > 1)
    {
      iWonderRand *= 35;
      iWonderRand /= 100;
  ...
  if (!bLandWar)
  {
    ...
    if (pWaterArea != NULL)
    {
      if (bPrimaryArea)
      {
        if (kPlayer.AI_totalWaterAreaUnitAIs(pWaterArea, UNITAI_EXPLORE_SEA) <
            std::min(1, kPlayer.AI_neededExplorers(pWaterArea)))
        {
          if (AI_chooseUnit(UNITAI_EXPLORE_SEA))
          {
            return;
  ...
  {
    if ((bLandWar) || ((kPlayer.getNumCities() <= 3) && (GC.getGameINLINE().getElapsedGameTurns() < 60)) ||
...
UnitTypes CvCityAI::AI_bestUnit(bool bAsync, AdvisorTypes eIgnoreAdvisor, UnitAITypes *peBestUnitAI)
{
  ...
  bLandWar = (bDefense || (area()->getAreaAIType(getTeam()) == AREAAI_OFFENSIVE) ||
  ...
  if (!bFinancialTrouble && ((bPrimaryArea) ? (GET_PLAYER(getOwnerINLINE()).findBestFoundValue() > 0)
                                            : (area()->getBestFoundValue(getOwnerINLINE()) > 0)))
  {
  ...
  if (bWarPossible)
  {
    aiUnitAIVal[UNITAI_ATTACK_CITY] +=
        ((iMilitaryWeight / ((bWarPlan || bLandWar || bAssault) ? 10 : 17)) + ((bPrimaryArea) ? 1 : 0));
    aiUnitAIVal[UNITAI_COUNTER] +=
        ((iMilitaryWeight / ((bWarPlan || bLandWar || bAssault) ? 13 : 22)) + ((bPrimaryArea) ? 1 : 0));
    aiUnitAIVal[UNITAI_PARADROP] +=
        ((iMilitaryWeight / ((bWarPlan || bLandWar || bAssault) ? 5 : 8)) + ((bPrimaryArea) ? 1 : 0));
    ...
    if (bPrimaryArea)
    {
      aiUnitAIVal[UNITAI_ICBM] +=
          std::max((GET_PLAYER(getOwnerINLINE()).getTotalPopulation() / 25),
                   ((GC.getGameINLINE().countCivPlayersAlive() + GC.getGameINLINE().countTotalNukeUnits()) /
                    (GC.getGameINLINE().countCivPlayersAlive() + 1)));
    }
  }
  ...
  else
  {
    ...
    if (pWaterArea != NULL)
    {
      ...
      if ((GET_PLAYER(getOwnerINLINE()).getNumCities() > 3) || (area()->getNumUnownedTiles() < 10))
      {
        ...
        if (bPrimaryArea && (GET_PLAYER(getOwnerINLINE()).AI_totalAreaUnitAIs(area(), UNITAI_MISSIONARY) > 0) &&
            (pWaterArea->getNumTiles() > 400))
        {
          aiUnitAIVal[UNITAI_MISSIONARY_SEA]++;
        }

        if (bPrimaryArea && (GET_PLAYER(getOwnerINLINE()).AI_totalAreaUnitAIs(area(), UNITAI_SPY) > 0) &&
        ...
        if (bWarPossible)
        {
          ...
          aiUnitAIVal[UNITAI_RESERVE_SEA] +=
              std::min((pWaterArea->getNumTiles() / 200),
                       ((((iCoastalCities * 2) + (iMilitaryWeight / 7)) / 5) + ((bPrimaryArea) ? 1 : 0)));
          aiUnitAIVal[UNITAI_ESCORT_SEA] +=
              (GET_PLAYER(getOwnerINLINE()).AI_totalWaterAreaUnitAIs(pWaterArea, UNITAI_ASSAULT_SEA) +
               (GET_PLAYER(getOwnerINLINE()).AI_totalWaterAreaUnitAIs(pWaterArea, UNITAI_CARRIER_SEA) * 2));
          aiUnitAIVal[UNITAI_ASSAULT_SEA] += std::min(
              (pWaterArea->getNumTiles() / 250),
              ((((iCoastalCities * 2) + (iMilitaryWeight / 6)) / ((bAssault) ? 5 : 8)) + ((bPrimaryArea) ? 1 : 0)));
          aiUnitAIVal[UNITAI_CARRIER_SEA] +=
              std::min((pWaterArea->getNumTiles() / 350),
                       ((((iCoastalCities * 2) + (iMilitaryWeight / 8)) / 7) + ((bPrimaryArea) ? 1 : 0)));
          aiUnitAIVal[UNITAI_MISSILE_CARRIER_SEA] +=
              std::min((pWaterArea->getNumTiles() / 350),
                       ((((iCoastalCities * 2) + (iMilitaryWeight / 8)) / 7) + ((bPrimaryArea) ? 1 : 0)));
        }
      }
    ...
    if ((iHasMetCount > 0) && bWarPossible)
    {
      if (bLandWar || bAssault || !bFinancialTrouble || (GET_PLAYER(getOwnerINLINE()).calculateUnitCost() == 0))
      {
        aiUnitAIVal[UNITAI_ATTACK] +=
            ((iMilitaryWeight / ((bLandWar || bAssault) ? 9 : 16)) + ((bPrimaryArea && !bAreaAlone) ? 1 : 0));
        aiUnitAIVal[UNITAI_ATTACK_CITY] +=
            ((iMilitaryWeight / ((bLandWar || bAssault) ? 7 : 15)) + ((bPrimaryArea && !bAreaAlone) ? 1 : 0));
        aiUnitAIVal[UNITAI_COLLATERAL] +=
            ((iMilitaryWeight / ((bDefense) ? 8 : 14)) + ((bPrimaryArea && !bAreaAlone) ? 1 : 0));
        aiUnitAIVal[UNITAI_PILLAGE] +=
            ((iMilitaryWeight / ((bLandWar || bAssault) ? 10 : 19)) + ((bPrimaryArea && !bAreaAlone) ? 1 : 0));
        aiUnitAIVal[UNITAI_RESERVE] +=
            ((iMilitaryWeight / ((bLandWar) ? 12 : 17)) + ((bPrimaryArea && !bAreaAlone) ? 1 : 0));
        aiUnitAIVal[UNITAI_COUNTER] +=
            ((iMilitaryWeight / ((bLandWar || bAssault) ? 9 : 16)) + ((bPrimaryArea && !bAreaAlone) ? 1 : 0));
        aiUnitAIVal[UNITAI_PARADROP] +=
            ((iMilitaryWeight / ((bLandWar || bAssault) ? 4 : 8)) + ((bPrimaryArea && !bAreaAlone) ? 1 : 0));
        ...
        if (pWaterArea != NULL)
        {
          if ((GET_PLAYER(getOwnerINLINE()).getNumCities() > 3) || (area()->getNumUnownedTiles() < 10))
          {
            aiUnitAIVal[UNITAI_ATTACK_SEA] += std::min(
                ...
                ((((iCoastalCities * 2) + (iMilitaryWeight / 10)) / ((bAssault) ? 5 : 7)) + ((bPrimaryArea) ? 1 : 0)));
            aiUnitAIVal[UNITAI_RESERVE_SEA] +=
                std::min((pWaterArea->getNumTiles() / 150),
                         ((((iCoastalCities * 2) + (iMilitaryWeight / 11)) / 8) + ((bPrimaryArea) ? 1 : 0)));
          }
        }
      }
    }
          ...
          if (!(bGrowMore && isFoodProduction(eLoopUnit)))
          {
            if (canTrain(eLoopUnit))
            {
              ...
              if ((iValue > ((iBestOriginalValue * 2) / 3)) &&
                  ((eUnitAI != UNITAI_EXPLORE) || (iValue >= iBestOriginalValue)))
              {
                ...
                for (iJ = 0; iJ < GC.getNumPromotionInfos(); iJ++)
                {
                  if (isFreePromotion((PromotionTypes)iJ) &&
                      !GC.getUnitInfo(eLoopUnit).getFreePromotions((PromotionTypes)iJ))
                  {
                    if ((GC.getUnitInfo(eLoopUnit).getUnitCombatType() != NO_UNITCOMBAT) &&
                        GC.getPromotionInfo((PromotionTypes)iJ)
                            .getUnitCombat(GC.getUnitInfo(eLoopUnit).getUnitCombatType()))
                    {
                      iPromotionValue += 15;
                      break;
                    }
                ...
                iValue /= (GET_PLAYER(getOwnerINLINE()).getUnitClassCountPlusMaking((UnitClassTypes)iI) +
                ...
                {
                  iValue /= std::max(
                      1, (getProductionTurnsLeft(eLoopUnit, 0) + (GC.getUnitInfo(eLoopUnit).isSuicide() ? 1 : 4)));
                }
                else
                {
...
BuildingTypes CvCityAI::AI_bestBuildingThreshold(int iFocusFlags, int iMaxTurns, int iMinThreshold, bool bAsync,
                                                 AdvisorTypes eIgnoreAdvisor)
{
  ...
  for (iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
  {
    if (!(GET_PLAYER(getOwnerINLINE())
              .isBuildingClassMaxedOut(((BuildingClassTypes)iI),
                                       GC.getBuildingClassInfo((BuildingClassTypes)iI).getExtraPlayerInstances())))
    {
      eLoopBuilding = ((BuildingTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(iI)));

      if ((eLoopBuilding != NO_BUILDING) && (getNumBuilding(eLoopBuilding) < GC.getCITY_MAX_NUM_BUILDINGS()) &&
          (!isProductionAutomated() ||
           !(isWorldWonderClass((BuildingClassTypes)iI) || isNationalWonderClass((BuildingClassTypes)iI))))
      {
        //don't build wonders?
        if (((iFocusFlags == 0) || (iFocusFlags & BUILDINGFOCUS_WONDEROK) ||
             (iFocusFlags & BUILDINGFOCUS_WORLDWONDER)) ||
            !(isWorldWonderClass((BuildingClassTypes)iI) || isTeamWonderClass((BuildingClassTypes)iI) ||
              isNationalWonderClass((BuildingClassTypes)iI) || isLimitedWonderClass((BuildingClassTypes)iI)))
        {
            ...
            if (canConstruct(eLoopBuilding))
            {
              ...
              if (GC.getBuildingInfo(eLoopBuilding).getFreeBuildingClass() != NO_BUILDINGCLASS)
              {
                BuildingTypes eFreeBuilding =
                    (BuildingTypes)GC.getCivilizationInfo(getCivilizationType())
                        .getCivilizationBuildings(GC.getBuildingInfo(eLoopBuilding).getFreeBuildingClass());
                if (NO_BUILDING != eFreeBuilding)
                {
                  iValue += (AI_buildingValue(eFreeBuilding, iFocusFlags) *
                             (GET_PLAYER(getOwnerINLINE()).getNumCities() -
                              GET_PLAYER(getOwnerINLINE())
                                  .getBuildingClassCountPlusMaking(
                                      (BuildingClassTypes)GC.getBuildingInfo(eLoopBuilding).getFreeBuildingClass())));
                }
              }
              if (isProductionAutomated())
              {
              ...
              if (iValue > 0)
              {
                ...
                if (isWorldWonderClass((BuildingClassTypes)iI))
                {
                  if (iProductionRank <= std::min(3, ((GET_PLAYER(getOwnerINLINE()).getNumCities() + 2) / 3)))
                  {
                    ...
                    {
                      iTempValue =
                          GC.getASyncRand().get(GC.getLeaderHeadInfo(getPersonalityType()).getWonderConstructRand(),
                                                "Wonder Construction Rand ASYNC");
                    }
                    else
                    {
                      iTempValue = GC.getGameINLINE().getSorenRandNum(
                          GC.getLeaderHeadInfo(getPersonalityType()).getWonderConstructRand(),
                          "Wonder Construction Rand");
                    }
                ...
                {
                  bValid = (iTurnsLeft <=
                            GC.getGameINLINE().AI_turnsPercent(
                                iMaxTurns,
    ...
    if ((iFocusFlags == 0) || (iValue > 0) || (iPass == 0))
    {
      ...
      for (iCivic = 0; iCivic < GC.getNumCivicInfos(); iCivic++)
      {
            ...
            (pCivicInfo.getBuildingCivicComboBuilding() == iBuildingClass))
        {
          ...
          {
            iValue += (iFocusFlags & BUILDINGFOCUS_GOLD) ? pCivicInfo.getBuildingCivicComboGold()
                                                         : 2 * pCivicInfo.getBuildingCivicComboGold();
          };
        };
      };
      // 3MiroCivic: end
      ...
      if ((iFocusFlags & BUILDINGFOCUS_DEFENSE) || (iPass > 0))
      {
        if (!bAreaAlone)
        {
          if ((GC.getGameINLINE().getBestLandUnit() == NO_UNIT) ||
              !(GC.getUnitInfo(GC.getGameINLINE().getBestLandUnit()).isIgnoreBuildingDefense()))
          {
            ...
            iValue += (std::max(0, std::min(((kBuilding.getDefenseModifier() + getBuildingDefense() +
                                              2 * kBuilding.getBombardImmuneDefense()) -
                                             getNaturalDefense() - 10),
                                            kBuilding.getDefenseModifier() + 2 * kBuilding.getBombardImmuneDefense())) /
                       4);
          }
        }
      ...
      if (((iFocusFlags & BUILDINGFOCUS_HAPPY) || (iPass > 0)) && !isNoUnhappiness())
      {
        ...
        {
          iValue +=
              (std::min(-(((iWarWearinessModifer * iWarWearinessPercentAnger) / 100) / GC.getPERCENT_ANGER_DIVISOR()),
                        iAngryPopulation) *
               8);
        ...
        {
          iValue +=
              (-(((iGlobalWarWearinessModifer * iWarWearinessPercentAnger / 100) / GC.getPERCENT_ANGER_DIVISOR())) *
               iNumCities);
        ...
        {
          iValue +=
              (std::min(((kBuilding.getCommerceHappiness(iI) * kOwner.getCommercePercent((CommerceTypes)iI)) / 100),
                        iAngryPopulation) *
               8);
        ...
        for (iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
        {
          ...
          {
            iValue +=
                (kBuilding.getBuildingHappinessChanges(iI) * kOwner.getBuildingClassCount((BuildingClassTypes)iI) * 8);
          }
        }
      }
      ...
      if ((iFocusFlags & BUILDINGFOCUS_EXPERIENCE) || (iPass > 0))
      {
        ...
        for (iI = 0; iI < NUM_DOMAIN_TYPES; iI++)
        {
          ...
          iValue +=
              (kBuilding.getDomainFreeExperience(iI) * ((iHasMetCount > 0) ? iDomainExpValue : iDomainExpValue / 2));
        }
      }
      ...
      if ((iFocusFlags & BUILDINGFOCUS_MAINTENANCE) || (iFocusFlags & BUILDINGFOCUS_GOLD) || (iPass > 0))
      {
        ...
        int iNewUpkeep =
            (iBaseMaintenance * std::max(0, 100 + getMaintenanceModifier() + kBuilding.getMaintenanceModifier())) / 100;
...
*/
      ...
      if (iPass > 0)
      {
        ...
        if (iAirDefense > 0)
        {
          if (((kOwner.AI_totalUnitAIs(UNITAI_DEFENSE_AIR) > 0) && (kOwner.AI_totalUnitAIs(UNITAI_ATTACK_AIR) > 0)) ||
        ...
        for (iI = 0; iI < GC.getNumSpecialistInfos(); iI++)
        {
          ...
          {
            iValue +=
                ((AI_specialistValue(((SpecialistTypes)iI), false, false) * kBuilding.getFreeSpecialistCount(iI)) / 50);
          }
        }
        ...
        for (iI = 0; iI < GC.getNumImprovementInfos(); iI++)
        {
          ...
          {
            iValue +=
                kBuilding.getImprovementFreeSpecialist(iI) * countNumImprovedPlots((ImprovementTypes)iI, true) * 50;
          }
        }
        ...
        for (iI = 0; iI < GC.getNumUnitInfos(); iI++)
        {
          ...
          {
            if (kOwner.AI_totalAreaUnitAIs(area(),
                                           ((UnitAITypes)(GC.getUnitInfo((UnitTypes)iI).getDefaultUnitAIType()))) == 0)
            {
              iValue += iNumCitiesInArea;
            }
          ...
          if (iPrereqBuildings > 0 && iPrereqBuildings <= iNumCities)
          {
            ...
            if (iPrereqBuildings > kOwner.getBuildingClassCountPlusMaking(eBuildingClass))
            {
              ...
              if (bCulturalVictory1)
              {
                ...
                {
                  int iLoopBuildingsBuilt =
                      kOwner.getBuildingClassCount((BuildingClassTypes)kLoopBuilding.getBuildingClassType());
      ...
      if (iPass > 0)
      {
        for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
        {
          ...
          {
            iTempValue +=
                kBuilding.getSeaPlotYieldChange(iI) * AI_buildingSpecialYieldChangeValue(eBuilding, (YieldTypes)iI);
          }
          ...
          iTempValue += ((kBuilding.getPowerYieldModifier(iI) * getBaseYieldRate((YieldTypes)iI)) /
                         ((bProvidesPower || isPower()) ? 12 : 15));
      ...
      else
      {
        if (iFocusFlags & BUILDINGFOCUS_FOOD)
        {
          ...
          if (kBuilding.getSeaPlotYieldChange(YIELD_FOOD) > 0)
          {
            ...
            iTempValue =
                kBuilding.getSeaPlotYieldChange(YIELD_FOOD) * AI_buildingSpecialYieldChangeValue(eBuilding, YIELD_FOOD);
        ...
        if (iFocusFlags & BUILDINGFOCUS_PRODUCTION)
        {
          ...
          iTempValue += ((kBuilding.getPowerYieldModifier(YIELD_PRODUCTION) * getBaseYieldRate(YIELD_PRODUCTION)) /
      ...
      if (iPass > 0)
      {
        for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
        {
          ...
          if ((iI == COMMERCE_CULTURE) && (kBuilding.getStateReligionCulture() > 0) &&
              (GET_PLAYER(getOwner()).getStateReligion() == kBuilding.getReligionType()))
          {
            ...
            if (bCulturalVictory1)
            {
              ...
              else
              {
                ...
                for (iJ = 0; bHaveEnough && iJ < GC.getNumBuildingClassInfos(); iJ++)
                {
                  ...
                  int iPrereqBuildings =
                      kOwner.getBuildingClassPrereqBuilding(eBuilding, (BuildingClassTypes)iJ, -iCountBuilt);
          ...
          {
            iTempValue +=
                (GC.getReligionInfo((ReligionTypes)(kBuilding.getGlobalReligionCommerce()))
                     .getGlobalReligionCommerce(iI) *
                 GC.getGameINLINE().countReligionLevels((ReligionTypes)(kBuilding.getGlobalReligionCommerce())) * 2);
          ...
          if (iCorpValue >= 0) //Don't build if it'll hurt us.
          {
            ...
            {
              int iGoldValue = (GC.getCorporationInfo((CorporationTypes)(kBuilding.getGlobalCorporationCommerce()))
                                    .getHeadquarterCommerce(iI) *
                                GC.getGameINLINE().countCorporationLevels(
                                    (CorporationTypes)(kBuilding.getGlobalCorporationCommerce())) *
                                2);

              iGoldValue += GC.getCorporationInfo((CorporationTypes)(kBuilding.getGlobalCorporationCommerce()))
                                .getHeadquarterCommerce(iI);
              if (iGoldValue > 0)
              {
      ...
      else
      {
        if (iFocusFlags & BUILDINGFOCUS_GOLD)
        {
          ...
          {
            iTempValue +=
                ((kBuilding.getBonusCommerceModifier(iJ, COMMERCE_GOLD) * getBaseCommerceRate(COMMERCE_GOLD)) / 40);
          }
          //BCM:End
        ...
        {
          iTempValue =
              ((kBuilding.getCommerceModifier(COMMERCE_RESEARCH) * getBaseCommerceRate(COMMERCE_RESEARCH)) / 40);
          //BCM:Added 27.9.09
          for (iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
          {
            iTempValue +=
                ((kBuilding.getBonusCommerceModifier(iJ, COMMERCE_RESEARCH) * getBaseCommerceRate(COMMERCE_RESEARCH)) /
                 40);
          }
          //BCM:End
        ...
        if (iFocusFlags & BUILDINGFOCUS_CULTURE)
        {
          ...
          {
            iValue +=
                ((kBuilding.getBonusCommerceModifier(iJ, COMMERCE_CULTURE) * getBaseCommerceRate(COMMERCE_CULTURE)) /
                 15);
          }
          ...
          {
            iValue +=
                ((kBuilding.getCommerceModifier(COMMERCE_ESPIONAGE) * getBaseCommerceRate(COMMERCE_ESPIONAGE)) / 15);

            for (iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
            {
              iValue += ((kBuilding.getBonusCommerceModifier(iJ, COMMERCE_ESPIONAGE) *
                          getBaseCommerceRate(COMMERCE_ESPIONAGE)) /
                         15);
            }
          }
        }
        ...
        if (iFocusFlags & BUILDINGFOCUS_ESPIONAGE ||
            (GC.getGameINLINE().isOption(GAMEOPTION_NO_ESPIONAGE) && (iFocusFlags & BUILDINGFOCUS_CULTURE)))
        {
          iTempValue =
              ((kBuilding.getCommerceModifier(COMMERCE_ESPIONAGE) * getBaseCommerceRate(COMMERCE_ESPIONAGE)) / 60);

          for (iJ = 0; iJ < GC.getNumBonusInfos(); iJ++)
          {
            iTempValue += ((kBuilding.getBonusCommerceModifier(iJ, COMMERCE_ESPIONAGE) *
                            getBaseCommerceRate(COMMERCE_ESPIONAGE)) /
                           60);
          }
  ...
  for (iI = 0; iI < GC.getNumProjectInfos(); iI++)
  {
    if (canCreate((ProjectTypes)iI))
    {
      ...
      if (iValue > 0)
      {
        ...
        if ((iTurnsLeft <= GC.getGameINLINE().AI_turnsPercent(
                               10, GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getCreatePercent())) ||
            !(GET_TEAM(getTeam()).isHuman()))
        {
          if ((iTurnsLeft <= GC.getGameINLINE().AI_turnsPercent(
                                 20, GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getCreatePercent())) ||
              (iProductionRank <= std::max(3, (GET_PLAYER(getOwnerINLINE()).getNumCities() / 2))))
          {
            if (iProductionRank == 1)
            {
              iValue += iTurnsLeft;
...
int CvCityAI::AI_projectValue(ProjectTypes eProject)
{
  ...
  //Start Sedna17
  iValue = 0;
  // 3Miro: try to improve this
  //iValue = 100; //Attempting to increase project building chance
  //End

  // 3MiroAI: count the resource bonuses given
  for (iI = 0; iI < GC.getNumBonusInfos(); iI++)
  {
    //if ( GC.getProjectInfo(eProject).getFreeBonus( (BonusTypes) iI ) > 0 ){
    iValue += 5 * GC.getProjectInfo(eProject).getFreeBonus((BonusTypes)iI);
    //};
  };
  ...
  for (iI = 0; iI < GC.getNumProjectInfos(); iI++)
  {
    ...
    if ((GC.getProjectInfo((ProjectTypes)iI).getPrereqProject(eProject) > 0) &&
        (GET_PLAYER(getOwner()).canCreate((ProjectTypes)iI)))
    {
      iValue += 1;
    };
  };
  ...
  for (iI = 0; iI < GC.getNumVictoryInfos(); iI++)
  {
    ...
    {
      iValue += (std::max(0, (GC.getProjectInfo(eProject).getVictoryThreshold(iI) -
  ...
  for (iI = 0; iI < GC.getNumProjectInfos(); iI++)
  {
    iValue += (std::max(0, (GC.getProjectInfo((ProjectTypes)iI).getProjectsNeeded(eProject) -
                            GET_TEAM(getTeam()).getProjectCount(eProject))) *
               10);
  }
...
bool CvCityAI::AI_isDefended(int iExtra)
{
  ...
  return ((plot()->plotCount(PUF_canDefendGroupHead, -1, -1, getOwnerINLINE(), NO_TEAM, PUF_isCityAIType) + iExtra) >=
          AI_neededDefenders()); // XXX check for other team's units?
}
...
bool CvCityAI::AI_isAirDefended(int iExtra)
{
  ...
  return ((plot()->plotCount(PUF_canAirDefend, -1, -1, getOwnerINLINE(), NO_TEAM, PUF_isDomainType, DOMAIN_AIR) +
           iExtra) >= AI_neededAirDefenders()); // XXX check for other team's units?
}
...
void CvCityAI::AI_updateBestBuild()
{
      ...
      if (NULL != pLoopPlot && pLoopPlot->getWorkingCity() == this)
      {
        ...
        if (m_aeBestBuild[iI] != NO_BUILD && m_aiBestBuildValue[iI] > 0)
        {
          if (iCount > 0)
          {
            ...
            if (eImprovement != NO_IMPROVEMENT)
            {
              ...
              {
                aiFinalYields[iJ] = (pLoopPlot->calculateNatureYield(((YieldTypes)iJ), getTeam(), bIgnoreFeature) +
                                     pLoopPlot->calculateImprovementYieldChange(eImprovement, ((YieldTypes)iJ),
        ...
        if (bUseBaseValue)
        {
          for (iJ = 0; iJ < NUM_YIELD_TYPES; iJ++)
          {
            ...
            {
              aiFinalYields[iJ] +=
                  std::max(0, -GC.getFeatureInfo(pLoopPlot->getFeatureType()).getYieldChange((YieldTypes)iJ));
            }
          }
        }
        ...
        if (pLoopPlot->isBeingWorked() || (((aiFinalYields[YIELD_FOOD] * 10) + (aiFinalYields[YIELD_PRODUCTION] * 6) +
                                            (aiFinalYields[YIELD_COMMERCE] * 4)) > 21))
        {
          iGoodTileCount++;
          if (pLoopPlot->isBeingWorked())
          {
        ...
        if ((pLoopPlot->getFeatureType()) != NO_FEATURE)
        {
          iFeatureFoodSurplus += std::max(0, pLoopPlot->calculateNatureYield(YIELD_FOOD, getTeam()) -
                                                 GC.getFOOD_CONSUMPTION_PER_POPULATION());
        }

        if ((pLoopPlot->isHills()))
        {
          iHillFoodDeficit += std::max(0, GC.getFOOD_CONSUMPTION_PER_POPULATION() -
                                              pLoopPlot->calculateNatureYield(YIELD_FOOD, getTeam()));
        }
      }
    }
  }
  ...
  if (iWorkableFoodPlotCount > 0)
  {
    ...
    iSpecialistAdjustment =
        (std::min(iSpecialistCount, iWorkableFoodPlotCount) * iWorkableFood) / iWorkableFoodPlotCount;
  }
...
*/
  ...
  {
    BuildingTypes eProductionBuilding = getProductionBuilding();
    bChop = (eProductionBuilding != NO_BUILDING &&
             isWorldWonderClass((BuildingClassTypes)(GC.getBuildingInfo(eProductionBuilding).getBuildingClassType())));
  }
  if (!bChop)
  {
  ...
  if (!bChop)
  {
    bChop = ((area()->getAreaAIType(getTeam()) == AREAAI_OFFENSIVE) ||
  ...
  for (iI = 0; iI < NUM_CITY_PLOTS; iI++)
  {
    ...
    if (iI != CITY_HOME_PLOT)
    {
      ...
      if (NULL != pLoopPlot && pLoopPlot->getWorkingCity() == this)
      {
        AI_bestPlotBuild(pLoopPlot, &(m_aiBestBuildValue[iI]), &(m_aeBestBuild[iI]), iFoodMultiplier,
                         iProductionMultiplier, iCommerceMultiplier, bChop, iHappyAdjust, iHealthAdjust,
                         iDesiredFoodChange);
                ...
                if (GC.getBuildInfo(m_aeBestBuild[iI]).getImprovement() != NO_IMPROVEMENT)
                {
                  if ((aiValues[iI] <= iBestUnworkedPlotValue) && (aiValues[iI] < 500))
                  {
                    m_aiBestBuildValue[iI] = 1;
                       ...
                       (GC.getBuildInfo(m_aeBestBuild[iI]).getImprovement() != NO_IMPROVEMENT))
              {
                for (iJ = 0; iJ < NUM_YIELD_TYPES; iJ++)
                {
                  aiYields[iJ] = pLoopPlot->getYield((YieldTypes)iJ);
...
void CvCityAI::AI_doDraft(bool bForce)
{
  ...
  if (canConscript())
  {
    if (GC.getUnitInfo(getConscriptUnit()).getCombat() > 5)
    {
      ...
      bool bLandWar = ((area()->getAreaAIType(getTeam()) == AREAAI_OFFENSIVE) ||
                       (area()->getAreaAIType(getTeam()) == AREAAI_DEFENSIVE) ||
                       (area()->getAreaAIType(getTeam()) == AREAAI_MASSING));
  ...
  for (iI = 0; iI < GC.getNumHurryInfos(); iI++)
  {
    if (canHurry((HurryTypes)iI))
    {
      ...
      if ((iHurryAngerLength == 0) && (iHurryPopulation == 0))
      {
        ...
        {
          int iValuePerTurn = AI_buildingValueThreshold(
              eProductionBuilding, BUILDINGFOCUS_GOLD | BUILDINGFOCUS_MAINTENANCE | BUILDINGFOCUS_PRODUCTION);
...
*/
      ...
      if (eProductionBuilding != NO_BUILDING)
      {
        ...
        {
          if ((getCommerceRateTimes100(COMMERCE_CULTURE) == 0) ||
              (plot()->calculateCulturePercent(getOwnerINLINE()) < 40))
          {
            iMinTurns = std::min(iMinTurns, 10);
            if (getCommerceRateTimes100(COMMERCE_CULTURE) == 0)
            {
      ...
      if (eProductionBuilding != NO_BUILDING)
      {
        if (GC.getBuildingInfo(eProductionBuilding).getSeaPlotYieldChange(YIELD_FOOD) > 0 ||
            GC.getBuildingInfo(eProductionBuilding).getCoastalPlotYieldChange(YIELD_FOOD) > 0 ||
            GC.getBuildingInfo(eProductionBuilding).getRiverPlotYieldChange(YIELD_FOOD) > 0)
        {
      ...
      {
        if (GET_PLAYER(getOwnerINLINE()).AI_neededWorkers(area()) >
            (area()->getNumAIUnits(getOwnerINLINE(), UNITAI_WORKER) * 2))
        {
          iMinTurns = std::min(iMinTurns, 5);
          bEssential = true;
          bGrowth = true;
        ...
        if (getProductionTurnsLeft() > iMinTurns)
        {
          ...
          if (!bWait)
          {
            if (iHurryAngerLength > 0)
            {
              ...
              if (!bEssential && ((iHurryPopulation < (1 + GC.getDefineINT("HURRY_POP_ANGER"))) ||
                                  ((getPopulation() - iHurryPopulation) <= std::max(3, (getHighestPopulation() / 2)))))
              {
                bWait = true;
              }
              else
                ...
                for (iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
                {
                  if (iJ != CITY_HOME_PLOT)
                  {
                    ...
                    if (pLoopPlot != NULL)
                    {
                      ...
                      {
                        iFoodSurplus +=
                            std::max(0, pLoopPlot->getYield(YIELD_FOOD) - GC.getFOOD_CONSUMPTION_PER_POPULATION());
                      }
                    }
                  }
                }
...
bool CvCityAI::AI_bestSpreadUnit(bool bMissionary, bool bExecutive, int iBaseChance, UnitTypes *eBestSpreadUnit,
                                 int *iBestSpreadUnitValue)
{
  CvPlayerAI &kPlayer = GET_PLAYER(getOwnerINLINE());
        ...
        if (kPlayer.getStateReligion() == eReligion)
        {
          ...
          if (iRoll > kGame.getSorenRandNum(100, "AI choose missionary"))
          {
            ...
            if (iReligionValue > 0)
            {
              ...
              {
                UnitTypes eLoopUnit =
                    ((UnitTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(iI)));
      ...
      if (canTrain((UnitTypes)UNIT_PROSECUTOR))
      {
        ...
        for (int iI = 0; iI < GC.getNumCivicInfos(); iI++)
        {
          if ((GET_PLAYER(getOwnerINLINE()).isCivic((CivicTypes)iI)) &&
              (GC.getCivicInfo((CivicTypes)iI).getNonStateReligionHappiness() > 0))
          {
            bOpressor = false;
          }
        }
        ...
        if (bOpressor)
        {
          ...
          if (lResult == 1)
          {
            ...
            {
              int iProb =
                  6 * GET_PLAYER(getOwnerINLINE()).AI_getFlavorValue((FlavorTypes)1) + 30; // max 90, Flavor religion
              int iMaxCount =
                  (getOwnerINLINE() == PAPAL_PLAYER)
                      ? 5
                      : 2; // Papal player used to be able to build many persecutors (when it had special rules for persecution), now it doesn't really matter
              if ((GET_PLAYER(getOwnerINLINE()).getUnitClassCount((UnitClassTypes)UNIT_PROSECUTOR_CLASS) < iMaxCount) &&
                  (GC.getGameINLINE().getSorenRandNum(100, " Shall we build a Prosecutor") < iProb))
              {
                ...
                {
                  UnitTypes eProsecutorUnit = ((UnitTypes)(GC.getCivilizationInfo(getCivilizationType())
                                                               .getCivilizationUnits(UNIT_PROSECUTOR_CLASS)));
                  ...
                  if (canTrain(eLoopUnit))
                  {
                         ...
                         pLoopUnit = kPlayer.nextUnit(&iLoop))
                    {
                      if ((pLoopUnit->AI_getUnitAIType() == UNITAI_MISSIONARY) &&
                          (pLoopUnit->getUnitInfo().getCorporationSpreads(eCorporation) > 0))
                      {
                        iTotalCount++;
      ...
      for (int iI = 0; iI < GC.getNumSpecialistInfos(); iI++)
      {
        if (isSpecialistValid((SpecialistTypes)iI, 1))
        {
          ...
          if (iForcedSpecialistCount > 0)
          {
            ...
            int iForcedValue = ((iForcedSpecialistCount * 128) / iTotalForcedSpecialists) -
                               ((iSpecialistCount * 128) / iTotalSpecialists);
      ...
      if (iI != CITY_HOME_PLOT)
      {
        if (!isWorkingPlot(iI))
        {
          ...
          if (pLoopPlot != NULL)
          {
            ...
            {
              int iValue =
                  AI_plotValue(pLoopPlot, bAvoidGrowth, /*bRemove*/ false, /*bIgnoreFood*/ false, bIgnoreGrowth);
  ...
  if (extraFreeSpecialists() < 0)
  {
    ...
    if (GC.getDefineINT("DEFAULT_SPECIALIST") != NO_SPECIALIST)
    {
      ...
      if (eIgnoreSpecialist != GC.getDefineINT("DEFAULT_SPECIALIST"))
      {
        ...
        if (getSpecialistCount((SpecialistTypes)(GC.getDefineINT("DEFAULT_SPECIALIST"))) >
            getForceSpecialistCount((SpecialistTypes)(GC.getDefineINT("DEFAULT_SPECIALIST"))))
        {
...
bool CvCityAI::AI_foodAvailable(int iExtra)
{
  ...
  for (iI = 0; iI < GC.getNumSpecialistInfos(); iI++)
  {
    iFoodCount += (GC.getSpecialistInfo((SpecialistTypes)iI).getYieldChange(YIELD_FOOD) *
...
int CvCityAI::AI_yieldValue(short *piYields, short *piCommerceYields, bool bAvoidGrowth, bool bRemove, bool bIgnoreFood,
                            bool bIgnoreGrowth, bool bIgnoreStarvation, bool bWorkerOptimization)
{
  ...
  for (int iJ = 0; iJ < NUM_COMMERCE_TYPES; iJ++)
  {
    ...
    int iCommerceTimes100 =
        aiYields[YIELD_COMMERCE] * GET_PLAYER(getOwnerINLINE()).getCommercePercent((CommerceTypes)iJ);
  ...
  if (isProductionProcess() && !bWorkerOptimization)
  {
    for (int iJ = 0; iJ < NUM_COMMERCE_TYPES; iJ++)
    {
      aiCommerceYieldsTimes100[iJ] +=
          GC.getProcessInfo(getProductionProcess()).getProductionToCommerceModifier(iJ) * aiYields[YIELD_PRODUCTION];
    }
...
*/
  ...
  if (!bIgnoreFood && aiYields[YIELD_FOOD] != 0)
  {
    ...
    int iExtraPopulationThatCanWork =
        std::min(iPopulation - range(-iHappinessLevel, 0, iPopulation) + std::min(0, extraFreeSpecialists()),
                 NUM_CITY_PLOTS) -
        getWorkingPopulation() + ((bRemove) ? 1 : 0);
    int iConsumtionPerPop = GC.getFOOD_CONSUMPTION_PER_POPULATION();

    int iAdjustedFoodDifference =
        (getYieldRate(YIELD_FOOD) + std::min(0, iHealthLevel)) -
        ((iPopulation + std::min(0, iHappinessLevel) - ((bRemove) ? 1 : 0)) * iConsumtionPerPop);
    ...
    if (!bIgnoreStarvation)
    {
      ...
      if ((iFoodPerTurn + iStarvingAllowance) < 0)
      {
        ...
        if (std::max(0, iExtraPopulationThatCanWork * aiYields[YIELD_FOOD]) >= -iFoodPerTurn)
        {
          ...
          if (iFoodPerTurn <= (AI_isEmphasizeGreatPeople() ? 0 : -iHighFoodThreshold) &&
              aiYields[YIELD_FOOD] >= iHighFoodThreshold)
          {
    ...
    if (bWorkerOptimization || !bFoodIsProduction)
    {
      ...
      if (!bAvoidGrowth)
      {
        ...
        if (bWorkerOptimization || (!bIgnoreGrowth)) // && !bEmphasizeFood))
        {
          ...
          if (iHappinessLevel < 10)
          {
            ...
            if (getMilitaryHappinessUnits() == 0)
            {
              if (GET_PLAYER(getOwnerINLINE()).getNumCities() > 2)
              {
                iHappinessLevel += ((GC.getDefineINT("NO_MILITARY_PERCENT_ANGER") * (iPopulation + 1)) /
        ...
        if ((iExtraPopulationThatCanWork > 1) && AI_isEmphasizeGreatPeople())
        {
          ...
          iFoodGPPValue +=
              std::max(0, aiYields[YIELD_FOOD] - iAdjust) * std::max(0, (12 + 5 * std::min(0, iHappinessLevel)));
        }
      }
    }
  }
                           ...
                           bool bIgnoreStarvation)
{
  ...
  int iYieldValue =
      (AI_yieldValue(aiYields, NULL, bAvoidGrowth, bRemove, bIgnoreFood, bIgnoreGrowth, bIgnoreStarvation) * 100);

  if (eFinalImprovement != NO_IMPROVEMENT)
  {
    for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
    {
      iYieldDiff = (pPlot->calculateImprovementYieldChange(eFinalImprovement, ((YieldTypes)iI), getOwnerINLINE()) -
                    pPlot->calculateImprovementYieldChange(eCurrentImprovement, ((YieldTypes)iI), getOwnerINLINE()));
  ...
  if (eCurrentImprovement != NO_IMPROVEMENT)
  {
    if (pPlot->getBonusType(getTeam()) ==
        NO_BONUS) // XXX double-check CvGame::doFeature that the checks are the same...
    {
      for (iI = 0; iI < GC.getNumBonusInfos(); iI++)
      {
  ...
  if ((eCurrentImprovement != NO_IMPROVEMENT) &&
      (GC.getImprovementInfo(pPlot->getImprovementType()).getImprovementUpgrade() != NO_IMPROVEMENT))
  {
    iValue += 200;
...
// Improved worker AI provided by Blake - thank you!
void CvCityAI::AI_bestPlotBuild(CvPlot *pPlot, int *piBestValue, BuildTypes *peBestBuild, int iFoodPriority,
                                int iProductionPriority, int iCommercePriority, bool bChop, int iHappyAdjust,
                                int iHealthAdjust, int iFoodChange)
{
  PROFILE_FUNC();
        ...
        if ((pAdjacentPlot != NULL) && (pAdjacentPlot->getOwner() == getOwner()) && (pAdjacentPlot->isCityRadius()))
        {
          if (!pAdjacentPlot->isFreshWater())
          {
            ...
            if (pAdjacentPlot->isPotentialIrrigation())
            {
              ...
              {
                pAdjacentPlot2 =
                    plotDirection(pAdjacentPlot->getX_INLINE(), pAdjacentPlot->getY_INLINE(), ((DirectionTypes)iJ));
    ...
    if (bValid)
    {
          ...
          if (GC.getImprovementInfo(eFinalImprovement).isImprovementBonusTrade(eNonObsoleteBonus))
          {
            ...
            if (eBestBuild != NO_BUILD)
            {
              if ((GC.getBuildInfo(eBestBuild).getImprovement() == NO_IMPROVEMENT) ||
                  (!GC.getImprovementInfo((ImprovementTypes)GC.getBuildInfo(eBestBuild).getImprovement())
      ...
      if (iValue >= 0)
      {
        ...
        for (iJ = 0; iJ < NUM_YIELD_TYPES; iJ++)
        {
          ...
          aiFinalYields[iJ] +=
              (pPlot->calculateImprovementYieldChange(eFinalImprovement, ((YieldTypes)iJ), getOwnerINLINE(), false));
          aiFinalYields[iJ] +=
              (pPlot->calculateImprovementYieldChange(eImprovement, ((YieldTypes)iJ), getOwnerINLINE(), false));
        ...
        if (iValue > 0)
        {
          ...
          if (aiFinalYields[YIELD_FOOD] >= GC.getFOOD_CONSUMPTION_PER_POPULATION())
          {
            ...
            if (iFoodChange > 0)
            {
              iValue += (10 * (1 + aiDiffYields[YIELD_FOOD]) *
                         (1 + aiFinalYields[YIELD_FOOD] - GC.getFOOD_CONSUMPTION_PER_POPULATION()) * iFoodChange *
                         iFoodPriority) /
                        100;
            }
          ...
          {
            if ((aiDiffYields[YIELD_PRODUCTION] > 0) &&
                (aiFinalYields[YIELD_FOOD] + aiFinalYields[YIELD_PRODUCTION] > 3))
            {
        ...
        if ((iHappiness != 0) && !(GET_PLAYER(getOwnerINLINE()).getAdvancedStartPoints() >= 0))
        {
          ...
          {
            iHappyValue +=
                200 * std::max(0, (bCanGrow ? std::min(6, 2 + iHealthLevel - iHappyLevel) : 0) - iHappyLevel);
          }
          else
          {
        ...
        {
          iValue *= std::max(
              0, (GC.getLeaderHeadInfo(getPersonalityType()).getImprovementWeightModifier(eFinalImprovement) + 200));
          iValue /= 200;
        }
        ...
        else
        {
          ...
          ImprovementTypes eImprovementDowngrade =
          ...
          {
            iValue -= (GC.getImprovementInfo(pPlot->getImprovementType()).getUpgradeTime() * 8 *
                       (pPlot->getUpgradeProgress())) /
                      std::max(1, GC.getGameINLINE().getImprovementUpgradeTime(pPlot->getImprovementType()));
          }
          ...
          if (eNonObsoleteBonus == NO_BONUS)
          {
            ...
            {
              if (((iFoodPriority < 100) && (aiFinalYields[YIELD_FOOD] >= GC.getFOOD_CONSUMPTION_PER_POPULATION())) ||
                  (GC.getImprovementInfo(pPlot->getImprovementType()).getImprovementPillage() != NO_IMPROVEMENT))
              {
                iValue -= 70;
                iValue *= 2;
                iValue /= 3;
  ...
  if (iClearFeatureValue > 0)
  {
...

    {
      if ((GC.getFeatureInfo(pPlot->getFeatureType()).getHealthPercent() < 0) ||
          ((GC.getFeatureInfo(pPlot->getFeatureType()).getYieldChange(YIELD_FOOD) +
            GC.getFeatureInfo(pPlot->getFeatureType()).getYieldChange(YIELD_PRODUCTION) +
            GC.getFeatureInfo(pPlot->getFeatureType()).getYieldChange(YIELD_COMMERCE)) < 0))
      {
...
bool CvCityAI::AI_doPanic()
{
  ...
  bool bLandWar =
      ((area()->getAreaAIType(getTeam()) == AREAAI_OFFENSIVE) ||
       (area()->getAreaAIType(getTeam()) == AREAAI_DEFENSIVE) || (area()->getAreaAIType(getTeam()) == AREAAI_MASSING));
    ...
    if (pLoopPlot != NULL)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          if (eNonObsoleteBonus != NO_BONUS)
          {
            iTempValue += (GET_PLAYER(getOwnerINLINE()).AI_bonusVal(eNonObsoleteBonus) *
...
void CvCityAI::AI_buildGovernorChooseProduction()
{
  ...
  // if we need to pop borders, then do that immediately if we have drama and can do it
  if ((getCultureLevel() <= (CultureLevelTypes)1) &&
      ((getCommerceRate(COMMERCE_CULTURE) < 2) || (iCulturePressure > 0)))
  {
    if (AI_chooseProcess(COMMERCE_CULTURE))
    {
      return;
...
int CvCityAI::AI_getYieldMagicValue(const int *piYieldsTimes100, bool bHealthy)
{
  ...
  int iValue = ((piYieldsTimes100[YIELD_FOOD] * 100 + piYieldsTimes100[YIELD_PRODUCTION] * 55 +
...
int CvCityAI::AI_getPlotMagicValue(CvPlot *pPlot, bool bHealthy, bool bWorkerOptimization)
{
  ...
  for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
  {
    if ((bWorkerOptimization) && (pPlot->getWorkingCity() == this) &&
  ...
  if (eCurrentImprovement != NO_IMPROVEMENT)
  {
    ...
    if ((eFinalImprovement != NO_IMPROVEMENT) && (eFinalImprovement != eCurrentImprovement))
    {
      ...
      {
        iYieldDiff =
            100 * pPlot->calculateImprovementYieldChange(eFinalImprovement, ((YieldTypes)iI), getOwnerINLINE());
        iYieldDiff -=
            100 * pPlot->calculateImprovementYieldChange(eCurrentImprovement, ((YieldTypes)iI), getOwnerINLINE());
...
int CvCityAI::AI_buildingSpecialYieldChangeValue(BuildingTypes eBuilding, YieldTypes eYield)
{
  ...
  if (iWorkedCount == 0)
  {
    ...
    if ((getPopulation() > 2) &&
        ((eDefaultSpecialist == NO_SPECIALIST) || (getSpecialistCount(eDefaultSpecialist) == 0)))
    {
      iValue /= 2;
    }
  }
...
void CvCityAI::AI_updateSpecialYieldMultiplier()
{
  ...
  if (eProductionBuilding != NO_BUILDING)
  {
    ...
    m_aiSpecialYieldMultiplier[YIELD_PRODUCTION] +=
        std::max(-25, GC.getBuildingInfo(eProductionBuilding).getFoodKept());
  ...
  if (!isHuman())
  {
    ...
    if ((kPlayer.AI_isDoStrategy(AI_STRATEGY_DAGGER) && getPopulation() >= 4) || (eAreaAIType == AREAAI_OFFENSIVE) ||
        (eAreaAIType == AREAAI_DEFENSIVE) || (eAreaAIType == AREAAI_MASSING) || (eAreaAIType == AREAAI_ASSAULT))
    {
    ...
    int iIncome = 1 + kPlayer.getCommerceRate(COMMERCE_GOLD) + kPlayer.getCommerceRate(COMMERCE_RESEARCH) +
                  std::max(0, kPlayer.getGoldPerTurn());
...
int CvCityAI::AI_countNumBonuses(BonusTypes eBonus, bool bIncludeOurs, bool bIncludeNeutral, int iOtherCultureThreshold,
                                 bool bLand, bool bWater)
{
  CvPlot *pLoopPlot;
      ...
      if ((pLoopPlot->area() == area()) || (bWater && pLoopPlot->isWater()))
      {
        ...
        if (eLoopBonus != NO_BONUS)
        {
          if ((eBonus == NO_BONUS) || (eBonus == eLoopBonus))
          {
            ...
            else if ((iOtherCultureThreshold > 0) &&
                     (pLoopPlot->isOwned() && (pLoopPlot->getOwnerINLINE() != getOwnerINLINE())))
            {
              if ((pLoopPlot->getCulture(pLoopPlot->getOwnerINLINE()) - pLoopPlot->getCulture(getOwnerINLINE())) <
...
int CvCityAI::AI_cityThreat(bool bDangerPercent)
{
  ...
  for (int iI = 0; iI < MAX_PLAYERS; iI++)
  {
    if ((iI != getOwner()) && GET_PLAYER((PlayerTypes)iI).isAlive())
    {
      ...
      {
        if ((bCrushStrategy) &&
            (GET_TEAM(getTeam()).AI_getWarPlan(GET_PLAYER((PlayerTypes)iI).getTeam()) != NO_WARPLAN))
        {
          iTempValue *= 400;
        }
  ...
  for (int iI = 0; iI < NUM_CITY_PLOTS; iI++)
  {
    ...
    if (NULL != pLoopPlot && pLoopPlot->getWorkingCity() == this)
    {
      if (pLoopPlot->getArea() == getArea())
      {
        ...
        iWorkersHave +=
            pLoopPlot->plotCount(PUF_isUnitAIType, UNITAI_WORKER, -1, getOwner(), getTeam(), PUF_isNoMission, -1, -1);
        if (iI != CITY_HOME_PLOT)
        {
          if (pLoopPlot->getImprovementType() == NO_IMPROVEMENT)
          {
