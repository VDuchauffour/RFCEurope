...
// trs.sound-once
void CvDeal::kill(bool bKillTeam, bool bPlaySound)
{
  if ((getLengthFirstTrades() > 0) || (getLengthSecondTrades() > 0))
  {
    ...
    if (GET_TEAM(GET_PLAYER(getFirstPlayer()).getTeam()).isHasMet(GET_PLAYER(getSecondPlayer()).getTeam()))
    {
      ...
      gDLL->getInterfaceIFace()->addHumanMessage((PlayerTypes)getFirstPlayer(), true, GC.getEVENT_MESSAGE_TIME(),
                                                 szString,
    ...
    if (GET_TEAM(GET_PLAYER(getSecondPlayer()).getTeam()).isHasMet(GET_PLAYER(getFirstPlayer()).getTeam()))
    {
      ...
      szString.Format(L"%s: %s", szCancelString.GetCString(), szDealString.getCString());
      gDLL->getInterfaceIFace()->addHumanMessage((PlayerTypes)getSecondPlayer(), true, GC.getEVENT_MESSAGE_TIME(),
                                                 szString,
                                                 bPlaySound ? "AS2D_DEAL_CANCELLED" : NULL // trs.sound-once
      );
    }
  ...
  if (getLengthFirstTrades() > 0) // Absinthe: fixes a rare crash (from edead)
  {
      ...
      for (int iI = 0; iI < MAX_TEAMS; iI++)
      {
        ...
        if ((eLoopTeam != eFirstTeam) && (eLoopTeam != eSecondTeam))
        {
          if (kLoopTeam.isAlive() && kLoopTeam.isVassal(eSecondTeam))
          {
            ...
            int iSecondSuccess = GET_TEAM(eFirstTeam).AI_getWarSuccess(eSecondTeam) +
                                 GC.getDefineINT("WAR_SUCCESS_CITY_CAPTURING") * GET_TEAM(eSecondTeam).getNumCities();
            GET_TEAM(eFirstTeam)
                .AI_setWarSuccess(eLoopTeam,
      ...
      for (int iI = 0; iI < MAX_TEAMS; iI++)
      {
        ...
        if ((eLoopTeam != eFirstTeam) && (eLoopTeam != eSecondTeam))
        {
          if (kLoopTeam.isAlive() && kLoopTeam.isVassal(eFirstTeam))
          {
            ...
            int iFirstSuccess = GET_TEAM(eSecondTeam).AI_getWarSuccess(eFirstTeam) +
                                GC.getDefineINT("WAR_SUCCESS_CITY_CAPTURING") * GET_TEAM(eFirstTeam).getNumCities();
            GET_TEAM(eSecondTeam)
                .AI_setWarSuccess(eLoopTeam,
...
void CvDeal::doTurn()
{
  ...
  if (!isPeaceDeal())
  {
    ...
    {
      iValue = (GET_PLAYER(getFirstPlayer()).AI_dealVal(getSecondPlayer(), getSecondTrades()) /
    ...
    if (getLengthFirstTrades() > 0)
    {
      iValue = (GET_PLAYER(getSecondPlayer()).AI_dealVal(getFirstPlayer(), getFirstTrades()) /
                GC.getDefineINT("PEACE_TREATY_LENGTH"));
...
bool CvDeal::startTrade(TradeData trade, PlayerTypes eFromPlayer, PlayerTypes eToPlayer)
{
  ...
  case TRADE_VASSAL:
    if (trade.m_iData == 0)
    {
      ...
      GET_TEAM(GET_PLAYER(eFromPlayer).getTeam())
          .setVassal(GET_PLAYER(eToPlayer).getTeam(), true, TRADE_SURRENDER == trade.m_eItemType);
    }
    else
    {
      bSave = true;
  ...
  case TRADE_DEFENSIVE_PACT:
    if (trade.m_iData == 0)
    {
      ...
      GET_TEAM(GET_PLAYER(eFromPlayer).getTeam())
          .setDefensivePact(((TeamTypes)(GET_PLAYER(eToPlayer).getTeam())), true);
    }
    else
    {
      bSave = true;
...
void CvDeal::endTeamTrade(TradeableItems eItem, TeamTypes eFromTeam, TeamTypes eToTeam)
{
       ...
       pLoopDeal = GC.getGameINLINE().nextDeal(&iLoop))
  {
    if (pLoopDeal != this)
    {
      ...
      if (bValid && GET_PLAYER(pLoopDeal->getFirstPlayer()).getTeam() == eToTeam &&
...
{
  return (eItem != TRADE_MAPS && eItem != TRADE_VASSAL && eItem != TRADE_SURRENDER && eItem != TRADE_OPEN_BORDERS &&
          eItem != TRADE_DEFENSIVE_PACT && eItem != TRADE_PERMANENT_ALLIANCE && eItem != TRADE_PEACE_TREATY);
}
