...
void CvDllTranslator::initializeTags(CvWString &szTagStartIcon, CvWString &szTagStartOur, CvWString &szTagStartCT,
                                     CvWString &szTagStartColor, CvWString &szTagStartLink, CvWString &szTagEndLink,
                                     CvWString &szEndLinkReplacement, std::map<std::wstring, CvWString> &aIconMap,
                                     std::map<std::wstring, CvWString> &aColorMap)
{
  ...
  aIconMap[L"[ICON_PROTESTANTISM]"] =
      std::wstring(1, (wchar)GC.getReligionInfo((ReligionTypes)PROTESTANTISM).getChar());
  ...
  for (int i = 0; i < GC.getNumColorInfos(); i++)
  {
    ...
    CvWString colorOut;
    colorOut.Format(L"<color=%i,%i,%i,%i>", (int)(color.r * 255), (int)(color.g * 255), (int)(color.b * 255),
                    (int)(color.a * 255));
