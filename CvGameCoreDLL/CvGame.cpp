...
void CvGame::reset(HandicapTypes eHandicap, bool bConstructorCall)
{
  ...
  if (!bConstructorCall)
  {
    ...
    FAssertMsg(m_paiCorporationGameTurnFounded == NULL,
               "about to leak memory, CvGame::m_paiCorporationGameTurnFounded");
...
void CvGame::initDiplomacy()
{
  ...
  for (iI = 0; iI < MAX_TEAMS; iI++)
  {
    ...
    // 3MiroPapal: The Pope has OB with all indeps
    if (PAPAL_PLAYER > -1)
    {
      for (iJ = INDEP_START; iJ <= INDEP_END; iJ++)
      {
        GET_TEAM((TeamTypes)iJ).meet((TeamTypes)PAPAL_PLAYER, false);
        GET_TEAM((TeamTypes)iJ).signOpenBorders((TeamTypes)PAPAL_PLAYER);
      };
    };
  } // 3Miro end
        ...
        if (!bValid)
        {
          for (iK = 0; iK < MAX_PLAYERS; iK++)
          {
            if (GET_PLAYER((PlayerTypes)iK).isAlive())
            {
              ...
              {
                if (GC.getCivilizationInfo(GET_PLAYER((PlayerTypes)iK).getCivilizationType())
                        .isCivilizationFreeTechs(iI))
                {
                  bValid = true;
                  break;
                }
...
void CvGame::assignStartingPlots()
{
  ...
  if (isTeamGame())
  {
    ...
    for (iJ = 0; iJ < MAX_CIV_PLAYERS; iJ++)
    {
      FAssertMsg(!GET_PLAYER((PlayerTypes)iJ).isAlive() || GET_PLAYER((PlayerTypes)iJ).getStartingPlot() != NULL,
    ...
    for (iI = 0; iI < MAX_CIV_PLAYERS; iI++)
    {
      ...
      if (GET_PLAYER((PlayerTypes)iLoopPlayer).isAlive())
      {
        if (GET_PLAYER((PlayerTypes)iLoopPlayer).isHuman())
        {
          ...
          {
            GET_PLAYER((PlayerTypes)iLoopPlayer)
                .setStartingPlot(GET_PLAYER((PlayerTypes)iLoopPlayer).findStartingPlot(), true);
  ...
  {
    iHumanSlot = range(
        (((countCivPlayersAlive() - 1) * GC.getHandicapInfo(getHandicapType()).getStartingLocationPercent()) / 100), 0,
        (countCivPlayersAlive() - 1));
  ...
  {
    GET_PLAYER((PlayerTypes)(*playerOrderIter))
        .setStartingPlot(GET_PLAYER((PlayerTypes)(*playerOrderIter)).findStartingPlot(), true);
  }
}
          ...
          if (GET_PLAYER((PlayerTypes)iJ).isAlive())
          {
            ...
            if (pPlotJ != NULL)
            {
              ...
              if (iDist == -1)
              {
                ...
                iDist = 5 * plotDistance(pPlotI->getX_INLINE(), pPlotI->getY_INLINE(), pPlotJ->getX_INLINE(),
                                         pPlotJ->getY_INLINE());
              }
                      ...
                      (GC.getFeatureInfo(pLoopPlot->getFeatureType()).getYieldChange(YIELD_PRODUCTION) <= 0))
                  {
                    if (pLoopPlot->isWater())
                    {
                      if (pLoopPlot->isAdjacentToLand() ||
                          (!(iDistance == iMaxRange) && (getSorenRandNum(2, "Remove Bad Feature") == 0)))
                      {
                        pLoopPlot->setFeatureType(NO_FEATURE);
                      }
                    }
                    ...
                    {
                      if (!(iDistance == iMaxRange) &&
                          (getSorenRandNum((2 + (pLoopPlot->getBonusType() == NO_BONUS) ? 0 : 2),
                                           "Remove Bad Feature") == 0))
                      {
                        pLoopPlot->setFeatureType(NO_FEATURE);
                      }
                    }
        ...
        for (iX = -iMaxRange; iX <= iMaxRange; iX++)
        {
          for (iY = -iMaxRange; iY <= iMaxRange; iY++)
          {
            ...
            if (pLoopPlot != NULL)
            {
              ...
              {
                if (!(pLoopPlot->isWater()) &&
                    ((iDistance <= iCityRange) || (pLoopPlot->isCoastalLand()) ||
                     (0 == getSorenRandNum(1 + iDistance - iCityRange, "Map Upgrade Terrain Food"))))
                {
                    ...
                    for (iK = 0; iK < GC.getNumTerrainInfos(); iK++)
                    {
                      if (!(GC.getTerrainInfo((TerrainTypes)iK).isWater()))
                      {
                        if ((GC.getTerrainInfo((TerrainTypes)iK).getYield(YIELD_FOOD) >= iTargetFood) &&
                            (GC.getTerrainInfo((TerrainTypes)iK).getYield(YIELD_FOOD) +
                             GC.getTerrainInfo((TerrainTypes)iK).getYield(YIELD_PRODUCTION)) == iTargetTotal)
                        {
                          if ((pLoopPlot->getFeatureType() == NO_FEATURE) ||
                              GC.getFeatureInfo(pLoopPlot->getFeatureType()).isTerrain(iK))
                          {
        ...
        for (int iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
        {
          ...
          if (pLoopPlot != NULL)
          {
            ...
            if (eBonus != NO_BONUS)
            {
              ...
              {
                if ((GC.getBonusInfo(eBonus).getTechCityTrade() == NO_TECH) ||
                    (GC.getTechInfo((TechTypes)(GC.getBonusInfo(eBonus).getTechCityTrade())).getEra() <= getStartEra()))
                {
                  if (pLoopPlot->isWater())
                  {
                    iFoodBonus += 2;
              ...
              if (pLoopPlot->getBonusType() == NO_BONUS)
              {
                for (int iK = 0; iK < GC.getNumBonusInfos(); iK++)
                {
                  if (GC.getBonusInfo((BonusTypes)iK).isNormalize())
                  {
                    ...
                    {
                      if ((GC.getBonusInfo((BonusTypes)iK).getTechCityTrade() == NO_TECH) ||
                          (GC.getTechInfo((TechTypes)(GC.getBonusInfo((BonusTypes)iK).getTechCityTrade())).getEra() <=
                           getStartEra()))
                      {
                        if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam())
                                .isHasTech((TechTypes)(GC.getBonusInfo((BonusTypes)iK).getTechReveal())))
                        {
      ...
      if (pStartingPlot != NULL)
      {
        ...
        for (iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
        {
          ...
          if (pLoopPlot != NULL)
          {
            if (pLoopPlot != pStartingPlot)
            {
              if ((pLoopPlot->calculateNatureYield(YIELD_FOOD, GET_PLAYER((PlayerTypes)iI).getTeam()) >=
                  ...
                  if (pLoopPlot->getBonusType() == NO_BONUS)
                  {
                        ...
                        GC.getFOOD_CONSUMPTION_PER_POPULATION())
                    {
                      for (iK = 0; iK < GC.getNumTerrainInfos(); iK++)
                      {
                        if (!(GC.getTerrainInfo((TerrainTypes)iK).isWater()))
                        {
                          if (GC.getTerrainInfo((TerrainTypes)iK).getYield(YIELD_FOOD) >=
                              GC.getFOOD_CONSUMPTION_PER_POPULATION())
                          {
                            pLoopPlot->setTerrainType((TerrainTypes)iK);
                            bChanged = true;
...
void CvGame::normalizeAddExtras()
{
  ...
  for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)
  {
    if (GET_PLAYER((PlayerTypes)iI).isAlive())
    {
      ...
      if (pStartingPlot != NULL)
      {
        int iValue = GET_PLAYER((PlayerTypes)iI)
                         .AI_foundValue(pStartingPlot->getX_INLINE(), pStartingPlot->getY_INLINE(), -1, true);
        iTotalValue += iValue;
        iPlayerCount++;
  ...
  for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)
  {
    if (GET_PLAYER((PlayerTypes)iI).isAlive())
    {
      ...
      if (pStartingPlot != NULL)
      {
        ...
        for (int iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
        {
          if (GET_PLAYER((PlayerTypes)iI)
                  .AI_foundValue(pStartingPlot->getX_INLINE(), pStartingPlot->getY_INLINE(), -1, true) >= iTargetValue)
          {
            break;
          }
        ...
        for (int iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
        {
          ...
          if ((pLoopPlot != NULL) && (pLoopPlot != pStartingPlot))
          {
            if (getSorenRandNum(((bLandBias && pLoopPlot->isWater()) ? 2 : 1), "Placing Bonuses") == 0)
            {
              ...
              if ((pLoopPlot != pStartingPlot) && !(bCoast && (iCoastFoodCount > 2)) &&
                  ...
                  if (pLoopPlot->getBonusType() == NO_BONUS)
                  {
                    for (int iK = 0; iK < GC.getNumBonusInfos(); iK++)
                    {
                      if (GC.getBonusInfo((BonusTypes)iK).isNormalize())
                      {
                        ...
                        {
                          if ((GC.getBonusInfo((BonusTypes)iK).getTechCityTrade() == NO_TECH) ||
                              (GC.getTechInfo((TechTypes)(GC.getBonusInfo((BonusTypes)iK).getTechCityTrade()))
                                   .getEra() <= getStartEra()))
                          {
                            if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam())
                                    .isHasTech((TechTypes)(GC.getBonusInfo((BonusTypes)iK).getTechReveal())))
                            {
                              if ((iPass == 0)
                                      ? CvMapGenerator::GetInstance().canPlaceBonusAt(
                                            ((BonusTypes)iK), pLoopPlot->getX(), pLoopPlot->getY(), bIgnoreLatitude)
          ...
          if (pLoopPlot != NULL)
          {
            if (!pLoopPlot->isWater())
            {
              if (!pLoopPlot->isHills())
              {
                ...
                {
                  if ((pLoopPlot->getBonusType() == NO_BONUS) || GC.getBonusInfo(pLoopPlot->getBonusType()).isHills())
                  {
...
void CvGame::normalizeStartingPlots()
{
  ...
  {
    if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(),
                                              "normalizeStartingPlotLocations", NULL) ||
  ...
  if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeAddRiver", NULL) ||
      gDLL->getPythonIFace()->pythonUsingDefaultImpl())
  {
    normalizeAddRiver();
  }

  if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeRemovePeaks",
                                            NULL) ||
      gDLL->getPythonIFace()->pythonUsingDefaultImpl())
  {
  ...
  if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeAddLakes", NULL) ||
      gDLL->getPythonIFace()->pythonUsingDefaultImpl())
  {
    normalizeAddLakes();
  }

  if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeRemoveBadFeatures",
                                            NULL) ||
      gDLL->getPythonIFace()->pythonUsingDefaultImpl())
  {
  ...
  if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeRemoveBadTerrain",
                                            NULL) ||
      gDLL->getPythonIFace()->pythonUsingDefaultImpl())
  {
  ...
  if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeAddFoodBonuses",
                                            NULL) ||
      gDLL->getPythonIFace()->pythonUsingDefaultImpl())
  {
  ...
  if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeAddGoodTerrain",
                                            NULL) ||
      gDLL->getPythonIFace()->pythonUsingDefaultImpl())
  {
  ...
  if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeAddExtras", NULL) ||
      gDLL->getPythonIFace()->pythonUsingDefaultImpl())
  {
    normalizeAddExtras();
  }
...
void CvGame::update()
{
  ...
  if (!gDLL->GetWorldBuilderMode() || isInAdvancedStart())
  {
    ...
    if (NO_PLAYER != getActivePlayer() && GET_PLAYER(getActivePlayer()).getAdvancedStartPoints() >= 0 &&
      ...
      if (pLoopUnit->getOwnerINLINE() == getActivePlayer())
      {
        if (pLoopUnit->canMove())
        {
          if (!isMPOption(MPOPTION_SIMULTANEOUS_TURNS) ||
              getTurnSlice() - pLoopUnit->getLastMoveTurn() > GC.getDefineINT("MIN_TIMER_UNIT_DOUBLE_MOVES"))
          {
...
void CvGame::implementDeal(PlayerTypes eWho, PlayerTypes eOtherWho, CLinkList<TradeData> *pOurList,
                           CLinkList<TradeData> *pTheirList, bool bForce)
{
  CvDeal *pDeal;
...
bool CvGame::isTeamVoteEligible(TeamTypes eTeam, VoteSourceTypes eVoteSource) const
{
  ...
  //Rhye - start
  // 3Miro: no celts and natives
  //if (eTeam == INDEPENDENT || eTeam == INDEPENDENT2 || eTeam == NATIVE || eTeam == CELTIA)
  // 3Miro: indeps do not vote
  //if (eTeam == INDEPENDENT || eTeam == INDEPENDENT2 )
  if (isIndep(eTeam))
  {
    return false;
  }
        ...
        for (iI = 0; iI < NUM_MAJOR_PLAYERS; iI++) //Rhye
        {
          ...
          if (kLoopPlayer.isAlive())
          {
            if (kLoopPlayer.getBuildingClassCount(
                    (BuildingClassTypes)GC.getBuildingInfo((BuildingTypes)iBuilding).getBuildingClassType()) > 0)
            {
    ...
    if (GET_PLAYER((PlayerTypes)iI).isAlive())
    {
           ...
           pLoopCity = GET_PLAYER((PlayerTypes)iI).nextCity(&iLoop))
      {
        if (pLoopCity->isHasReligion(eReligion))
        {
          ...
          iCount +=
              ((pLoopCity->getPopulation() + (pLoopCity->getReligionCount() / 2)) / pLoopCity->getReligionCount());
        }
      }
    }
  }
...
bool CvGame::canTrainNukes() const
{
  for (int iI = 0; iI < MAX_PLAYERS; iI++)
  {
    ...
    if (kPlayer.isAlive())
    {
      ...
      {
        UnitTypes eUnit =
            (UnitTypes)GC.getCivilizationInfo(kPlayer.getCivilizationType()).getCivilizationUnits((UnitClassTypes)iJ);
  ...
  if (isMPOption(MPOPTION_TURN_TIMER))
  {
    if (getElapsedGameTurns() > 0 || !isOption(GAMEOPTION_ADVANCED_START))
    {
      ...
      if (getElapsedGameTurns() == 0 && !isPitboss())
      {
        ...
        FAssertMsg(eTurnTimer >= 0 && eTurnTimer < GC.getNumTurnTimerInfos(),
                   "Invalid TurnTimer selection in InitCore");
...
void CvGame::initScoreCalculation()
{
  ...
  // 3MiroProjects: max projects score
...
void CvGame::updateUnitEnemyGlow()
{
  ...
  for (int i = 0; i < MAX_PLAYERS; i++)
  {
    ...
    int iLoop;
    for (CvUnit *pLoopUnit = GET_PLAYER(playerType).firstUnit(&iLoop); pLoopUnit != NULL;
         pLoopUnit = GET_PLAYER(playerType).nextUnit(&iLoop))
    {
      //update glow
  ...
  if ((getWinner() != eNewWinner) || (getVictory() != eNewVictory))
  {
    ...
    if (getVictory() != NO_VICTORY)
    {
      if (getWinner() != NO_TEAM)
      {
        ...
        szBuffer =
            gDLL->getText("TXT_KEY_GAME_WON", GET_PLAYER((PlayerTypes)getWinner()).getCivilizationShortDescription(),
                          GC.getVictoryInfo(getVictory()).getTextKeyWide());
        //Rhye - end
        addReplayMessage(REPLAY_MESSAGE_MAJOR_EVENT, GET_TEAM(getWinner()).getLeaderID(), szBuffer, -1, -1,
...
bool CvGame::isUnitClassMaxedOut(UnitClassTypes eIndex, int iExtra)
{
  ...
  FAssertMsg(getUnitClassCreatedCount(eIndex) <= GC.getUnitClassInfo(eIndex).getMaxGlobalInstances(),
...
bool CvGame::isBuildingClassMaxedOut(BuildingClassTypes eIndex, int iExtra)
{
  ...
  FAssertMsg(getBuildingClassCreatedCount(eIndex) <= GC.getBuildingClassInfo(eIndex).getMaxGlobalInstances(),
             "Index is expected to be within maximum bounds (invalid Index)");
...
bool CvGame::isProjectMaxedOut(ProjectTypes eIndex, int iExtra)
{
  ...
  FAssertMsg(getProjectCreatedCount(eIndex) <= GC.getProjectInfo(eIndex).getMaxGlobalInstances(),
             "Index is expected to be within maximum bounds (invalid Index)");
  ...
  if (pOldValue != pNewValue)
  {
    ...
    if (getHolyCity(eIndex) != NULL)
    {
      ...
      if (bAnnounce)
      {
        ...
        {
          szBuffer = gDLL->getText("TXT_KEY_MISC_REL_FOUNDED", GC.getReligionInfo(eIndex).getTextKeyWide(),
                                   pHolyCity->getNameKey());
          addReplayMessage(REPLAY_MESSAGE_MAJOR_EVENT, pHolyCity->getOwnerINLINE(), szBuffer, pHolyCity->getX_INLINE(),
                           pHolyCity->getY_INLINE(), (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
            ...
            if (GET_PLAYER((PlayerTypes)iI).isAlive())
            {
              if (pHolyCity->isRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false))
              {
                ...
                gDLL->getInterfaceIFace()->addHumanMessage(
                    ...
                    GC.getReligionInfo(eIndex).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"),
                    pHolyCity->getX_INLINE(), pHolyCity->getY_INLINE());
              }
              else
              {
                szBuffer =
                    gDLL->getText("TXT_KEY_MISC_REL_FOUNDED_UNKNOWN", GC.getReligionInfo(eIndex).getTextKeyWide());
                gDLL->getInterfaceIFace()->addHumanMessage(
                    ((PlayerTypes)iI), false, GC.getDefineINT("EVENT_MESSAGE_TIME_LONG"), szBuffer,
                    GC.getReligionInfo(eIndex).getSound(), MESSAGE_TYPE_MAJOR_EVENT,
  ...
  if (pOldValue != pNewValue)
  {
    ...
    if (NULL != pHeadquarters)
    {
      ...
      if (bAnnounce)
      {
        ...
        {
          CvWString szBuffer =
              gDLL->getText("TXT_KEY_MISC_CORPORATION_FOUNDED", GC.getCorporationInfo(eIndex).getTextKeyWide(),
                            pHeadquarters->getNameKey());
          addReplayMessage(REPLAY_MESSAGE_MAJOR_EVENT, pHeadquarters->getOwnerINLINE(), szBuffer,
                           pHeadquarters->getX_INLINE(), pHeadquarters->getY_INLINE(),
                           (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
            ...
            if (GET_PLAYER((PlayerTypes)iI).isAlive())
            {
              if (pHeadquarters->isRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false))
              {
                gDLL->getInterfaceIFace()->addHumanMessage(
                    ...
                    (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"), pHeadquarters->getX_INLINE(),
                    pHeadquarters->getY_INLINE());
              }
              else
              {
                CvWString szBuffer2 = gDLL->getText("TXT_KEY_MISC_CORPORATION_FOUNDED_UNKNOWN",
                                                    GC.getCorporationInfo(eIndex).getTextKeyWide());
                gDLL->getInterfaceIFace()->addHumanMessage(
                    ((PlayerTypes)iI), false, GC.getDefineINT("EVENT_MESSAGE_TIME_LONG"), szBuffer2,
                    GC.getCorporationInfo(eIndex).getSound(), MESSAGE_TYPE_MAJOR_EVENT,
                    GC.getCorporationInfo(eIndex).getButton(),
                    (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
              }
    ...
    else if (isTeamVote(pTriggeredData->kVoteOption.eVote))
    {
      if ((PlayerVoteTypes)GET_PLAYER(eOwnerIndex).getTeam() != ePlayerVote)
      {
        for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; ++iPlayer)
        {
          ...
          if (kLoopPlayer.isAlive())
          {
            if (kLoopPlayer.getTeam() != GET_PLAYER(eOwnerIndex).getTeam() &&
                kLoopPlayer.getTeam() == (TeamTypes)ePlayerVote)
            {
...
void CvGame::doGlobalWarming()
{
  ...
  for (int i = 0; i < GC.getMapINLINE().numPlotsINLINE(); ++i)
  {
    ...
    if (!pPlot->isWater())
    {
      ...
      {
        if (GC.getFeatureInfo(pPlot->getFeatureType()).getGrowthProbability() >
            0) // hack, but we don't want to add new XML field in the patch just for this
        {
  ...
  iGlobalWarmingDefense =
      iGlobalWarmingDefense * GC.getDefineINT("GLOBAL_WARMING_FOREST") / std::max(1, GC.getMapINLINE().getLandPlots());
          ...
          if (pCity != NULL)
          {
            if (pPlot->isVisible(pCity->getTeam(), false))
            {
              ...
              gDLL->getInterfaceIFace()->addHumanMessage(pCity->getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(),
                                                         szBuffer, bSoundPlayed ? NULL : "AS2D_GLOBALWARMING",
        ...
        if (eBestPlayer != NO_PLAYER)
        {
          ...
          if (NO_RELIGION != eReligion)
          {
            ...
            if (eReligion == iSchismReligion)
            {
              ...
              if ((GET_PLAYER(eBestPlayer).getStateReligion() == iParentReligion) ||
                  (GET_PLAYER(eBestPlayer).getStateReligion() == iSchismReligion))
              {
            ...
            if (kLoopTeam.getNumCities() > 0)
            {
              ...
              for (int i = 0; i < GC.getNUM_CORPORATION_PREREQ_BONUSES(); ++i)
              {
                if (NO_BONUS != kCorporation.getPrereqBonus(i) &&
                    kLoopTeam.hasBonus((BonusTypes)kCorporation.getPrereqBonus(i)))
                {
                  bHasBonus = true;
                  break;
                }
          ...
          if (kLoopPlayer.isAlive())
          {
            if (kLoopPlayer.getTeam() == eBestTeam)
            {
              if (kLoopPlayer.getNumCities() > 0)
              {
                ...
                {
                  if (NO_BONUS != kCorporation.getPrereqBonus(i) &&
                      kLoopPlayer.hasBonus((BonusTypes)kCorporation.getPrereqBonus(i)))
                  {
                    bHasBonus = true;
                    break;
                  }
...
void CvGame::createBarbarianCities()
{
  ...
  if (getElapsedGameTurns() < (((GC.getHandicapInfo(getHandicapType()).getBarbarianCityCreationTurnsElapsed() *
                                 GC.getGameSpeedInfo(getGameSpeedType()).getBarbPercent()) /
                                100) /
  ...
  if (getSorenRandNum(100, "Barb City Creation") >=
      GC.getHandicapInfo(getHandicapType()).getBarbarianCityCreationProb())
  {
    return;
  }
  ...
  {
    int iTargetBarbCities =
        (getNumCivCities() * 5 * GC.getHandicapInfo(getHandicapType()).getBarbarianCityCreationProb()) / 100;
  ...
  for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
  {
    ...
    if (!(pLoopPlot->isWater()))
    {
      if (!(pLoopPlot->isVisibleToCivTeam()))
      {
        ...
        {
          iValue = GET_PLAYER(BARBARIAN_PLAYER)
                       .AI_foundValue(pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE(),
...
void CvGame::createBarbarianUnits()
{
  ...
  else
  {
         ...
         pLoopArea = GC.getMapINLINE().nextArea(&iLoop))
    {
      ...
      {
        iNeededBarbs =
            ((pLoopArea->getNumUnownedTiles() / iDivisor) -
             pLoopArea->getUnitsPerPlayer(
                 BARBARIAN_PLAYER)); // XXX eventually need to measure how many barbs of eBarbUnitAI we have in this area...
        ...
        if (iNeededBarbs > 0)
        {
          ...
          for (iI = 0; iI < iNeededBarbs; iI++)
          {
            pPlot = GC.getMapINLINE().syncRandPlot(
                (RANDPLOT_NOT_VISIBLE_TO_CIV | RANDPLOT_ADJACENT_LAND | RANDPLOT_PASSIBLE), pLoopArea->getID(),
                GC.getDefineINT("MIN_BARBARIAN_STARTING_DISTANCE"));
            ...
            if (pPlot != NULL)
            {
              ...
              if ((pPlot->getTerrainType() <= 3) && (pPlot->getFeatureType() >= 2) && (pPlot->getFeatureType() <= 4))
              {
                ...
                {
                  bool bValid = false;
                  eLoopUnit = ((UnitTypes)(GC.getCivilizationInfo(GET_PLAYER(BARBARIAN_PLAYER).getCivilizationType())
                                               .getCivilizationUnits(iJ)));
                  ...
                  if (eLoopUnit != NO_UNIT)
                  {
                    ...
                    if (bValid)
                    {
                      ...
                      {
                        if (!GET_TEAM(BARBARIAN_TEAM)
                                 .isHasTech((TechTypes)GC.getBonusInfo((BonusTypes)kUnit.getPrereqAndBonus())
                                                .getTechCityTrade()))
                        {
                          bValid = false;
                        }
                    ...
                    if (bValid)
                    {
                      ...
                      for (int i = 0; i < GC.getNUM_UNIT_PREREQ_OR_BONUSES(); ++i)
                      {
                        ...
                        {
                          TechTypes eTech =
                              (TechTypes)GC.getBonusInfo((BonusTypes)kUnit.getPrereqOrBonuses(i)).getTechCityTrade();
                          if (NO_TECH != eTech)
                          {
                            bRequires = true;
                ...
                if (eBestUnit != NO_UNIT)
                {
                  GET_PLAYER(BARBARIAN_PLAYER)
                      .initUnit(eBestUnit, pPlot->getX_INLINE(), pPlot->getY_INLINE(), eBarbUnitAI);
                }
                //Rhye - start
              }
              //Rhye - end
    ...
    for (pLoopUnit = GET_PLAYER(BARBARIAN_PLAYER).firstUnit(&iLoop); pLoopUnit != NULL;
         pLoopUnit = GET_PLAYER(BARBARIAN_PLAYER).nextUnit(&iLoop))
    {
...
void CvGame::createAnimals()
{
       ...
       pLoopArea = GC.getMapINLINE().nextArea(&iLoop))
  {
    ...
    {
      iNeededAnimals =
          ((pLoopArea->getNumUnownedTiles() / GC.getHandicapInfo(getHandicapType()).getUnownedTilesPerGameAnimal()) -
           pLoopArea->getUnitsPerPlayer(BARBARIAN_PLAYER));
      ...
      if (iNeededAnimals > 0)
      {
        ...
        {
          pPlot = GC.getMapINLINE().syncRandPlot((RANDPLOT_NOT_VISIBLE_TO_CIV | RANDPLOT_PASSIBLE), pLoopArea->getID(),
                                                 GC.getDefineINT("MIN_ANIMAL_STARTING_DISTANCE"));
          ...
          if (pPlot != NULL)
          {
            ...
            for (iJ = 0; iJ < GC.getNumUnitClassInfos(); iJ++)
            {
              ...
              if (eLoopUnit != NO_UNIT)
              {
                ...
                {
                  if ((pPlot->getFeatureType() != NO_FEATURE)
                          ? GC.getUnitInfo(eLoopUnit).getFeatureNative(pPlot->getFeatureType())
                          : GC.getUnitInfo(eLoopUnit).getTerrainNative(pPlot->getTerrainType()))
                  {
            ...
            {
              GET_PLAYER(BARBARIAN_PLAYER)
                  .initUnit(eBestUnit, pPlot->getX_INLINE(), pPlot->getY_INLINE(), UNITAI_ANIMAL);
            }
          }
        }
      }
  ...
  for (iI = 0; iI < MAX_PLAYERS; iI++)
  {
    ...
    if (player.isAlive())
    {
      if (player.isTurnActive())
      {
        ...
        if (player.isAutoMoves())
        {
          for (pLoopSelectionGroup = player.firstSelectionGroup(&iLoop); pLoopSelectionGroup;
               pLoopSelectionGroup = player.nextSelectionGroup(&iLoop))
          {
...
bool CvGame::testVictory(VictoryTypes eVictory, TeamTypes eTeam, bool *pbEndScore) const
{
  ...
  if (bValid)
  {
    if ((GC.getVictoryInfo(eVictory).getCityCulture() != NO_CULTURELEVEL) &&
        (GC.getVictoryInfo(eVictory).getNumCultureCities() > 0))
    {
      int iCount = 0;
      ...
      for (int iK = 0; iK < MAX_CIV_PLAYERS; iK++)
      {
        if (GET_PLAYER((PlayerTypes)iK).isAlive())
        {
          ...
          {
            int iLoop;
            for (CvCity *pLoopCity = GET_PLAYER((PlayerTypes)iK).firstCity(&iLoop); pLoopCity != NULL;
                 pLoopCity = GET_PLAYER((PlayerTypes)iK).nextCity(&iLoop))
            {
  ...
  if (bValid)
  {
    ...
    {
      int iThreshold =
          ((GET_TEAM(eTeam).countTotalCulture() * 100) / GC.getVictoryInfo(eVictory).getTotalCultureRatio());
  ...
  if (bValid)
  {
    ...
    {
      if (GC.getProjectInfo((ProjectTypes)iK).getVictoryMinThreshold(eVictory) >
          GET_TEAM(eTeam).getProjectCount((ProjectTypes)iK))
      {
        bValid = false;
        break;
      }
...
bool CvGame::changePlayer(int playerIdx, int newCivType, int newLeader, int teamIdx, bool bIsHuman,
                          bool bChangeGraphics)
{
  ...
  if ((LeaderHeadTypes)newLeader != GC.getInitCore().getLeader((PlayerTypes)playerIdx))
  {
    ...
    GC.getInitCore().setLeaderName((PlayerTypes)playerIdx,
                                   GC.getLeaderHeadInfo((LeaderHeadTypes)newLeader).getDescription(0)); //Rhye
  ...
  if (bChangeGraphics)
  {
    ...
    GC.getInitCore().setColor((PlayerTypes)playerIdx,
                              (PlayerColorTypes)GC.getCivilizationInfo(civType).getDefaultPlayerColor());
    GC.getInitCore().setArtStyle((PlayerTypes)playerIdx,
                                 (ArtStyleTypes)GC.getCivilizationInfo(civType).getArtStyleType());
...
*/
    ...
    for (pLoopUnit = GET_PLAYER((PlayerTypes)playerIdx).firstUnit(&iLoop); pLoopUnit != NULL;
         pLoopUnit = GET_PLAYER((PlayerTypes)playerIdx).nextUnit(&iLoop))
    {
      // Concept borrowed from CvUnit::gift
      pNewUnit = GET_PLAYER((PlayerTypes)playerIdx)
                     .initUnit(pLoopUnit->getUnitType(), pLoopUnit->getX_INLINE(), pLoopUnit->getY_INLINE(),
...
void CvGame::addReplayMessage(ReplayMessageTypes eType, PlayerTypes ePlayer, CvWString pszText, int iPlotX, int iPlotY,
                              ColorTypes eColor)
{
...
{
  for (ReplayMessageList::const_iterator itList = m_listReplayMessages.begin(); itList != m_listReplayMessages.end();
       itList++)
  {
...
void CvGame::write(FDataStreamBase *pStream)
{
  ...
  for (std::vector<EventTriggerTypes>::iterator it = m_aeInactiveTriggers.begin(); it != m_aeInactiveTriggers.end();
       ++it)
  {
    pStream->Write(*it);
  }
...
void CvGame::showEndGameSequence()
{
  ...
  for (iI = 0; iI < MAX_CIV_PLAYERS; iI++)
  {
    ...
    if (player.isHuman())
    {
      addReplayMessage(REPLAY_MESSAGE_MAJOR_EVENT, (PlayerTypes)iI,
      ...
      if (NULL != pInfo)
      {
        ...
        {
          pInfo->setText(gDLL->getText("TXT_KEY_GAME_WON", GET_TEAM(getWinner()).getName().GetCString(),
                                       GC.getVictoryInfo(getVictory()).getTextKeyWide()));
        }
        else
        {
...
*/
    ...
    /************************************************************************************************/
    {
      ...
      {
        if (iK != GC.getCivilizationInfo((CivilizationTypes)GC.getDefineINT("BARBARIAN_CIVILIZATION"))
...
{
  for (std::vector<PlotExtraYield>::const_iterator it = m_aPlotExtraYields.begin(); it != m_aPlotExtraYields.end();
       ++it)
  {
...
void CvGame::setVoteSourceReligion(VoteSourceTypes eVoteSource, ReligionTypes eReligion, bool bAnnounce)
{
  ...
  if (bAnnounce)
  {
    if (NO_RELIGION != eReligion)
    {
      CvWString szBuffer = gDLL->getText("TXT_KEY_VOTE_SOURCE_RELIGION", GC.getReligionInfo(eReligion).getTextKeyWide(),
      ...
      for (int iI = 0; iI < MAX_PLAYERS; iI++)
      {
        ...
        {
          gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer,
                                                     GC.getReligionInfo(eReligion).getSound(), MESSAGE_TYPE_MAJOR_EVENT,
...
void CvGame::changeShrineBuilding(BuildingTypes eBuilding, ReligionTypes eReligion, bool bRemove)
{
  ...
  FAssertMsg(bRemove || m_iShrineBuildingCount < GC.getNumBuildingInfos(),
             "trying to add too many buildings to CvGame::changeShrineBuilding");
      ...
      if (GC.getVoteInfo((VoteTypes)iI).isVoteSourceType(eVoteSource))
      {
        if (isChooseElection((VoteTypes)iI))
        {
          ...
          if (GC.getVoteInfo(kData.eVote).isOpenBorders())
          {
            ...
            {
              kData.szText =
                  gDLL->getText("TXT_KEY_POPUP_ELECTION_OPEN_BORDERS", getVoteRequired(kData.eVote, eVoteSource),
          ...
          else if (GC.getVoteInfo(kData.eVote).isDefensivePact())
          {
            ...
            {
              kData.szText =
                  gDLL->getText("TXT_KEY_POPUP_ELECTION_DEFENSIVE_PACT", getVoteRequired(kData.eVote, eVoteSource),
                                countPossibleVote(kData.eVote, eVoteSource));
              pData->aVoteOptions.push_back(kData);
            }
          }
              ...
              if (kTeam1.isAlive())
              {
                ...
                if (isValidVoteSelection(eVoteSource, kData))
                {
                  ...
                  kData.szText = gDLL->getText("TXT_KEY_POPUP_ELECTION_FORCE_PEACE",
                                               GET_PLAYER((PlayerTypes)iTeam1).getCivilizationDescriptionKey(),
                                               getVoteRequired(kData.eVote, eVoteSource),
                                               countPossibleVote(kData.eVote, eVoteSource)); //Rhye
                  pData->aVoteOptions.push_back(kData);
                }
              }
            }
          ...
          else if (GC.getVoteInfo(kData.eVote).isAssignCity())
          {
            for (int iPlayer1 = 0; iPlayer1 < MAX_CIV_PLAYERS; ++iPlayer1)
            {
              ...
              int iLoop;
              for (CvCity *pLoopCity = kPlayer1.firstCity(&iLoop); NULL != pLoopCity;
                   pLoopCity = kPlayer1.nextCity(&iLoop))
              {
                ...
                if (NO_PLAYER != eNewOwner)
                {
                  ...
                  if (isValidVoteSelection(eVoteSource, kData))
                  {
                    ...
                    kData.szText = gDLL->getText(
                        "TXT_KEY_POPUP_ELECTION_ASSIGN_CITY", kPlayer1.getCivilizationAdjectiveKey(),
                        pLoopCity->getNameKey(), GET_PLAYER(eNewOwner).getCivilizationDescriptionKey(),
                        getVoteRequired(kData.eVote, eVoteSource), countPossibleVote(kData.eVote, eVoteSource)); //Rhye
                    pData->aVoteOptions.push_back(kData);
                  }
                }
              }
          ...
          {
            kData.szText =
                gDLL->getText("TXT_KEY_POPUP_ELECTION_OPTION", GC.getVoteInfo(kData.eVote).getTextKeyWide(),
                              getVoteRequired(kData.eVote, eVoteSource), countPossibleVote(kData.eVote, eVoteSource));
      ...
      if (kPlayer.isVotingMember(eVoteSource))
      {
        ...
        else
        {
          castVote(((PlayerTypes)iI), pData->getID(),
                   GET_PLAYER((PlayerTypes)iI).AI_diploVote(kOptionData, eVoteSource, false));
        }
      }
    }
  }
...
{
  int iLoop;
  for (VoteTriggeredData *pVoteTriggered = m_votesTriggered.beginIter(&iLoop); NULL != pVoteTriggered;
       pVoteTriggered = m_votesTriggered.nextIter(&iLoop))
  {
    CvWString szBuffer;
    CvWString szMessage;
      ...
      for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; ++iPlayer)
      {
        ...
        if (kPlayer.isVotingMember(eVoteSource))
        {
          ...
          szMessage.Format(L"%s: %s", gDLL->getText("TXT_KEY_ELECTION_CANCELLED").GetCString(),
                           GC.getVoteInfo(eVote).getDescription());
          gDLL->getInterfaceIFace()->addHumanMessage((PlayerTypes)iPlayer, false, GC.getEVENT_MESSAGE_TIME(), szMessage,
    ...
    else
    {
      ...
      if (isTeamVote(eVote))
      {
        ...
        {
          szBuffer +=
              NEWLINE + gDLL->getText("TXT_KEY_POPUP_DIPLOMATIC_VOTING_VICTORY", GET_TEAM(eTeam).getName().GetCString(),
                                      countVote(*pVoteTriggered, (PlayerVoteTypes)eTeam),
                                      getVoteRequired(eVote, eVoteSource), countPossibleVote(eVote, eVoteSource));
        }
        ...
        for (int iI = MAX_CIV_TEAMS; iI >= 0; --iI)
        {
          for (int iJ = 0; iJ < MAX_CIV_PLAYERS; iJ++)
          {
            if (GET_PLAYER((PlayerTypes)iJ).isVotingMember(eVoteSource))
            {
              ...
              {
                szBuffer += NEWLINE + gDLL->getText("TXT_KEY_POPUP_VOTES_FOR", GET_PLAYER((PlayerTypes)iJ).getNameKey(),
        ...
        for (int iJ = 0; iJ < MAX_CIV_PLAYERS; iJ++)
        {
          if (GET_PLAYER((PlayerTypes)iJ).isVotingMember(eVoteSource))
          {
            ...
            {
              szBuffer += NEWLINE + gDLL->getText("TXT_KEY_POPUP_ABSTAINS", GET_PLAYER((PlayerTypes)iJ).getNameKey(),
                                                  GET_PLAYER((PlayerTypes)iJ).getVotes(eVote, eVoteSource));
            }
          }
        }
      ...
      else
      {
        ...
        szBuffer +=
            NEWLINE + gDLL->getText((bPassed ? "TXT_KEY_POPUP_DIPLOMATIC_VOTING_SUCCEEDS"
                                             : "TXT_KEY_POPUP_DIPLOMATIC_VOTING_FAILURE"),
                                    GC.getVoteInfo(eVote).getTextKeyWide(), countVote(*pVoteTriggered, PLAYER_VOTE_YES),
                                    getVoteRequired(eVote, eVoteSource), countPossibleVote(eVote, eVoteSource));
            ...
            if (GET_PLAYER((PlayerTypes)iJ).isVotingMember(eVoteSource))
            {
              if (getPlayerVote(((PlayerTypes)iJ), pVoteTriggered->getID()) == (PlayerVoteTypes)iI)
              {
                ...
                case PLAYER_VOTE_ABSTAIN:
                  szBuffer +=
                      NEWLINE + gDLL->getText("TXT_KEY_POPUP_ABSTAINS", GET_PLAYER((PlayerTypes)iJ).getNameKey(),
                                              GET_PLAYER((PlayerTypes)iJ).getVotes(eVote, eVoteSource));
                  break;
                case PLAYER_VOTE_NEVER:
                  szBuffer +=
                      NEWLINE + gDLL->getText("TXT_KEY_POPUP_VOTES_YES_NO", GET_PLAYER((PlayerTypes)iJ).getNameKey(),
                                              L"TXT_KEY_POPUP_VOTE_NEVER",
                                              GET_PLAYER((PlayerTypes)iJ).getVotes(eVote, eVoteSource));
                  break;
                case PLAYER_VOTE_NO:
                  szBuffer += NEWLINE + gDLL->getText("TXT_KEY_POPUP_VOTES_YES_NO",
                                                      GET_PLAYER((PlayerTypes)iJ).getNameKey(), L"TXT_KEY_POPUP_NO",
                                                      GET_PLAYER((PlayerTypes)iJ).getVotes(eVote, eVoteSource));
                  break;
                case PLAYER_VOTE_YES:
                  szBuffer += NEWLINE + gDLL->getText("TXT_KEY_POPUP_VOTES_YES_NO",
                                                      GET_PLAYER((PlayerTypes)iJ).getNameKey(), L"TXT_KEY_POPUP_YES",
                                                      GET_PLAYER((PlayerTypes)iJ).getVotes(eVote, eVoteSource));
                  break;
                default:
                  FAssert(false);
      ...
      for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)
      {
        ...
        if (kPlayer.isHuman())
        {
          ...
          if (!bShow)
          {
            if (iI == pVoteTriggered->kVoteOption.ePlayer &&
                GET_PLAYER(pVoteTriggered->kVoteOption.ePlayer).isVotingMember(pVoteTriggered->eVoteSource))
            {
              bShow = true;
            }
          }
          ...
          if (!bShow)
          {
            if (iI == pVoteTriggered->kVoteOption.eOtherPlayer &&
                GET_PLAYER(pVoteTriggered->kVoteOption.eOtherPlayer).isVotingMember(pVoteTriggered->eVoteSource))
            {
              bShow = true;
            }
          }
          ...
          {
            CvWString szMessage =
                gDLL->getText("TXT_KEY_VOTE_RESULTS", GC.getVoteSourceInfo(eVoteSource).getTextKeyWide(),
                              pVoteTriggered->kVoteOption.szText.GetCString());
            gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szMessage,
                                                       "AS2D_NEW_ERA", MESSAGE_TYPE_MINOR_EVENT, NULL,
                                                       (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
          }
        }
      }
...
void CvGame::doVoteSelection()
{
  for (int iI = 0; iI < GC.getNumVoteSourceInfos(); ++iI)
  {
    ...
    if (isDiploVote(eVoteSource))
    {
      ...
      {
        setVoteTimer(eVoteSource, (GC.getVoteSourceInfo(eVoteSource).getVoteInterval() *
        ...
        if (canHaveSecretaryGeneral(eVoteSource))
        {
          ...
          else
          {
            ...
            {
              if (GC.getVoteInfo((VoteTypes)iJ).isSecretaryGeneral() &&
                  GC.getVoteInfo((VoteTypes)iJ).isVoteSourceType(iI))
              {
                VoteSelectionSubData kOptionData;
                kOptionData.iCityId = -1;
                kOptionData.ePlayer = NO_PLAYER;
                kOptionData.eVote = (VoteTypes)iJ;
                kOptionData.szText = gDLL->getText(
                    "TXT_KEY_POPUP_ELECTION_OPTION", GC.getVoteInfo((VoteTypes)iJ).getTextKeyWide(),
                    getVoteRequired((VoteTypes)iJ, eVoteSource), countPossibleVote((VoteTypes)iJ, eVoteSource));
...
{
  for (std::vector<EventTriggerTypes>::const_iterator it = m_aeInactiveTriggers.begin();
  ...
  for (int iTrigger = 0; iTrigger < GC.getNumEventTriggerInfos(); ++iTrigger)
  {
    if (isOption(GAMEOPTION_NO_EVENTS) ||
        getSorenRandNum(100, "Event Active?") >=
            GC.getEventTriggerInfo((EventTriggerTypes)iTrigger).getPercentGamesActive())
    {
      m_aeInactiveTriggers.push_back((EventTriggerTypes)iTrigger);
    }
  }
...
bool CvGame::isUnitEverActive(UnitTypes eUnit) const
{
  for (int iCiv = 0; iCiv < GC.getNumCivilizationInfos(); ++iCiv)
  {
    ...
    {
      if (eUnit == GC.getCivilizationInfo((CivilizationTypes)iCiv)
                       .getCivilizationUnits(GC.getUnitInfo(eUnit).getUnitClassType()))
      {
        return true;
      }
    }
...
bool CvGame::isBuildingEverActive(BuildingTypes eBuilding) const
{
  for (int iCiv = 0; iCiv < GC.getNumCivilizationInfos(); ++iCiv)
  {
    if (isCivEverActive((CivilizationTypes)iCiv))
    {
      if (eBuilding == GC.getCivilizationInfo((CivilizationTypes)iCiv)
                           .getCivilizationBuildings(GC.getBuildingInfo(eBuilding).getBuildingClassType()))
      {
        return true;
      }
    }
...
{
  long lResult = -1;
  if (gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "isBonusIgnoreLatitude", NULL,
                                           &lResult))
  {
...
int CvGame::getLargestOtherCity(int x, int y)
{
  ...
  {
    for (pCity = GET_PLAYER((PlayerTypes)iPlayer).firstCity(&iLoop); pCity != NULL;
         pCity = GET_PLAYER((PlayerTypes)iPlayer).nextCity(&iLoop))
    {
...
bool CvGame::isTopCultureCity(int x, int y)
{
  if (GC.getMapINLINE().plot(x, y)->isCity())
  {
    ...
    int iCulture = GC.getMapINLINE().plot(x, y)->getPlotCity()->getCulture(
        GC.getMapINLINE().plot(x, y)->getPlotCity()->getOwner());
    int iPlayer, iLoop;
    CvCity *pCity;
    for (iPlayer = 0; iPlayer < NUM_ALL_PLAYERS; iPlayer++)
    {
...
int CvGame::doesOwnCities(int iCiv, int BLx, int BLy, int TRx, int TRy)
{
  bool hasCity = false;
  bool ownsCity = true;
  CvPlot *plot;
...
int CvGame::doesOwnOrVassalCities(int iCiv, int BLx, int BLy, int TRx, int TRy)
{
  bool hasCity = false;
  bool ownsCity = true;
  int iOwner;
  int iCivTeam = GET_PLAYER((PlayerTypes)iCiv).getTeam();
  CvPlot *plot;
  for (int x = BLx; x <= TRx; x++)
  {
    for (int y = BLy; y <= TRy; y++)
    {
      ...
      {
        hasCity = true;
        iOwner = plot->getPlotCity()->getOwner();
        if ((iOwner != iCiv) &&
            (!(GET_TEAM((TeamTypes)GET_PLAYER((PlayerTypes)iOwner).getTeam()).isVassal((TeamTypes)iCivTeam))))
          ownsCity = false;
      };
    };
  };
...
bool CvGame::doesHaveOtherReligion(int BLx, int BLy, int TRx, int TRy, int AllowR)
{
  CvPlot *plot;
  int x, y, i;
  for (x = BLx; x <= TRx; x++)
  {
    for (y = BLy; y <= TRy; y++)
    {
      ...
      if (plot->isCity())
      {
        ...
        {
          if ((i != AllowR) && (plot->getPlotCity()->isHasReligion((ReligionTypes)i)))
          {
            return true;
          };
        };
...
int CvGame::countOwnedCities(int iCiv, int BLx, int BLy, int TRx, int TRy)
{
  CvPlot *plot;
  int x, y, count = 0;
  for (x = BLx; x <= TRx; x++)
  {
    ...
    {
      plot = GC.getMapINLINE().plot(x, y);
      if ((plot->isCity()) && (plot->getPlotCity()->getOwner() == iCiv))
      {
        count++;
      };
    };
...
bool CvGame::canSeeAllTerrain(int iCiv, int iTerrain)
{
  CvPlot *pPlot;
  int iTeam = GET_PLAYER((PlayerTypes)iCiv).getTeam();
  for (int x = 0; x < EARTH_X; x++)
  {
    for (int y = 0; y < EARTH_Y; y++)
    {
      pPlot = GC.getMapINLINE().plot(x, y);
      if ((pPlot->getTerrainType() == iTerrain) && !pPlot->isRevealed((TeamTypes)iTeam, false))
      {
        return false;
      };
    };
...
bool CvGame::controlMostTeritory(int iCiv, int BLx, int BLy, int TRx, int TRy)
{
  int OwnedPlots[MAX_PLAYERS];
  int i, iOwner, iMax;
  CvPlot *pPlot;
  ...
  for (int x = BLx; x < TRx; x++)
  {
    ...
    {
      pPlot = GC.getMapINLINE().plot(x, y);
      if (pPlot->isHills() || pPlot->isFlatlands())
      {
        iOwner = pPlot->getOwner();
        if ((iOwner > -1) && (iOwner < MAX_PLAYERS))
        {
          OwnedPlots[iOwner]++;
        };
      };
  ...
  iMax = OwnedPlots[iCiv];
  OwnedPlots[iCiv] = 0;
  for (i = 0; i < NUM_MAJOR_PLAYERS; i++)
  {
    if (OwnedPlots[i] >= iMax)
      return false;
  };
  return true;
};

int CvGame::countCitiesLostTo(int iCiv, int iNewOwner)
{
  int iLostCities = 0;
  CvPlot *pPlot;
  CvCity *iCity;
  for (int x = 0; x < EARTH_X; x++)
  {
    for (int y = 0; y < EARTH_Y; y++)
    {
      ...
      if (pPlot->isCity())
      {
        iCity = pPlot->getPlotCity();
        if ((iCity->getOwner() == iNewOwner) && (iCity->getOriginalOwner() == iCiv))
          iLostCities++;
      };
    };
  };
