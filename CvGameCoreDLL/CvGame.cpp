...
//Speed: End Modify
		...
		GET_PLAYER(getActivePlayer()).initUnit(((UnitTypes)6), 5, 72); //Rhye (catapult) // 3Miro catapult, change to warrior //Sedna, not warrior anymore
...
void CvGame::setAIAutoPlayCatapult(int iNewValue)
{
	...
	if (iOldValue != iNewValue)
	{
		...
		if ((iOldValue == 0) && (getAIAutoPlay() > 0))
		{
			CvPlot* pPlot = GC.getMapINLINE().plotINLINE( 5, 72); // 3Miro: the plot
			if (pPlot->isUnit()) {
				GC.getMapINLINE().plotINLINE( 5, 72)->getUnitByIndex(0)->kill(false); // 3Miro catapult coordinates
				for (int iI = 0; iI < MAX_PLAYERS; iI++)
				{
					if (GET_PLAYER((PlayerTypes)iI).isHuman())
					{
...
bool CvGame::isTopCultureCity( int x, int y ){
	if ( GC.getMapINLINE().plot(x,y) -> isCity() ){
		...
		for ( iPlayer = 0; iPlayer < NUM_ALL_PLAYERS; iPlayer++ ){
			for ( pCity = GET_PLAYER((PlayerTypes)iPlayer).firstCity(&iLoop); pCity != NULL; pCity = GET_PLAYER((PlayerTypes)iPlayer).nextCity(&iLoop) ){
				if ( pCity ->countTotalCultureTimes100() > iCulture ){
					return false;
				};
			};
		};
...
int CvGame::doesOwnCities( int iCiv, int BLx, int BLy, int TRx, int TRy ){
	...
	for ( int x = BLx; x<= TRx; x++ ){
		for ( int y = BLy; y<= TRy; y++ ){
			...
			if ( plot ->isCity() ){
				hasCity = true;
				if ( plot ->getPlotCity() ->getOwner() != iCiv ) ownsCity = false;
			};
		};
	};
	if ( ownsCity ){
		if ( hasCity ){ return 11; }else{ return 10; };
	}else{
		return 0;
	};
};
	...
	for ( int x = BLx; x<= TRx; x++ ){
		for ( int y = BLy; y<= TRy; y++ ){
			...
			if ( plot ->isCity() ){
				...
				if ( (iOwner != iCiv)&&( ! (GET_TEAM((TeamTypes) GET_PLAYER((PlayerTypes)iOwner).getTeam() ).isVassal((TeamTypes)iCivTeam) )) ) ownsCity = false;
			};
		};
	};
	if ( ownsCity ){
...
bool CvGame::controlMostTeritory( int iCiv, int BLx, int BLy, int TRx, int TRy ){
	...
	int i, iOwner, iMax;
	CvPlot *pPlot;
	for( i=0; i<NUM_MAJOR_PLAYERS; i++ ) OwnedPlots[i] = 0;
	for( int x=BLx; x<TRx; x++ ){
		for( int y=BLy; y<TRy; y++ ){
			pPlot = GC.getMapINLINE().plot(x,y);
	...
	for( i=0; i<NUM_MAJOR_PLAYERS; i++ ){
		if ( OwnedPlots[i] >= iMax ) return false;
	};
	return true;
};
	...
	for( int x=0; x<EARTH_X; x++ ){
		for( int y=0; y<EARTH_Y; y++ ){
			...
			if ( pPlot ->isCity() ){
				...
				if ( (iCity ->getOwner() == iNewOwner) && (iCity ->getOriginalOwner() == iCiv) ) iLostCities++;
			};
		};
	};
	return iLostCities;
