...
// BUFFY - Security Checks - end
  ...
  DllExport void cycleSelectionGroups(bool bClear, bool bForward = true,
                                      bool bWorkers = false) const; // Exposed to Python
  DllExport bool cyclePlotUnits(CvPlot *pPlot, bool bForward = true, bool bAuto = false,
                                int iCount = -1) const; // Exposed to Python
  ...
  DllExport void selectionListGameNetMessage(int eMessage, int iData2 = -1, int iData3 = -1, int iData4 = -1,
                                             int iFlags = 0, bool bAlt = false,
                                             bool bShift = false) const; // Exposed to Python
  DllExport void selectedCitiesGameNetMessage(int eMessage, int iData2 = -1, int iData3 = -1, int iData4 = -1,
                                              bool bOption = false, bool bAlt = false, bool bShift = false,
                                              bool bCtrl = false) const; // Exposed to Python
  DllExport void cityPushOrder(CvCity *pCity, OrderTypes eOrder, int iData, bool bAlt = false, bool bShift = false,
                               bool bCtrl = false) const; // Exposed to Python
  ...
  DllExport void implementDeal(PlayerTypes eWho, PlayerTypes eOtherWho, CLinkList<TradeData> *pOurList,
                               CLinkList<TradeData> *pTheirList, bool bForce = false);
  void verifyDeals();
  ...
  bool changePlayer(int playerIdx, int newCivType, int newLeader, int teamIdx, bool bIsHuman,
                    bool bChangeGraphics); //Rhye (jdog)
  void convertUnits(int playerIdx);        //Rhye (jdog)

  void addReplayMessage(ReplayMessageTypes eType = NO_REPLAY_MESSAGE, PlayerTypes ePlayer = NO_PLAYER,
                        CvWString pszText = L"", int iPlotX = -1, int iPlotY = -1, ColorTypes eColor = NO_COLOR);
  ...
  void setVoteSourceReligion(VoteSourceTypes eVoteSource, ReligionTypes eReligion,
                             bool bAnnounce = false); // Exposed to Python
  ...
  DllExport void handleCityScreenPlotDoublePicked(CvCity *pCity, CvPlot *pPlot, bool bAlt, bool bShift,
                                                  bool bCtrl) const;
  DllExport void handleCityScreenPlotRightPicked(CvCity *pCity, CvPlot *pPlot, bool bAlt, bool bShift,
                                                 bool bCtrl) const;
  ...
  // returns false if the region cities only with the allowed religion
  int countOwnedCities(int iCiv, int BLx, int BLy, int TRx, int TRy);
  // counts the number of cities owned by the player in the region
  bool canSeeAllTerrain(int iCiv, int iTerrain);
  // true if iCiv has all sea tiles revealed
  bool controlMostTeritory(int iCiv, int BLx, int BLy, int TRx, int TRy);
  // true if iCiv control more plots than any other nation

  int countCitiesLostTo(
      int iCiv,
      int iNewOwner); // count how many cities are lost to specific civ (i.e. belongs to iNewOwner and iCiv is the original Owner)
  bool safeMotherland(
      int iCiv); // returns true if the civ is someone else's Vassal or still keeps some of its Core and Normal Areas
