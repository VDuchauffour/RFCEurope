      ...
      if (pLoopPlot != NULL)
      {
        if (GET_PLAYER(getActivePlayer()).getAdvancedStartCityCost(true, pLoopPlot) > 0)
        {
          ...
          if (pLoopPlot->isRevealed(getActiveTeam(), false))
          {
            ...
            color.a = 0.4f;
            gDLL->getEngineIFace()->fillAreaBorderPlot(pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE(), color,
      ...
      for (iI = 0; iI < NUM_CITY_PLOTS; iI++)
      {
        if (pHeadSelectedCity->isWorkingPlot(iI))
        {
          ...
          if (pLoopPlot != NULL)
          {
            NiColorA color(GC.getColorInfo((ColorTypes)GC.getInfoTypeForString("COLOR_WHITE")).getColor());
            color.a = 0.7f;
            gDLL->getEngineIFace()->addColoredPlot(pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE(), color,
    ...
    else
    {
      ...
      while (pSelectedCityNode != NULL)
      {
        ...
        if (pSelectedCity != NULL)
        {
          ...
          {
            gDLL->getEngineIFace()->addColoredPlot(
                pRallyPlot->getX_INLINE(), pRallyPlot->getY_INLINE(),
                GC.getColorInfo((ColorTypes)GC.getInfoTypeForString("COLOR_YELLOW")).getColor(), PLOT_STYLE_CIRCLE,
  ...
  else if (pHeadSelectedUnit != NULL)
  {
        ...
        for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
        {
          ...
          if (pLoopPlot->getOwnerINLINE() == pHeadSelectedUnit->getOwnerINLINE())
          {
            if (pLoopPlot->getWorkingCity() != NULL)
            {
              NiColorA color(
                  GC
                      .getColorInfo((ColorTypes)GC.getInfoTypeForString(
                          "COLOR_HIGHLIGHT_TEXT") /*(GC.getPlayerColorInfo(GET_PLAYER(pHeadSelectedUnit->getOwnerINLINE()).getPlayerColor()).getColorTypePrimary())*/)
                      .getColor());
              color.a = 1.0f;
              gDLL->getEngineIFace()->fillAreaBorderPlot(pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE(), color,
                                                         AREA_BORDER_LAYER_CITY_RADIUS);
      ...
      if (iMaxAirRange > 0)
      {
        for (iDX = -(iMaxAirRange); iDX <= iMaxAirRange; iDX++)
        {
          for (iDY = -(iMaxAirRange); iDY <= iMaxAirRange; iDY++)
          {
            ...
            if (pLoopPlot != NULL)
            {
              if (plotDistance(pHeadSelectedUnit->getX_INLINE(), pHeadSelectedUnit->getY_INLINE(),
                               pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE()) <= iMaxAirRange)
              {
    ...
    else if (pHeadSelectedUnit->airRange() > 0) //other ranged units
    {
      ...
      for (iDX = -(iRange); iDX <= iRange; iDX++)
      {
        for (iDY = -(iRange); iDY <= iRange; iDY++)
        {
          ...
          {
            if (plotDistance(pHeadSelectedUnit->getX_INLINE(), pHeadSelectedUnit->getY_INLINE(),
                             pTargetPlot->getX_INLINE(), pTargetPlot->getY_INLINE()) <= iRange)
            {
                                                        ...
                                                        pHeadSelectedUnit->getFacingDirection(true)))
              {
                NiColorA color(GC.getColorInfo((ColorTypes)GC.getInfoTypeForString("COLOR_YELLOW")).getColor());
                color.a = 0.5f;
                gDLL->getEngineIFace()->fillAreaBorderPlot(pTargetPlot->getX_INLINE(), pTargetPlot->getY_INLINE(),
    ...
    if (!(GET_PLAYER(getActivePlayer()).isOption(PLAYEROPTION_NO_UNIT_RECOMMENDATIONS)))
    {
      if ((pHeadSelectedUnit->AI_getUnitAIType() == UNITAI_WORKER) ||
          (pHeadSelectedUnit->AI_getUnitAIType() == UNITAI_WORKER_SEA))
      {
          ...
          if (pCity != NULL)
          {
            if (pHeadSelectedUnit->AI_bestCityBuild(pCity, &pBestPlot))
            {
              ...
              if (pHeadSelectedUnit->AI_bestCityBuild(pCity, &pNextBestPlot, NULL, pBestPlot))
              {
                ...
                gDLL->getEngineIFace()->addColoredPlot(
                    pNextBestPlot->getX_INLINE(), pNextBestPlot->getY_INLINE(),
                    GC.getColorInfo((ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT")).getColor(),
                    PLOT_STYLE_CIRCLE, PLOT_LANDSCAPE_LAYER_RECOMMENDED_PLOTS);
              }
            }
        ...
        for (iDY = -(iRange); iDY <= iRange; iDY++)
        {
          ...
          if (pLoopPlot != NULL)
          {
            if ((pLoopPlot->area() == pHeadSelectedUnit->area()) ||
                pLoopPlot->isAdjacentToArea(pHeadSelectedUnit->area()))
            {
...
// BUFFY - Don't Recommend Plots in Fog of War - start
              ...
              if (plotDistance(pHeadSelectedUnit->getX_INLINE(), pHeadSelectedUnit->getY_INLINE(),
                               pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE()) <= iRange)
              {
                if (pLoopPlot->isVisible(pHeadSelectedUnit->getTeam(), false))
                {
                ...
                for (int j = -iBlockadeRange; j <= iBlockadeRange; ++j)
                {
                  ...
                  if (NULL != pLoopPlot && pLoopPlot->isRevealed(getActiveTeam(), false))
                  {
                    ...
                    if (pLoopPlot->isWater() && pLoopPlot->area() == pLoopUnit->area())
                    {
                      NiColorA color(
                          GC.getColorInfo(
                                (ColorTypes)GC.getPlayerColorInfo(GET_PLAYER(getActivePlayer()).getPlayerColor())
                                    .getColorTypePrimary())
                              .getColor());
                      color.a = 0.5f;
                      gDLL->getEngineIFace()->fillAreaBorderPlot(pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE(),
...
void CvGame::updateSelectionList()
{
  ...
  {
    if ((gDLL->getInterfaceIFace()->getOriginalPlot() == NULL) ||
        !(cyclePlotUnits(gDLL->getInterfaceIFace()->getOriginalPlot(), true, true,
                         gDLL->getInterfaceIFace()->getOriginalPlotCount())))
    {
      if ((gDLL->getInterfaceIFace()->getSelectionPlot() == NULL) ||
          !(cyclePlotUnits(gDLL->getInterfaceIFace()->getSelectionPlot(), true, true)))
      {
...
void CvGame::updateTestEndTurn()
{
  ...
  bAny = ((gDLL->getInterfaceIFace()->getHeadSelectedUnit() != NULL) &&
        ...
        else
        {
          ...
          else
          {
            if (GET_PLAYER(getActivePlayer()).isOption(PLAYEROPTION_WAIT_END_TURN) ||
                !(gDLL->getInterfaceIFace()->isHasMovedUnit()) || isHotSeat() || isPbem())
            {
...
void CvGame::cycleSelectionGroups(bool bClear, bool bForward, bool bWorkers) const
{
  ...
  else
  {
    ...
    pNextSelectionGroup =
        GC.getMapINLINE().findSelectionGroup(((pPlot != NULL) ? pPlot->getX() : 0),
                                             ((pPlot != NULL) ? pPlot->getY() : 0), getActivePlayer(), true, bWorkers);
  }
  ...
  if ((pCycleUnit != gDLL->getInterfaceIFace()->getHeadSelectedUnit()) ||
...
bool CvGame::selectCity(CvCity *pSelectCity, bool bCtrl, bool bAlt, bool bShift) const
{
  ...
  {
    int iLoop;
    for (CvCity *pLoopCity = GET_PLAYER(pSelectCity->getOwnerINLINE()).firstCity(&iLoop); pLoopCity != NULL;
         pLoopCity = GET_PLAYER(pSelectCity->getOwnerINLINE()).nextCity(&iLoop))
    {
...
void CvGame::selectionListMove(CvPlot *pPlot, bool bAlt, bool bShift, bool bCtrl) const
{
  ...
  selectionListGameNetMessage(GAMEMESSAGE_PUSH_MISSION, MISSION_MOVE_TO, pPlot->getX(), pPlot->getY(), 0, false,
                              bShift);
}

void CvGame::selectionListGameNetMessage(int eMessage, int iData2, int iData3, int iData4, int iFlags, bool bAlt,
                                         bool bShift) const
{
  CLLNode<IDInfo> *pSelectedUnitNode;
  CvUnit *pHeadSelectedUnit;
  ...
  argsList.add(bShift);
  long lResult = 0;
  gDLL->getPythonIFace()->callFunction(PYGameModule, "cannotSelectionListGameNetMessage", argsList.makeFunctionArgs(),
                                       &lResult);
  if (lResult == 1)
  {
    return;
        ...
        while (pSelectedUnitNode != NULL)
        {
          ...
          if ((iData2 == COMMAND_DELETE) && bAlt)
          {
            ...
            for (CvUnit *pLoopUnit = kPlayer.firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = kPlayer.nextUnit(&iLoop))
            {
              ...
              {
                CvMessageControl::getInstance().sendDoCommand(pLoopUnit->getID(), ((CommandTypes)iData2), iData3,
                                                              iData4, bAlt);
              }
            }
          }
          else
          {
            ...
            while (pSelectedUnitNode != NULL)
            {
              ...
              CvMessageControl::getInstance().sendDoCommand(pSelectedUnit->getID(), ((CommandTypes)iData2), iData3,
                                                            iData4, bAlt);
            }
          }
        ...
        else if ((eMessage == GAMEMESSAGE_PUSH_MISSION) || (eMessage == GAMEMESSAGE_AUTO_MISSION))
        {
          ...
          if (eMessage == GAMEMESSAGE_PUSH_MISSION)
          {
            ...
            if (((iData2 == MISSION_FORTIFY) || (iData2 == MISSION_SLEEP)) && bAlt)
            {
              ...
              for (CvSelectionGroup *pLoopSelectionGroup = kPlayer.firstSelectionGroup(&iLoop); pLoopSelectionGroup;
                   pLoopSelectionGroup = kPlayer.nextSelectionGroup(&iLoop))
              {
                if (pLoopSelectionGroup->allMatch(eUnit))
                  CvMessageControl::getInstance().sendPushMission(pLoopSelectionGroup->getHeadUnit()->getID(),
                                                                  ((MissionTypes)iData2), iData3, iData4, iFlags,
                                                                  bShift);
              }
            }
            else
            {
              // unchanged
              CvMessageControl::getInstance().sendPushMission(pHeadSelectedUnit->getID(), ((MissionTypes)iData2),
                                                              iData3, iData4, iFlags, bShift);
            }
            // BUG - All Units Actions - end
          }
          else
  ...
  void CvGame::selectedCitiesGameNetMessage(int eMessage, int iData2, int iData3, int iData4, bool bOption, bool bAlt,
                                            bool bShift, bool bCtrl) const
  {
    CLLNode<IDInfo> *pSelectedCityNode;
    CvCity *pSelectedCity;
      ...
      if (pSelectedCity != NULL)
      {
        if (pSelectedCity->getOwnerINLINE() == getActivePlayer())
        {
          ...
          case GAMEMESSAGE_DO_TASK:
            CvMessageControl::getInstance().sendDoTask(pSelectedCity->getID(), ((TaskTypes)iData2), iData3, iData4,
                                                       bOption, bAlt, bShift, bCtrl);
            break;
        ...
        if (isMPOption(MPOPTION_SIMULTANEOUS_TURNS) || GET_PLAYER(pHeadSelectedUnit->getOwnerINLINE()).isTurnActive())
        {
          ...
          if (GC.getActionInfo(iAction).getMissionType() != NO_MISSION)
          {
            ...
            if (pSelectedInterfaceList->canStartMission(GC.getActionInfo(iAction).getMissionType(),
                                                        GC.getActionInfo(iAction).getMissionData(), -1, pMissionPlot,
                                                        bTestVisible, bUseCache))
            {
              return true;
            }
          }
          ...
          {
            if (pSelectedInterfaceList->canDoCommand(((CommandTypes)(GC.getActionInfo(iAction).getCommandType())),
                                                     GC.getActionInfo(iAction).getCommandData(), -1, bTestVisible,
                                                     bUseCache))
            {
              return true;
            }
          ...
          if (gDLL->getInterfaceIFace()->canDoInterfaceMode(
                  ((InterfaceModeTypes)GC.getActionInfo(iAction).getInterfaceModeType()), pSelectedInterfaceList))
          {
            return true;
          }
        }
  ...
  void CvGame::handleAction(int iAction)
  {
    ...
    if (gDLL->getInterfaceIFace()->canDoInterfaceMode(
            (InterfaceModeTypes)GC.getActionInfo(iAction).getInterfaceModeType(),
            gDLL->getInterfaceIFace()->getSelectionList()))
    {
      pHeadSelectedUnit = gDLL->getInterfaceIFace()->getHeadSelectedUnit();

      if (pHeadSelectedUnit != NULL)
      {
        if (GC.getInterfaceModeInfo((InterfaceModeTypes)GC.getActionInfo(iAction).getInterfaceModeType())
                .getSelectAll())
        {
          gDLL->getInterfaceIFace()->selectGroup(pHeadSelectedUnit, false, false, true);
        }
        else if (GC.getInterfaceModeInfo((InterfaceModeTypes)GC.getActionInfo(iAction).getInterfaceModeType())
                     .getSelectType())
        {
          gDLL->getInterfaceIFace()->selectGroup(pHeadSelectedUnit, false, true, false);
        }
    ...
    {
      // BUG - All Units Actions - start
      selectionListGameNetMessage(GAMEMESSAGE_PUSH_MISSION, GC.getActionInfo(iAction).getMissionType(),
                                  GC.getActionInfo(iAction).getMissionData(), -1, 0, bAlt, bShift);
      // BUG - All Units Actions - end
    }
    ...
    if (GC.getActionInfo(iAction).getCommandType() != NO_COMMAND)
    {
      ...
      if (!bSkip)
      {
        ...
        {
          selectionListGameNetMessage(GAMEMESSAGE_DO_COMMAND, GC.getActionInfo(iAction).getCommandType(),
        ...
        for (int iI = 0; iI < (int)plotUnits.size(); iI++)
        {
          ...
          if (pUnit->getOwnerINLINE() == getActivePlayer())
          {
            if (!isMPOption(MPOPTION_SIMULTANEOUS_TURNS) ||
                getTurnSlice() - pUnit->getLastMoveTurn() > GC.getDefineINT("MIN_TIMER_UNIT_DOUBLE_MOVES"))
            {
              if (pUnit->isHurt())
              {
                if (pGroupHead != NULL)
  ...
  void CvGame::startFlyoutMenu(const CvPlot *pPlot, std::vector<CvFlyoutMenuData> &aFlyoutItems) const
  {
    ...
    if (pCity != NULL)
    {
      if (pCity->getOwnerINLINE() == getActivePlayer())
      {
        ...
        {
          UnitTypes eLoopUnit =
              (UnitTypes)GC.getCivilizationInfo(pCity->getCivilizationType()).getCivilizationUnits(iI);
          if (eLoopUnit != NO_UNIT)
          {
            if (pCity->canTrain(eLoopUnit))
            {
              szBuffer.Format(L"%s (%d)", GC.getUnitInfo(eLoopUnit).getDescription(),
                              pCity->getProductionTurnsLeft(eLoopUnit, 0));
              aFlyoutItems.push_back(
                  CvFlyoutMenuData(FLYOUT_TRAIN, eLoopUnit, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer));
            }
          }
        }
        ...
        for (int iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
        {
          ...
          if (eLoopBuilding != NO_BUILDING)
          {
            ...
            {
              szBuffer.Format(L"%s (%d)", GC.getBuildingInfo(eLoopBuilding).getDescription(),
                              pCity->getProductionTurnsLeft(eLoopBuilding, 0));
              aFlyoutItems.push_back(CvFlyoutMenuData(FLYOUT_CONSTRUCT, eLoopBuilding, pPlot->getX_INLINE(),
                                                      pPlot->getY_INLINE(), szBuffer));
            }
          }
        }
        ...
        for (int iI = 0; iI < GC.getNumProjectInfos(); iI++)
        {
          ...
          {
            szBuffer.Format(L"%s (%d)", GC.getProjectInfo(eProject).getDescription(),
                            pCity->getProductionTurnsLeft(eProject, 0));
            aFlyoutItems.push_back(
                CvFlyoutMenuData(FLYOUT_CREATE, eProject, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer));
          }
        }
        ...
        for (int iI = 0; iI < GC.getNumProcessInfos(); iI++)
        {
          if (pCity->canMaintain((ProcessTypes)iI))
          {
            ...
            aFlyoutItems.push_back(
                CvFlyoutMenuData(FLYOUT_MAINTAIN, iI, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer));
          }
        }
        ...
        for (int iI = 0; iI < GC.getNumHurryInfos(); iI++)
        {
          if (pCity->canHurry((HurryTypes)iI))
          {
            ...
            aFlyoutItems.push_back(
                CvFlyoutMenuData(FLYOUT_HURRY, iI, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer));
          }
        }
        ...
        if (pCity->canConscript())
        {
          ...
          {
            szBuffer = gDLL->getText("TXT_KEY_DRAFT_UNIT", GC.getUnitInfo(eConscriptUnit).getDescription(),
                                     pCity->getConscriptPopulation());
            aFlyoutItems.push_back(
                CvFlyoutMenuData(FLYOUT_CONSCRIPT, iI, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer));
          }
        }
      }
    }
    ...
    if (pHeadSelectedUnit != NULL && !pHeadSelectedUnit->atPlot(pPlot))
    {
      ...
      if ((pHeadSelectedUnit->getDomainType() == DOMAIN_AIR) ||
          gDLL->getFAStarIFace()->GeneratePath(&GC.getInterfacePathFinder(), pHeadSelectedUnit->getX(),
                                               pHeadSelectedUnit->getY(), pPlot->getX(), pPlot->getY(), false,
                                               MOVE_DECLARE_WAR, true))
      {
        if (pHeadSelectedUnit->getDomainType() == DOMAIN_AIR)
        {
        ...
        else
        {
          szBuffer = gDLL->getText("TXT_KEY_FLYOUT_MENU_MOVE_TO",
                                   gDLL->getFAStarIFace()->GetLastNode(&GC.getInterfacePathFinder())->m_iData2);
        }
        aFlyoutItems.push_back(
            CvFlyoutMenuData(FLYOUT_MOVE_TO, 0, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer));
      }
    }
    ...
    if (bUnits)
    {
      ...
      aFlyoutItems.push_back(
          CvFlyoutMenuData(FLYOUT_SELECT_ALL, 0, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer));

      if (bWakeUnit)
      {
        szBuffer = gDLL->getText("TXT_KEY_FLYOUT_MENU_WAKE_ALL");
        aFlyoutItems.push_back(
            CvFlyoutMenuData(FLYOUT_WAKE_ALL, 0, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer));
      }
      ...
      if (bFortifyUnit)
      {
        ...
        aFlyoutItems.push_back(
            CvFlyoutMenuData(FLYOUR_FORTIFY_ALL, 0, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer));
      }
      else if (bSleepUnit)
      {
        szBuffer = gDLL->getText("TXT_KEY_FLYOUT_MENU_SLEEP_ALL");
        aFlyoutItems.push_back(
            CvFlyoutMenuData(FLYOUR_SLEEP_ALL, 0, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer));
      }
      ...
      for (int iI = 0; iI < (int)plotUnits.size(); iI++)
      {
        ...
        if (pLoopUnit->getOwnerINLINE() == getActivePlayer())
        {
          ...
          aFlyoutItems.push_back(CvFlyoutMenuData(FLYOUT_SELECT_UNIT, pLoopUnit->getID(), pPlot->getX_INLINE(),
                                                  pPlot->getY_INLINE(), szTempBuffer.getCString()));
        }
      }
    }
  }
    ...
    if (pPlot != NULL)
    {
      ...
      case FLYOUT_HURRY:
      {
        ...
        if (pCity != NULL)
        {
          if (pCity->getOwnerINLINE() == getActivePlayer())
          {
            CvMessageControl::getInstance().sendDoTask(pCity->getID(), TASK_HURRY, kItem.m_iID, -1, false, false, false,
                                                       false);
          }
        }
      }
      ...
      case FLYOUT_CONSCRIPT:
      {
        ...
        if (pCity != NULL)
        {
          if (pCity->getOwnerINLINE() == getActivePlayer())
          {
            CvMessageControl::getInstance().sendDoTask(pCity->getID(), TASK_CONSCRIPT, -1, -1, false, false, false,
                                                       false);
          }
        }
      }
      ...
      case FLYOUR_SLEEP_ALL:
      {
        ...
        while (pUnitNode != NULL)
        {
          ...
          if (pLoopUnit->isGroupHead())
          {
            CvMessageControl::getInstance().sendPushMission(
                pLoopUnit->getID(), ((pLoopUnit->isFortifyable()) ? MISSION_FORTIFY : MISSION_SLEEP), -1, -1, 0, false);
          }
        }
      }
      break;
  ...
  CvPlot *CvGame::getNewHighlightPlot() const
  {
    ...
    if (gDLL->GetWorldBuilderMode())
    {
      ...
      CyArgsList argsList;
      argsList.add(0);
      bOK = gDLL->getPythonIFace()->callFunction(PYScreensModule, "WorldBuilderGetHighlightPlot",
                                                 argsList.makeFunctionArgs(), &coords);
      if (bOK && coords.size())
      {
  ...
  void CvGame::loadBuildQueue(const CvString &strItem) const
  {
    ...
    for (int iI = 0; iI < GC.getNumBuildingInfos(); iI++)
    {
      ...
      {
        eBuilding =
            (BuildingTypes)GC.getCivilizationInfo(getActiveCivilizationType()).getCivilizationBuildings(eBuildingClass);
    ...
    for (int iI = 0; iI < GC.getNumProcessInfos(); iI++)
    {
      ...
      {
        selectedCitiesGameNetMessage(GAMEMESSAGE_PUSH_ORDER, ORDER_MAINTAIN, ((ProcessTypes)iI), -1, false, false,
                                     true);
        return;
      }
    }
  ...
  bool CvGame::isSoundtrackOverride(CvString & strSoundtrack) const
  {
    ...
    {
      // Absinthe: victory/defeat soundtrack should not be looped in extended game state
  ...
  void CvGame::initSelection() const
  {
    ...
    int iLoop;
    for (CvUnit *pLoopUnit = GET_PLAYER(getActivePlayer()).firstUnit(&iLoop); pLoopUnit != NULL;
         pLoopUnit = GET_PLAYER(getActivePlayer()).nextUnit(&iLoop))
    {
      if (pLoopUnit->getGroup()->readyToSelect())
      {
    ...
    if (!gDLL->getInterfaceIFace()->isCitySelection())
    {
      if (GET_PLAYER(getActivePlayer()).isTurnActive())
      {
        if (isNetworkMultiPlayer())
        {
          ...
          if (isSimultaneousTeamTurns() && 1 == GET_TEAM(getActiveTeam()).countNumHumanGameTurnActive() &&
  ...
  {
    if (gDLL->getInterfaceIFace()->isCitySelection() || gDLL->getInterfaceIFace()->getHeadSelectedCity() != NULL ||
        gDLL->isDiplomacy() || gDLL->isMPDiplomacyScreenUp() || gDLL->GetWorldBuilderMode())
    {
      return false;
    }
  ...
  EndTurnButtonStates CvGame::getEndTurnState() const
  {
    ...
    if ((isNetworkMultiPlayer() &&
         (isMPOption(MPOPTION_SIMULTANEOUS_TURNS) && 1 == countNumHumanGameTurnActive() ||
          (!isSimultaneousTeamTurns() && 1 == GET_TEAM(getActiveTeam()).countNumHumanGameTurnActive() &&
           GET_TEAM(getActiveTeam()).getAliveCount() > 1))))
    {
  ...
  void CvGame::handleCityScreenPlotPicked(CvCity * pCity, CvPlot * pPlot, bool bAlt, bool bShift, bool bCtrl) const
  {
    ...
    if (pCity != NULL && pPlot != NULL)
    {
      ...
      if ((pPlot->getOwnerINLINE() == getActivePlayer()) && (pCity->getOwnerINLINE() == getActivePlayer()) &&
          (iIndex != -1))
      {
        CvMessageControl::getInstance().sendDoTask(pCity->getID(), TASK_CHANGE_WORKING_PLOT, iIndex, -1, false, bAlt,
                                                   bShift, bCtrl);
      }
  ...
  void CvGame::handleCityScreenPlotDoublePicked(CvCity * pCity, CvPlot * pPlot, bool bAlt, bool bShift, bool bCtrl)
      const
  {
    if (pCity != NULL)
    {
  ...
  void CvGame::handleCityScreenPlotRightPicked(CvCity * pCity, CvPlot * pPlot, bool bAlt, bool bShift, bool bCtrl) const
  {
    if (pCity != NULL && pPlot != NULL)
    {
      if ((pCity->getOwnerINLINE() == getActivePlayer()) && (pPlot->getOwnerINLINE() == getActivePlayer()) &&
          (pCity->getCityPlotIndex(pPlot) != -1))
      {
        CvMessageControl::getInstance().sendDoTask(pCity->getID(), TASK_CLEAR_WORKING_OVERRIDE,
                                                   pCity->getCityPlotIndex(pPlot), -1, false, bAlt, bShift, bCtrl);
      }
    }
  }
