...
void CvInitCore::resetGame(CvInitCore *pSource, bool bClear, bool bSaveGameType)
{
  ...
  FAssertMsg(!bClear || !bSaveGameType,
             "Should not be clearing data while trying to preserve gametype info in CvInitCore::resetGame");
...
void CvInitCore::resetPlayer(PlayerTypes eID, CvInitCore *pSource, bool bClear, bool bSaveSlotInfo)
{
  ...
  FAssertMsg(!bClear || !bSaveSlotInfo,
             "Should not be clearing data while trying to preserve slot info in CvInitCore::resetPlayer");
...
void CvInitCore::setGameSpeed(const CvWString &szGameSpeed)
{
  ...
  {
    if (wcsicmp(szGameSpeed.GetCString(), CvWString(GC.getGameSpeedInfo((GameSpeedTypes)i).getType()).GetCString()) ==
        0)
    {
...
void CvInitCore::setTurnTimer(const CvWString &szTurnTimer)
{
  ...
  {
    if (wcsicmp(szTurnTimer.GetCString(), CvWString(GC.getTurnTimerInfo((TurnTimerTypes)i).getType()).GetCString()) ==
        0)
    {
...
void CvInitCore::refreshCustomMapOptions()
{
  ...
  if (!getWBMapScript())
  {
    if (gDLL->pythonMapExists(CvString(getMapScriptName()).GetCString()))
    {
      ...
      bOK = gDLL->getPythonIFace()->callFunction(CvString(getMapScriptName()).GetCString(), "getNumCustomMapOptions",
                                                 NULL, &iNumOptions);
      if (bOK)
      {
        ...
        for (int i = 0; i < iNumOptions; ++i)
        {
          ...
          bOK = gDLL->getPythonIFace()->callFunction(CvString(getMapScriptName()).GetCString(),
                                                     "getCustomMapOptionDefault", argsList.makeFunctionArgs(),
                                                     &iOptionDefault);
          FAssertMsg(bOK,
                     "Call to python fn \"getCustomMapOptionDefault\" failed in CvInitCore::refreshCustomMapOptions");
          if (bOK)
          {
