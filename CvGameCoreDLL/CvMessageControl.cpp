...
void CvMessageControl::sendPushOrder(int iCityID, OrderTypes eOrder, int iData, bool bAlt, bool bShift, bool bCtrl)
{
  if (NO_PLAYER != GC.getGameINLINE().getActivePlayer())
  {
    gDLL->sendMessageData(
        new CvNetPushOrder(GC.getGameINLINE().getActivePlayer(), iCityID, eOrder, iData, bAlt, bShift, bCtrl));
  }
}
...
void CvMessageControl::sendDoTask(int iCity, TaskTypes eTask, int iData1, int iData2, bool bOption, bool bAlt,
                                  bool bShift, bool bCtrl)
{
  if (NO_PLAYER != GC.getGameINLINE().getActivePlayer())
  {
    gDLL->sendMessageData(new CvNetDoTask(GC.getGameINLINE().getActivePlayer(), iCity, eTask, iData1, iData2, bOption,
                                          bAlt, bShift, bCtrl));
  }
}
...
void CvMessageControl::sendAdvancedStartAction(AdvancedStartActionTypes eAction, PlayerTypes ePlayer, int iX, int iY,
                                               int iData, bool bAdd)
{
...
void CvMessageControl::sendPushMission(int iUnitID, MissionTypes eMission, int iData1, int iData2, int iFlags,
                                       bool bShift)
{
  if (NO_PLAYER != GC.getGameINLINE().getActivePlayer())
  {
    gDLL->sendMessageData(
        new CvNetPushMission(GC.getGameINLINE().getActivePlayer(), iUnitID, eMission, iData1, iData2, iFlags, bShift));
  }
}
...
void CvMessageControl::sendDoCommand(int iUnitID, CommandTypes eCommand, int iData1, int iData2, bool bAlt)
{
  ...
  {
    gDLL->sendMessageData(
        new CvNetDoCommand(GC.getGameINLINE().getActivePlayer(), iUnitID, eCommand, iData1, iData2, bAlt));
  }
}
...
void CvMessageControl::sendChangeVassal(TeamTypes eMasterTeam, bool bVassal, bool bCapitulated)
{
  ...
  {
    gDLL->sendMessageData(
        new CvNetChangeVassal(GC.getGameINLINE().getActivePlayer(), eMasterTeam, bVassal, bCapitulated));
  }
}
