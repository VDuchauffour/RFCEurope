...
void CvPlotGroup::recalculatePlots()
{
  ...
  if (pPlotNode != NULL)
  {
    ...
    gDLL->getFAStarIFace()->GeneratePath(&GC.getPlotGroupFinder(), pPlot->getX_INLINE(), pPlot->getY_INLINE(), -1, -1,
                                         false, eOwner);
...
void CvPlotGroup::read(FDataStreamBase *pStream)
{
  ...
  FAssertMsg((0 < GC.getNumBonusInfos()),
             "GC.getNumBonusInfos() is not greater than zero but an array is being allocated in CvPlotGroup::read");
...
void CvPlotGroup::write(FDataStreamBase *pStream)
{
  ...
  FAssertMsg((0 < GC.getNumBonusInfos()),
             "GC.getNumBonusInfos() is not greater than zero but an array is being allocated in CvPlotGroup::write");
