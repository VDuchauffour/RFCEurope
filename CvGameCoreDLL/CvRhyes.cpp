...
bool MiroBelongToCore(int iCiv, int x, int y)
{
  ...
  if ((x >= CoreAreasRect[iCiv][0]) && (y >= CoreAreasRect[iCiv][1]) && (x <= CoreAreasRect[iCiv][2]) &&
      (y <= CoreAreasRect[iCiv][3]))
  {
    return true;
  }
  else
  {
    ...
    {
      if ((CoreAreasMinus[iCiv][2 * i] == x) && (CoreAreasMinus[iCiv][2 * i + 1] == y))
        return true;
    };
  };
  return false;
...
{
  if ((x >= NormalAreasRect[iCiv][0]) && (y >= NormalAreasRect[iCiv][1]) && (x <= NormalAreasRect[iCiv][2]) &&
      (y <= NormalAreasRect[iCiv][3]))
  {
    for (int i = 0; i < NormalAreasMinusCount[iCiv]; i++)
    {
      if ((NormalAreasMinus[iCiv][2 * i] == x) && (NormalAreasMinus[iCiv][2 * i + 1] == y))
        return false;
    };
    return true;
  };
...
{
  if (warsMaps == NULL || iCiv >= NUM_MAJOR_PLAYERS)
  { //fixed value for the Pope and the Independents
    return 0;
  }
  else
...
{
  if ((iCiv >= INDEP_START) && (iCiv <= INDEP_END))
    return true;
  return false;
};
