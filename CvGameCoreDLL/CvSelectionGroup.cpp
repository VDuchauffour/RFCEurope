...
void CvSelectionGroup::doTurn()
{
  ...
  if (getNumUnits() > 0)
  {
    ...
    if ((eActivityType == ACTIVITY_HOLD) || ((eActivityType == ACTIVITY_HEAL) && (AI_isControlled() || !bHurt)) ||
...
// BUG - Sentry Actions - start
      ...
      if (((eActivityType == ACTIVITY_SENTRY_NAVAL_UNITS) && (sentryAlertSameDomainType())) ||
          ...
          ((eActivityType == ACTIVITY_SENTRY_WHILE_HEAL) &&
           (sentryAlertSameDomainType() || AI_isControlled() || !bHurt)))
      {
        setActivityType(ACTIVITY_AWAKE);
      }
#endif
      ...
      if (isAutomated() && getAutomateType() == AUTOMATE_EXPLORE &&
          getBugOptionBOOL("Actions__SentryHealing", true, "BUG_SENTRY_HEALING") && sentryAlert())
      {
        if (!(getBugOptionBOOL("Actions__SentryHealingOnlyNeutral", true, "BUG_SENTRY_HEALING_ONLY_NEUTRAL") &&
              plot()->isOwned()))
        {
          setActivityType(ACTIVITY_AWAKE);
        }
      ...
      if (eActivityType == ACTIVITY_HEAL && getBugOptionBOOL("Actions__SentryHealing", true, "BUG_SENTRY_HEALING") &&
          sentryAlert())
      {
        if (!(getBugOptionBOOL("Actions__SentryHealingOnlyNeutral", true, "BUG_SENTRY_HEALING_ONLY_NEUTRAL") &&
              plot()->isOwned()))
    ...
    if (AI_isControlled())
    {
      ...
      if ((getActivityType() != ACTIVITY_MISSION) ||
          (!canFight() && (GET_PLAYER(getOwnerINLINE()).AI_getAnyPlotDanger(plot(), 2))))
      /************************************************************************************************/
    ...
    if (isHuman())
    {
      if (GC.getGameINLINE().isMPOption(MPOPTION_SIMULTANEOUS_TURNS))
      {
        ...
        while (pUnitNode != NULL)
        {
          ...
          iWaitTurns = (GC.getDefineINT("MIN_TIMER_UNIT_DOUBLE_MOVES") -
                        (GC.getGameINLINE().getTurnSlice() - pLoopUnit->getLastMoveTurn()));
...
void CvSelectionGroup::pushMission(MissionTypes eMission, int iData1, int iData2, int iFlags, bool bAppend,
                                   bool bManual, MissionAITypes eMissionAI, CvPlot *pMissionAIPlot,
                                   CvUnit *pMissionAIUnit)
{
  PROFILE_FUNC();
...
// BUG - Sentry Actions - start
    ...
    case MISSION_MOVE_TO_SENTRY:
      ...
      {
        checkLastPathPlot(
            GC.getMapINLINE().plotINLINE(headMissionQueueNode()->m_data.iData1, headMissionQueueNode()->m_data.iData2));
      }
      break;
      // BUG - Safe Move - end
...
// BUG - Sentry Actions - start
        ...
        case MISSION_NUKE:
          if (pLoopUnit->nuke(headMissionQueueNode()->m_data.iData1, headMissionQueueNode()->m_data.iData2))
          {
            ...
            if (GC.getMapINLINE()
                    .plotINLINE(headMissionQueueNode()->m_data.iData1, headMissionQueueNode()->m_data.iData2)
        ...
        case MISSION_ESPIONAGE:
          if (pLoopUnit->espionage((EspionageMissionTypes)headMissionQueueNode()->m_data.iData1,
        ...
        if (getOwnerINLINE() == GC.getGameINLINE().getActivePlayer())
        {
          if (IsSelected())
          {
            gDLL->getInterfaceIFace()->changeCycleSelectionCounter(
                (GET_PLAYER(getOwnerINLINE()).isOption(PLAYEROPTION_QUICK_MOVES)) ? 1 : 2);
          }
        }
...
void CvSelectionGroup::continueMission(int iSteps)
{
  ...
  if (headMissionQueueNode()->m_data.iPushTurn == GC.getGameINLINE().getGameTurn() ||
      (headMissionQueueNode()->m_data.iFlags & MOVE_THROUGH_ENEMY))
  {
    if (headMissionQueueNode()->m_data.eMissionType == MISSION_MOVE_TO)
    {
      bool bFailedAlreadyFighting;
      if (groupAttack(headMissionQueueNode()->m_data.iData1, headMissionQueueNode()->m_data.iData2,
                      headMissionQueueNode()->m_data.iFlags, bFailedAlreadyFighting))
      {
        bDone = true;
      }
    }
...
// BUG - Sentry Actions - start
        ...
        case MISSION_MOVE_TO_SENTRY:
          ...
          {
            checkLastPathPlot(GC.getMapINLINE().plotINLINE(headMissionQueueNode()->m_data.iData1,
                                                           headMissionQueueNode()->m_data.iData2));
          }
          // BUG - Safe Move - end
          ...
          else
          {
            ...
            if (headMissionQueueNode() != NULL && headMissionQueueNode()->m_data.eMissionType == MISSION_MOVE_TO)
            {
              if (getX() != headMissionQueueNode()->m_data.iData1 && getY() != headMissionQueueNode()->m_data.iData2)
              {
                if (!(plot()->isWater() &&
                      GC.getMap()
                          .plot(headMissionQueueNode()->m_data.iData1, headMissionQueueNode()->m_data.iData2)
                          ->isCoastalLand()))
                {
                  pushMission(MISSION_SKIP);
                }
        ...
        case MISSION_ROUTE_TO:
          if (groupRoadTo(headMissionQueueNode()->m_data.iData1, headMissionQueueNode()->m_data.iData2,
                          headMissionQueueNode()->m_data.iFlags))
          {
            bAction = true;
          }
        ...
        case MISSION_MOVE_TO_UNIT:
          ...
          pTargetUnit = GET_PLAYER((PlayerTypes)headMissionQueueNode()->m_data.iData1)
...
bool CvSelectionGroup::canEverDoCommand(CommandTypes eCommand, int iData1, int iData2, bool bTestVisible,
                                        bool bUseCache)
{
...
{
  return ((getActivityType() == ACTIVITY_HOLD) || (getActivityType() == ACTIVITY_SLEEP) ||
          (getActivityType() == ACTIVITY_HEAL) || (getActivityType() == ACTIVITY_SENTRY) ||
// BUG - Sentry Actions - start
#ifdef _MOD_SENTRY
          (getActivityType() == ACTIVITY_SENTRY_WHILE_HEAL) || (getActivityType() == ACTIVITY_SENTRY_NAVAL_UNITS) ||
          (getActivityType() == ACTIVITY_SENTRY_LAND_UNITS) ||
#endif
          // BUG - Sentry Actions - end
          (getActivityType() == ACTIVITY_PATROL) || (getActivityType() == ACTIVITY_PLUNDER) ||
...
bool CvSelectionGroup::canMoveThrough(CvPlot *pPlot)
{
  ...
  if (getNumUnits() > 0)
  {
    ...
    while (pUnitNode != NULL)
    {
      iCounter++; //Rhye (fix)
      if (iCounter > 50)
        return false; //Rhye (fix) // 3Miro: increase this to 50
...

{
  ...
  for (pLoopCity = GET_PLAYER(ePlayer).firstCity(&iLoop); pLoopCity != NULL;
...
bool CvSelectionGroup::groupAttack(int iX, int iY, int iFlags, bool &bFailedAlreadyFighting)
{
  ...
  bool bStack = (isHuman() &&
                 ((getDomainType() == DOMAIN_AIR) || GET_PLAYER(getOwnerINLINE()).isOption(PLAYEROPTION_STACK_ATTACK)));
  ...
  {
    if ((getDomainType() == DOMAIN_AIR) ||
        (stepDistance(getX(), getY(), pDestPlot->getX_INLINE(), pDestPlot->getY_INLINE()) == 1))
    {
      // Absinthe: added missing flag
      if ((iFlags & MOVE_DIRECT_ATTACK) || (getDomainType() == DOMAIN_AIR) || (iFlags & MOVE_THROUGH_ENEMY) ||
          (generatePath(plot(), pDestPlot, iFlags) && (getPathFirstPlot() == pDestPlot)))
...
// BUG - Sentry Actions - start
#ifdef _MOD_SENTRY
    bool bSentryAlert = isHuman() && NULL != headMissionQueueNode() &&
                        headMissionQueueNode()->m_data.eMissionType == MISSION_MOVE_TO_SENTRY &&
    ...
    while (pUnitNode != NULL)
    {
      if (movedUnitIds.empty() ||
          std::find(movedUnitIds.begin(), movedUnitIds.end(), pUnitNode->m_data.iID) == movedUnitIds.end())
      {
        pLoopUnit = ::getUnit(pUnitNode->m_data);
...
// BUG - Sentry Actions - start
    ...
    if ((!bSentryAlert && pLoopUnit->canMove() &&
...
#else
    if ((pLoopUnit->canMove() && ((bCombat && (!(pLoopUnit->isNoCapture()) || !(pPlot->isEnemyCity(*pLoopUnit))))
                                      ? pLoopUnit->canMoveOrAttackInto(pPlot)
                                      : pLoopUnit->canMoveInto(pPlot))) ||
        (pLoopUnit == pCombatUnit))
#endif
    ...
    else
    {
      ...
      pLoopUnit->ExecuteMove(
          ((float)(GC.getMissionInfo(MISSION_MOVE_TO).getTime() * gDLL->getMillisecsPerTurn())) / 1000.0f, false);
      /************************************************************************************************/
      ...
      if (pLoopUnit->m_iStuckLoopCount > 5)
      {
        ...
        if (NULL != pHeadUnit)
        {
          ...
          sprintf(szOut, "Unit stuck in loop: %S(%S)[%d, %d] (%S)\n", pHeadUnit->getName().GetCString(),
                  GET_PLAYER(pHeadUnit->getOwnerINLINE()).getName(), pHeadUnit->getX_INLINE(), pHeadUnit->getY_INLINE(),
...
bool CvSelectionGroup::groupBuild(BuildTypes eBuild)
{
    ...
    if (AI_isControlled())
    {
      if (GET_PLAYER(getOwnerINLINE()).isOption(PLAYEROPTION_SAFE_AUTOMATION))
      {
        if ((pPlot->getImprovementType() != NO_IMPROVEMENT) &&
            (pPlot->getImprovementType() != (ImprovementTypes)(GC.getDefineINT("RUINS_IMPROVEMENT"))))
        {
  ...
  if (eFeature != NO_FEATURE && isHuman() && kBuildInfo.isFeatureRemove(eFeature) &&
      kBuildInfo.getFeatureProduction(eFeature) != 0)
  {
  ...
  while (pUnitNode != NULL)
  {
    ...
    if (pLoopUnit->canBuild(pPlot, eBuild))
    {
      ...
      if (bCheckChop && pPlot->getBuildTurnsLeft(eBuild, getOwnerINLINE()) == 1)
      {
        ...
        {
          CvWString szBuffer =
              gDLL->getText("TXT_KEY_BUG_PRECLEARING_FEATURE_BONUS", GC.getFeatureInfo(eFeature).getTextKeyWide(),
                            iProduction, pCity->getNameKey());
          gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer,
                                                ARTFILEMGR.getInterfaceArtInfo("WORLDBUILDER_CITY_EDIT")->getPath(),
                                                MESSAGE_TYPE_INFO, GC.getFeatureInfo(eFeature).getButton(),
                                                (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), getX(), getY(),
                                                true, true);
        }
        bContinue = false;
        break;
      ...
      while (pUnitNode != NULL && !bLoadedOne)
      {
        ...
        if (pLoopUnit != NULL && pLoopUnit->getTransportUnit() != pTransportUnit)
        {
          ...
          bool bSpaceAvailable =
              pTransportUnit->cargoSpaceAvailable(pLoopUnit->getSpecialUnitType(), pLoopUnit->getDomainType());
          if (bSpaceAvailable)
          {
        ...
        if ((pLoopUnit1->getCargo() > 0) && (pLoopUnit1->domainCargo() == DOMAIN_LAND))
        {
          ...
          for (uint i = 0; i < aCargoGroups.size(); ++i)
          {
            ...
            if (pGroup->canAllMove())
            {
              ...
              pGroup->pushMission(MISSION_MOVE_TO, pPlot->getX_INLINE(), pPlot->getY_INLINE(),
                                  (MOVE_IGNORE_DANGER | iFlags));
              bLanding = true;
            }
          }
        }
...
{
  return (((bAny) ? canAnyMove() : canAllMove()) && (headMissionQueueNode() == NULL) &&
          (getActivityType() == ACTIVITY_AWAKE) && !isBusy() && !isCargoBusy());
}
...
// BUG - Sentry Actions - start
    ...
    if (isHuman() && (isAutomated() ||
                      (GET_PLAYER((GC.getGameINLINE().isNetworkMultiPlayer()) ? getOwnerINLINE()
...
bool CvSelectionGroup::generatePath(const CvPlot *pFromPlot, const CvPlot *pToPlot, int iFlags, bool bReuse,
                                    int *piPathTurns) const
{
  ...
  bSuccess =
      gDLL->getFAStarIFace()->GeneratePath(&GC.getPathFinder(), pFromPlot->getX_INLINE(), pFromPlot->getY_INLINE(),
                                           pToPlot->getX_INLINE(), pToPlot->getY_INLINE(), false, iFlags, bReuse);
