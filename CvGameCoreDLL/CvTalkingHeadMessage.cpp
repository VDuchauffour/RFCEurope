...
CvTalkingHeadMessage::CvTalkingHeadMessage(int iMessageTurn, int iLen, LPCWSTR pszDesc, LPCTSTR pszSound,
                                           InterfaceMessageTypes eType, LPCTSTR pszIcon, ColorTypes eColor, int iX,
                                           int iY, bool bShowOffScreenArrows, bool bShowOnScreenArrows)
    : m_iTurn(iMessageTurn), m_szDescription(pszDesc), m_szSound(pszSound), m_szIcon(pszIcon), m_iLength(iLen),
      m_eFlashColor(eColor), m_iFlashX(iX), m_iFlashY(iY), m_bOffScreenArrows(bShowOffScreenArrows),
      m_bOnScreenArrows(bShowOnScreenArrows), m_eMessageType(eType), m_eFromPlayer(NO_PLAYER), m_eTarget(NO_CHATTARGET),
      m_bShown(false)
{
}
