...
CvTeam::CvTeam()
{
  ...
  m_paiFreeBonus = NULL; // 3MiroProjects
...
void CvTeam::uninit()
{
  ...
  SAFE_DELETE_ARRAY(m_paiFreeBonus); // 3MiroProjects
...
{
  //gDLL->logMsg("sdkDbg.log", " team reset ");
  int iI, iJ;
  ...
  if (!bConstructorCall)
  {
    ...
    FAssertMsg(m_aiForceTeamVoteEligibilityCount == NULL,
               "about to leak memory, CvTeam::m_aiForceTeamVoteEligibilityCount");
...
void CvTeam::addTeam(TeamTypes eTeam)
{
  ...
  for (iI = 0; iI < MAX_PLAYERS; iI++)
  {
    if (GET_PLAYER((PlayerTypes)iI).isAlive())
    {
      ...
      {
        if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(getID()) &&
            GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(eTeam))
        {
          szBuffer = gDLL->getText("TXT_KEY_MISC_PLAYER_PERMANENT_ALLIANCE", getName().GetCString(),
          ...
          gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer,
                                                     "AS2D_THEIRALLIANCE", MESSAGE_TYPE_MINOR_EVENT, NULL,
  ...
  szBuffer = gDLL->getText("TXT_KEY_MISC_PLAYER_PERMANENT_ALLIANCE", getName().GetCString(),
                           GET_TEAM(eTeam).getName().GetCString());
  GC.getGameINLINE().addReplayMessage(REPLAY_MESSAGE_MAJOR_EVENT, getLeaderID(), szBuffer, -1, -1,
                                      (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
       ...
       pLoopDeal = GC.getGameINLINE().nextDeal(&iLoop))
  {
    if (((GET_PLAYER(pLoopDeal->getFirstPlayer()).getTeam() == getID()) &&
         (GET_PLAYER(pLoopDeal->getSecondPlayer()).getTeam() == eTeam)) ||
        ((GET_PLAYER(pLoopDeal->getFirstPlayer()).getTeam() == eTeam) &&
      ...
      if (pLoopDeal->getFirstTrades() != NULL)
      {
        for (pNode = pLoopDeal->getFirstTrades()->head(); pNode; pNode = pLoopDeal->getFirstTrades()->next(pNode))
        {
          if ((pNode->m_data.m_eItemType == TRADE_OPEN_BORDERS) ||
              ...
              (pNode->m_data.m_eItemType == TRADE_PEACE_TREATY) || (pNode->m_data.m_eItemType == TRADE_VASSAL) ||
  ...
  for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
  {
    ...
    {
      pLoopPlot->changeInvisibleVisibilityCount(getID(), ((InvisibleTypes)iJ),
                                                pLoopPlot->getInvisibleVisibilityCount(eTeam, ((InvisibleTypes)iJ)));
    }
  ...
  for (iI = 0; iI < MAX_TEAMS; iI++)
  {
    if ((iI != getID()) && (iI != eTeam))
    {
      GET_TEAM((TeamTypes)iI)
          .setWarWeariness(getID(), ((GET_TEAM((TeamTypes)iI).getWarWeariness(getID()) +
                                      GET_TEAM((TeamTypes)iI).getWarWeariness(eTeam)) /
                                     2));
      GET_TEAM((TeamTypes)iI)
          .setStolenVisibilityTimer(getID(), ((GET_TEAM((TeamTypes)iI).getStolenVisibilityTimer(getID()) +
                                               GET_TEAM((TeamTypes)iI).getStolenVisibilityTimer(eTeam)) /
                                              2));
      GET_TEAM((TeamTypes)iI)
          .AI_setAtWarCounter(getID(), ((GET_TEAM((TeamTypes)iI).AI_getAtWarCounter(getID()) +
                                         GET_TEAM((TeamTypes)iI).AI_getAtWarCounter(eTeam)) /
                                        2));
      GET_TEAM((TeamTypes)iI)
          .AI_setAtPeaceCounter(getID(), ((GET_TEAM((TeamTypes)iI).AI_getAtPeaceCounter(getID()) +
                                           GET_TEAM((TeamTypes)iI).AI_getAtPeaceCounter(eTeam)) /
                                          2));
      GET_TEAM((TeamTypes)iI)
          .AI_setHasMetCounter(getID(), ((GET_TEAM((TeamTypes)iI).AI_getHasMetCounter(getID()) +
                                          GET_TEAM((TeamTypes)iI).AI_getHasMetCounter(eTeam)) /
                                         2));
      GET_TEAM((TeamTypes)iI)
          .AI_setDefensivePactCounter(getID(), ((GET_TEAM((TeamTypes)iI).AI_getDefensivePactCounter(getID()) +
                                                 GET_TEAM((TeamTypes)iI).AI_getDefensivePactCounter(eTeam)) /
                                                2));
      GET_TEAM((TeamTypes)iI)
          .AI_setShareWarCounter(getID(), ((GET_TEAM((TeamTypes)iI).AI_getShareWarCounter(getID()) +
                                            GET_TEAM((TeamTypes)iI).AI_getShareWarCounter(eTeam)) /
                                           2));
      GET_TEAM((TeamTypes)iI)
          .AI_setWarSuccess(getID(), ((GET_TEAM((TeamTypes)iI).AI_getWarSuccess(getID()) +
                                       GET_TEAM((TeamTypes)iI).AI_getWarSuccess(eTeam)) /
                                      2));
      GET_TEAM((TeamTypes)iI)
          .AI_setEnemyPeacetimeTradeValue(getID(), ((GET_TEAM((TeamTypes)iI).AI_getEnemyPeacetimeTradeValue(getID()) +
                                                     GET_TEAM((TeamTypes)iI).AI_getEnemyPeacetimeTradeValue(eTeam)) /
                                                    2));
      GET_TEAM((TeamTypes)iI)
          .AI_setEnemyPeacetimeGrantValue(getID(), ((GET_TEAM((TeamTypes)iI).AI_getEnemyPeacetimeGrantValue(getID()) +
...
void CvTeam::shareItems(TeamTypes eTeam)
{
  ...
  {
    setEspionagePointsAgainstTeam((TeamTypes)iTeam,
                                  std::max(GET_TEAM(eTeam).getEspionagePointsAgainstTeam((TeamTypes)iTeam),
                                           getEspionagePointsAgainstTeam((TeamTypes)iTeam)));
  }
...
void CvTeam::processBuilding(BuildingTypes eBuilding, int iChange)
{
  for (int i = 0; i < GC.getNumVoteSourceInfos(); ++i)
  {
    ...
    {
      changeForceTeamVoteEligibilityCount((VoteSourceTypes)i,
                                          (GC.getBuildingInfo(eBuilding).isForceTeamVoteEligible()) ? iChange : 0);
    }
  }
...
void CvTeam::updateYield()
{
  ...
  // }
  // }
  for (std::vector<PlayerTypes>::const_iterator iter = m_aePlayerMembers.begin(); iter != m_aePlayerMembers.end();
       ++iter)
  {
...
void CvTeam::declareWar(TeamTypes eTeam, bool bNewDiplo, WarPlanTypes eWarPlan)
{
  ...
  if (!isAtWar(eTeam))
  {
    ...
    GET_TEAM(eTeam).AI_setWarPlan(getID(),
                                  ((isBarbarian() || isMinorCiv()) ? WARPLAN_ATTACKED : WARPLAN_ATTACKED_RECENT));
            ...
            if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
            {
              ...
              else if (GET_PLAYER((PlayerTypes)iJ).getTeam() != getID())
              {
                if (GET_TEAM(GET_PLAYER((PlayerTypes)iJ).getTeam()).isHasMet(eTeam))
                {
                  ...
                  //Rhye - end
                  {
                    GET_PLAYER((PlayerTypes)iJ)
                        .AI_changeMemoryCount(((PlayerTypes)iI), MEMORY_DECLARED_WAR_ON_FRIEND, 1);
                  }
                }
              }
            }
...

      {
...

        {
          ...
          if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID() || GET_PLAYER((PlayerTypes)iI).getTeam() == eTeam
              ...
              || GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isAtWar(getID()) ||
              GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isAtWar(eTeam))

          {
...

            {
              ...
              if (GET_PLAYER((PlayerTypes)iJ).isAlive() &&
                  GET_PLAYER((PlayerTypes)iJ).getTeam() != GET_PLAYER((PlayerTypes)iI).getTeam())
    ...
    if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
    {
      ...
      // <trs.sound-once>
      bool const bPlaySound =
          ((!isAVassal() || isHuman()) && !GET_TEAM(eTeam).isAVassal() || GET_TEAM(eTeam).isHuman());
      // </trs.sound-once>

      if (!isBarbarian() && !(GET_TEAM(eTeam).isBarbarian()) && !isMinorCiv() && !(GET_TEAM(eTeam).isMinorCiv()))
      {
        for (iI = 0; iI < MAX_PLAYERS; iI++)
        {
          if (GET_PLAYER((PlayerTypes)iI).isAlive())
          {
            if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
            {
              ...
              szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_DECLARED_WAR_ON",
                                       GET_PLAYER((PlayerTypes)eTeam).getCivilizationShortDescription()); //Rhye
              gDLL->getInterfaceIFace()->addHumanMessage(
                  ((PlayerTypes)iI), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, bPlaySound ? "AS2D_DECLAREWAR" : NULL,
            ...
            else if (GET_PLAYER((PlayerTypes)iI).getTeam() == eTeam)
            {
              ...
              szBuffer = gDLL->getText("TXT_KEY_MISC_DECLARED_WAR_ON_YOU",
                                       GET_PLAYER((PlayerTypes)getID()).getCivilizationShortDescription()); //Rhye
              gDLL->getInterfaceIFace()->addHumanMessage(
                  ((PlayerTypes)iI), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, bPlaySound ? "AS2D_DECLAREWAR" : NULL,
                  MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"));
            }
            else if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(getID()) &&
                     GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(eTeam))
            {
              ...
              szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_DECLARED_WAR",
                                       GET_PLAYER((PlayerTypes)getID()).getCivilizationShortDescription(),
                                       GET_PLAYER((PlayerTypes)eTeam).getCivilizationShortDescription()); //Rhye
              gDLL->getInterfaceIFace()->addHumanMessage(
                  ((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, bPlaySound ? "AS2D_DECLAREWAR" : NULL,
                  MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"));
            }
          }
        }
        ...
        szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_DECLARES_WAR",
                                 GET_PLAYER((PlayerTypes)getID()).getCivilizationShortDescription(),
                                 GET_PLAYER((PlayerTypes)eTeam).getCivilizationShortDescription());
        GC.getGameINLINE().addReplayMessage(REPLAY_MESSAGE_MAJOR_EVENT, getLeaderID(), szBuffer, -1, -1,
      ...
      for (iI = 0; iI < MAX_TEAMS; iI++)
      {
        if (GET_TEAM((TeamTypes)iI).isAtWar(getID()))
        {
          for (int iJ = 0; iJ < GET_TEAM(eTeam).getPlayerMemberListSize(); iJ++)
          {
            ...
            {
              GET_PLAYER(GET_TEAM(eTeam).getPlayerMemberAt(iJ))
                  .AI_invalidateAttitudeCache(GET_TEAM((TeamTypes)iI).getPlayerMemberAt(iK));
              GET_PLAYER(GET_TEAM((TeamTypes)iI).getPlayerMemberAt(iK))
                  .AI_invalidateAttitudeCache(GET_TEAM(eTeam).getPlayerMemberAt(iJ));
            }
          }
        }
        if (GET_TEAM((TeamTypes)iI).isAtWar(eTeam))
        {
          for (std::vector<PlayerTypes>::const_iterator iter1 = m_aePlayerMembers.begin();
               iter1 != m_aePlayerMembers.end(); ++iter1)
          {
...
void CvTeam::makePeace(TeamTypes eTeam, bool bBumpUnits)
{
  ...
  if (isAtWar(eTeam))
  {
...

              {
...

                {
                  ...
                  if (GET_PLAYER((PlayerTypes)iJ).getTeam() == getID() ||
                      GET_PLAYER((PlayerTypes)iJ).getTeam() == eTeam)
        ...
        if (GET_PLAYER((PlayerTypes)iI).isAlive() && GET_PLAYER((PlayerTypes)eTeam).isAlive())
        {
          if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
          {
            ...
            gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), true, GC.getEVENT_MESSAGE_TIME(), szBuffer,
                                                       "AS2D_MAKEPEACE", MESSAGE_TYPE_MAJOR_EVENT, NULL,
                                                       (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
          }
          else if (GET_PLAYER((PlayerTypes)iI).getTeam() == eTeam)
          {
                   ...
                   GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(eTeam))
          {
            ...
            szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_MADE_PEACE",
                                     ...
                                     GET_PLAYER((PlayerTypes)eTeam).getCivilizationShortDescription()); //Rhye
            gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer,
                                                       "AS2D_THEIRMAKEPEACE", MESSAGE_TYPE_MAJOR_EVENT, NULL,
                                                       (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
          }
        }
      }
    ...
    if (GC.getGameINLINE().isFinalInitialized())
    {
      ...
      for (std::vector<PlayerTypes>::const_iterator iter1 = m_aePlayerMembers.begin(); iter1 != m_aePlayerMembers.end();
           ++iter1)
      {
        for (iI = 0; iI < GET_TEAM(eTeam).getPlayerMemberListSize(); iI++)
        {
...
bool CvTeam::canVassalRevolt(TeamTypes eMaster) const
{
  ...
  if (GC.getDefineINT("FREE_VASSAL_POPULATION_PERCENT") < 0 ||
      100 * getTotalPopulation(false) <
          kMaster.getTotalPopulation(false) * GC.getDefineINT("FREE_VASSAL_POPULATION_PERCENT"))
  {
    return false;
  }
  ...
  for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
  {
    ...
    if (pLoopPlot != NULL)
    {
      if (pLoopPlot->area() == pArea)
      {
        ...
        {
          iCount += pLoopPlot->plotCount(PUF_canDefendEnemy, getLeaderID(), false, NO_PLAYER, NO_TEAM, PUF_isVisible,
                                         getLeaderID());
        }
      }
    }
...
HandicapTypes CvTeam::getHandicapType() const
{
  ...
  if (iCount > 0)
  {
    FAssertMsg((iGameHandicap / iCount) >= 0,
               "(iGameHandicap / iCount) is expected to be non-negative (invalid Index)");
                ...
                if (GET_PLAYER((PlayerTypes)iI).isAlive())
                {
                  if (GET_PLAYER((PlayerTypes)iI).getTeam() == eIndex)
                  {
                    if (GET_PLAYER(getLeaderID()).canContact((PlayerTypes)iI))
                    {
                      if (GET_PLAYER((PlayerTypes)iI).isHuman())
                      {
                        if (m_abHasEverMet[iI] == false)
                        { //Rhye
                          pDiplo = new CvDiploParameters(getLeaderID());
                          FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
                          pDiplo->setDiploComment(
                              (DiploCommentTypes)GC.getInfoTypeForString("AI_DIPLOCOMMENT_FIRST_CONTACT"));
                          pDiplo->setAIContact(true);
                          gDLL->beginDiplomacy(pDiplo, ((PlayerTypes)iI));
                        } //Rhye
                      }
  ...
  if (isDefensivePact(eIndex) != bNewValue)
  {
    ...
    if (bNewValue && !GET_TEAM(eIndex).isDefensivePact(getID()))
    {
      ...
      CvWString szBuffer = gDLL->getText("TXT_KEY_MISC_PLAYERS_SIGN_DEFENSIVE_PACT",
                                         GET_PLAYER((PlayerTypes)getID()).getCivilizationShortDescription(),
                                         GET_PLAYER((PlayerTypes)eIndex).getCivilizationShortDescription()); //Rhye
      GC.getGameINLINE().addReplayMessage(REPLAY_MESSAGE_MAJOR_EVENT, getLeaderID(), szBuffer, -1, -1,
                                          (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
...
void CvTeam::setVassal(TeamTypes eIndex, bool bNewValue, bool bCapitulated)
{
  ...
  FAssertMsg(!bNewValue || !GET_TEAM(eIndex).isAVassal(), "can't become a vassal of a vassal")
      ...
      //Rhye - start
      if (isMinorCiv() || isBarbarian()) return;
  //Rhye - end
  ...
  if (isVassal(eIndex) != bNewValue)
  {
    ...
    for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
    {
      ...
      {
        int iLoop;
        for (CvUnit *pLoopUnit = kLoopPlayer.firstUnit(&iLoop); NULL != pLoopUnit;
             pLoopUnit = kLoopPlayer.nextUnit(&iLoop))
        {
          CvPlot *pPlot = pLoopUnit->plot();
          if (pLoopUnit->getTeam() != pPlot->getTeam() &&
              (pPlot->getTeam() == NO_TEAM || !GET_TEAM(pPlot->getTeam()).isVassal(pLoopUnit->getTeam())))
          {
    ...
    if (isVassal(eIndex))
    {
      ...
      if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
      {
        ...
        GC.getGameINLINE().addReplayMessage(REPLAY_MESSAGE_MAJOR_EVENT, getLeaderID(), szReplayMessage, -1, -1,
                                            (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
        ...
        for (int iI = 0; iI < MAX_PLAYERS; iI++)
        {
          ...
          if (kPlayer.isAlive())
          {
            ...
            {
              gDLL->getInterfaceIFace()->addHumanMessage(
                  ((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szReplayMessage, "AS2D_WELOVEKING",
    ...
    else
    {
      ...
      if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()) && isAlive() &&
          GET_TEAM(eIndex).isAlive())
      {
        CvWString szReplayMessage;
        ...
        for (int iI = 0; iI < MAX_PLAYERS; iI++)
        {
          ...
          {
            CvWString szBuffer;
            if (getID() == kPlayer.getTeam() || eIndex == kPlayer.getTeam() ||
                isHasMet(kPlayer.getTeam()) && GET_TEAM(eIndex).isHasMet(kPlayer.getTeam()))
            {
...
void CvTeam::freeVassal(TeamTypes eVassal) const
{
       ...
       pLoopDeal = GC.getGameINLINE().nextDeal(&iLoop))
  {
    ...
    if ((GET_PLAYER(pLoopDeal->getFirstPlayer()).getTeam() == eVassal) &&
        (GET_PLAYER(pLoopDeal->getSecondPlayer()).getTeam() == getID()))
    {
    ...
    {
      if ((GET_PLAYER(pLoopDeal->getFirstPlayer()).getTeam() == getID()) &&
          (GET_PLAYER(pLoopDeal->getSecondPlayer()).getTeam() == eVassal))
      {
        if (pLoopDeal->getSecondTrades() != NULL)
        {
          for (pNode = pLoopDeal->getSecondTrades()->head(); pNode; pNode = pLoopDeal->getSecondTrades()->next(pNode))
...
bool CvTeam::isProjectMaxedOut(ProjectTypes eIndex, int iExtra) const
{
  ...
  FAssertMsg(getProjectCount(eIndex) <= GC.getProjectInfo(eIndex).getMaxTeamInstances(),
             "Current Project count is expected to not exceed the maximum number of instances for this project");
...
void CvTeam::changeProjectCount(ProjectTypes eIndex, int iChange)
{
  ...
  if (iChange != 0)
  {
    ...
    FAssertMsg(getProjectCount(eIndex) == (int)m_pavProjectArtTypes[eIndex].size(),
    ...
    // 3MiroProjects: updating new projects effects go here - BEGIN
    CvCity *pOneCapitolCity = NULL;
    //for (iI = 0; iI < MAX_PLAYERS; iI++ ){
    //	if ( GET_PLAYER((PlayerTypes) iI).getTeam() == getID() && pOneCapitolCity == NULL ){
    //		pOneCapitolCity = GET_PLAYER((PlayerTypes) iI).getCapitalCity();
    //	};
    //};
    pOneCapitolCity = GET_PLAYER(getLeaderID()).getCapitalCity();
    if (pOneCapitolCity != NULL)
    {
      //pOneCapitolCity ->plot() ->updatePlotGroupBonus(false);
      for (iI = 0; iI < GC.getNumBonusInfos(); iI++)
      {
    ...
    // 3MiroProjects: updating new projects effects go here - END
    ...
    if (iChange > 0)
    {
      ...
      if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
      {
        ...
        szBuffer = gDLL->getText("TXT_KEY_MISC_COMPLETES_PROJECT",
                                 GET_PLAYER((PlayerTypes)getID()).getCivilizationShortDescription(),
                                 GC.getProjectInfo(eIndex).getTextKeyWide()); //Rhye
        GC.getGameINLINE().addReplayMessage(REPLAY_MESSAGE_MAJOR_EVENT, getLeaderID(), szBuffer, -1, -1,
                                            (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
    ...
    // Absinthe: SoI has an option for losing projects to another player, implemented in this function
    //				might be useful for RFCE colonies too
  }
}
...
bool CvTeam::isUnitClassMaxedOut(UnitClassTypes eIndex, int iExtra) const
{
  ...
  FAssertMsg(
      getUnitClassCount(eIndex) <= GC.getUnitClassInfo(eIndex).getMaxTeamInstances(),
      "The current unit class count is expected not to exceed the maximum number of instances allowed for this team");
...
bool CvTeam::isBuildingClassMaxedOut(BuildingClassTypes eIndex, int iExtra) const
{
  ...
  FAssertMsg(getBuildingClassCount(eIndex) <= GC.getBuildingClassInfo(eIndex).getMaxTeamInstances(),
             "The current building class count is expected not to exceed the maximum number of instances allowed for "
             "this team");
        ...
        if (GET_PLAYER((PlayerTypes)iI).isAlive())
        {
          if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
          {
                 ...
                 pLoopCity = GET_PLAYER((PlayerTypes)iI).nextCity(&iLoop))
            {
              ...
              {
                pLoopCity->processBuilding(eIndex,
                                           ((isObsoleteBuilding(eIndex)) ? -pLoopCity->getNumBuilding(eIndex)
...
int CvTeam::changeResearchProgressPercent(TechTypes eIndex, int iPercent, PlayerTypes ePlayer)
{
  ...
  if (0 != iPercent && !isHasTech(eIndex))
  {
    ...
    {
      iBeakers =
          std::max(getResearchLeft(eIndex) - getResearchCost(eIndex), (getResearchCost(eIndex) * iPercent) / 100);
    }
...
bool CvTeam::isTerrainTrade(TerrainTypes eIndex) const
{
  ...
  //if (getID() == 9 && eIndex == 11) // (Norway ID == 9, Ocean ID == 11)
...
int CvTeam::getVictoryDelay(VictoryTypes eVictory) const
{
  ...
  for (int iProject = 0; iProject < GC.getNumProjectInfos(); ++iProject)
  {
    ...
    {
      iExtraDelayPercent += ((kProject.getVictoryThreshold(eVictory) - iCount) * kProject.getVictoryDelayPercent()) /
...
void CvTeam::resetVictoryProgress()
{
  for (int iI = 0; iI < GC.getNumVictoryInfos(); ++iI)
  {
    if (getVictoryCountdown((VictoryTypes)iI) >= 0 && GC.getGameINLINE().getGameState() == GAMESTATE_ON)
    {
      ...
      CvWString szBuffer = gDLL->getText("TXT_KEY_VICTORY_RESET", getName().GetCString(),
      ...
      for (int iJ = 0; iJ < MAX_PLAYERS; ++iJ)
      {
        ...
        if (kPlayer.isAlive())
        {
          gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iJ), false, GC.getEVENT_MESSAGE_TIME(), szBuffer,
...
void CvTeam::announceTechToPlayers(TechTypes eIndex, bool bPartial)
{
  ...
  for (int iI = 0; iI < MAX_PLAYERS; iI++)
  {
    if (GET_PLAYER((PlayerTypes)iI).isAlive())
    {
      ...
      {
        CvWString szBuffer =
            gDLL->getText((bPartial ? "TXT_KEY_MISC_PROGRESS_TOWARDS_TECH" : "TXT_KEY_MISC_YOU_DISCOVERED_TECH"),
                          GC.getTechInfo(eIndex).getTextKeyWide());

        gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), false, (bSound ? GC.getEVENT_MESSAGE_TIME() : -1),
                                                   szBuffer, (bSound ? GC.getTechInfo(eIndex).getSoundMP() : NULL),
              ...
              if (GC.getReligionInfo((ReligionTypes)iI).getTechPrereq() == eIndex)
              {
                ...
                for (iJ = 0; iJ < MAX_PLAYERS; iJ++)
                {
                  if (GET_PLAYER((PlayerTypes)iJ).isAlive())
                  {
                    ...
                    if ((iI == iSchismReligion) &&
                        !((GET_PLAYER((PlayerTypes)iJ).getStateReligion() == iSchismReligion) ||
              ...
              if (GET_PLAYER((PlayerTypes)iI).isAlive())
              {
                if (isHasMet(GET_PLAYER((PlayerTypes)iI).getTeam()))
                {
                  ...
                  szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_FIRST_TO_TECH",
                                           GET_PLAYER(ePlayer).getCivilizationDescriptionKey(),
                                           GC.getTechInfo(eIndex).getTextKeyWide()); // Absinthe
                }
                else
                {
                  szBuffer =
                      gDLL->getText("TXT_KEY_MISC_UNKNOWN_FIRST_TO_TECH", GC.getTechInfo(eIndex).getTextKeyWide());
                }
                gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(),
                                                           szBuffer, "AS2D_FIRSTTOTECH", MESSAGE_TYPE_MAJOR_EVENT, NULL,
                                                           (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
              }
            }

            szBuffer = gDLL->getText("TXT_KEY_MISC_SOMEONE_FIRST_TO_TECH", GET_PLAYER(ePlayer).getName(),
                                     GC.getTechInfo(eIndex).getTextKeyWide());
            GC.getGameINLINE().addReplayMessage(REPLAY_MESSAGE_MAJOR_EVENT, ePlayer, szBuffer, -1, -1,
                                                (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
          }
            ...
            if (pLoopPlot->getTeam() == getID())
            {
              ...
              if (eBonus != NO_BONUS)
              {
                if (GC.getBonusInfo(eBonus).getTechReveal() == eIndex && !isForceRevealedBonus(eBonus))
                {
                  ...
                  if (pCity != NULL)
                  {
                    szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_DISCOVERED_BONUS",
                                             GC.getBonusInfo(eBonus).getTextKeyWide(), pCity->getNameKey());
                    gDLL->getInterfaceIFace()->addHumanMessage(
                        pLoopPlot->getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_DISCOVERBONUS",
                        MESSAGE_TYPE_INFO, GC.getBonusInfo(eBonus).getButton(),
                        (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), pLoopPlot->getX_INLINE(),
          ...
          for (iI = 0; iI < MAX_PLAYERS; iI++)
          {
            if (GET_PLAYER((PlayerTypes)iI).isAlive())
            {
              if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
              {
                if (GET_PLAYER((PlayerTypes)iI).isHuman())
                {
                  if (!bReligionFounded || (GET_PLAYER((PlayerTypes)iI).getLastStateReligion() != NO_RELIGION) ||
                      (iI != ePlayer))
                  {
...
void CvTeam::doWarWeariness()
{
  ...
  for (iI = 0; iI < MAX_TEAMS; iI++)
  {
    if (getWarWeariness((TeamTypes)iI) > 0)
    {
      ...
      {
        setWarWeariness(((TeamTypes)iI),
                        ((getWarWeariness((TeamTypes)iI) * GC.getDefineINT("WW_DECAY_PEACE_PERCENT")) / 100));
      }
    }
  }
}
    ...
    if (GC.getDefineINT("CIRCUMNAVIGATE_FREE_MOVES") != 0)
    {
      ...
      for (int iI = 0; iI < MAX_PLAYERS; iI++)
      {
        if (GET_PLAYER((PlayerTypes)iI).isAlive())
        {
          ...
          gDLL->getInterfaceIFace()->addHumanMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer,
                                                     "AS2D_GLOBECIRCUMNAVIGATED", MESSAGE_TYPE_MAJOR_EVENT, NULL,
                                                     (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
        }
      }
...
void CvTeam::processTech(TechTypes eTech, int iChange)
{
  ...
  for (iI = 0; iI < GC.getNumBuildingInfos(); iI++)
  {
    ...
    {
      if (GC.getSpecialBuildingInfo(
                (SpecialBuildingTypes)GC.getBuildingInfo((BuildingTypes)iI).getSpecialBuildingType())
  ...
  for (iI = 0; iI < GC.getNumImprovementInfos(); iI++)
  {
    for (iJ = 0; iJ < NUM_YIELD_TYPES; iJ++)
    {
      changeImprovementYieldChange(
          ((ImprovementTypes)iI), ((YieldTypes)iJ),
          (GC.getImprovementInfo((ImprovementTypes)iI).getTechYieldChanges(eTech, iJ) * iChange));
    }
  }
  ...
  for (iI = 0; iI < MAX_PLAYERS; iI++)
  {
    if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
    {
      GET_PLAYER((PlayerTypes)iI)
          .changeFeatureProductionModifier(GC.getTechInfo(eTech).getFeatureProductionModifier() * iChange);
...
void CvTeam::read(FDataStreamBase *pStream)
{
  ...
  pStream->Read(GC.getNumBonusInfos(), m_paiFreeBonus); // 3MiroProjects: save game
...
void CvTeam::write(FDataStreamBase *pStream)
{
  ...
  pStream->Write(GC.getNumBonusInfos(), m_paiFreeBonus); // 3MiroProjects: save game
...
// 3MiroProjects
int CvTeam::getFreeBonus(BonusTypes eIndex) const
{
  FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
...
void CvTeam::changeFreeBonus(BonusTypes eIndex, int iChange) const
{
  FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
  FAssertMsg(eIndex < GC.getNumBonusInfos(), "eIndex is expected to be within maximum bounds (invalid Index)");
...
void CvTeam::updateFreeBonusCity(CvCity *pCity, bool bAdd)
{
  int iI, iChange;
  if (bAdd)
  {
  ...
  for (iI = 0; iI < GC.getNumBonusInfos(); iI++)
  {
    pCity->changeFreeBonus((BonusTypes)iI, iChange * getFreeBonus((BonusTypes)iI));
  };
};

bool CvTeam::isTrainVassalUU() const
{
  return (m_iVassalUU > 0);
};
int CvTeam::getTrainVassalUU() const
{
  return m_iVassalUU;
};
