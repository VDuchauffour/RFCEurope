...
public:
  ...
  void setHasTech(TechTypes eIndex, bool bNewValue, PlayerTypes ePlayer, bool bFirst,
                  bool bAnnounce); // Exposed to Python
  ...
  void changeFreeBonus(BonusTypes eIndex, int iChange) const;
  void updateFreeBonusCity(CvCity *pCity, bool bAdd);

  // 3Miro: Topkapi, VassalUU
...
protected:
  ...
  int *m_paiFreeBonus; // 3MiroProjects
