...
AreaAITypes CvTeamAI::AI_calculateAreaAIType(CvArea *pArea, bool bPreparingTotal) const
{
  ...
  if (!(pArea->isWater()))
  {
    if (isBarbarian())
    {
      ...
      if ((countNumAIUnitsByArea(pArea, UNITAI_ATTACK) + countNumAIUnitsByArea(pArea, UNITAI_ATTACK_CITY) +
           countNumAIUnitsByArea(pArea, UNITAI_PILLAGE) + countNumAIUnitsByArea(pArea, UNITAI_ATTACK_AIR)) >
        ...
        if (AI_getWarPlan((TeamTypes)iI) != NO_WARPLAN)
        {
              ...
              (GET_TEAM((TeamTypes)iI).countNumUnitsByArea(pArea) > 4))
          {
            ...
            if (AI_isChosenWar((TeamTypes)iI))
            {
              ...
              if ((isAtWar((TeamTypes)iI)) ? (AI_getAtWarCounter((TeamTypes)iI) < 10)
    ...
    if (bTargets)
    {
      if (iAreaCities > (getNumMembers() * 3))
      {
        if (GC.getGameINLINE().isOption(GAMEOPTION_AGGRESSIVE_AI) ||
            ...
            (countPowerByArea(pArea) > ((countEnemyPowerByArea(pArea) * 3) / 2)))
        {
          return AREAAI_MASSING;
        }
      }
...
int CvTeamAI::AI_calculateCapitalProximity(TeamTypes eTeam) const
{
          ...
          for (iJ = 0; iJ < MAX_PLAYERS; iJ++)
          {
            if (GET_PLAYER((PlayerTypes)iJ).isAlive())
            {
              if (GET_PLAYER((PlayerTypes)iJ).getTeam() != getID())
              {
                ...
                {
                  int iDistance = (plotDistance(pOurCapitalCity->getX_INLINE(), pOurCapitalCity->getY_INLINE(),
                                                pTheirCapitalCity->getX_INLINE(), pTheirCapitalCity->getY_INLINE()) *
  ...
  if (iCount > 0)
  {
    ...
    return ((GC.getMapINLINE().maxPlotDistance() * (iMaxDistance - ((iTotalDistance / iCount) - iMinDistance))) /
            iMaxDistance);
  }
...
bool CvTeamAI::AI_isAllyLandTarget(TeamTypes eTeam) const
{
  for (int iTeam = 0; iTeam < MAX_CIV_TEAMS; iTeam++)
  {
    ...
    if (iTeam != getID())
    {
      if (iTeam == eTeam || kLoopTeam.isVassal(eTeam) || GET_TEAM(eTeam).isVassal((TeamTypes)iTeam) ||
          kLoopTeam.isDefensivePact(eTeam))
      {
  ...
  for (int iI = 0; iI < GC.getNumVictoryInfos(); iI++)
  {
    if (GC.getVictoryInfo((VictoryTypes)iI).getLandPercent() > 0)
    {
      iPercentOfDomination =
          100 * iOurLandPercent / std::max(1, GC.getGameINLINE().getAdjustedLandPercent((VictoryTypes)iI));
    }
  }
...
int CvTeamAI::AI_endWarVal(TeamTypes eTeam) const
{
      ...
      (!(GET_TEAM(eTeam).isHuman()) && (GET_TEAM(eTeam).AI_getWarPlan(getID()) == WARPLAN_TOTAL)))
  {
    iValue *= 2;
           ...
           (!(GET_TEAM(eTeam).isHuman()) && (GET_TEAM(eTeam).AI_getWarPlan(getID()) == WARPLAN_DOGPILE) &&
            (getAtWarCount(true) > 1)))
  {
    iValue *= 3;
    iValue /= 2;
  }
  ...
  if (GC.getGameINLINE().isOption(GAMEOPTION_AGGRESSIVE_AI))
  {
    ...
    for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; iPlayer++)
    {
      ...
      if (kPlayer.getTeam() == eTeam)
      {
        ...
        for (pTheirLoopCity = kPlayer.firstCity(&iLoop); pTheirLoopCity != NULL;
             pTheirLoopCity = kPlayer.nextCity(&iLoop))
        {
...
DenialTypes CvTeamAI::AI_techTrade(TechTypes eTech, TeamTypes eTeam) const
{
  ...
  if (eAttitude < ATTITUDE_FRIENDLY)
  {
        ...
        (GC.getGameINLINE().getTeamRank(eTeam) < (GC.getGameINLINE().countCivTeamsEverAlive() / 2)))
    {
      ...
      iNoTechTradeThreshold *=
          std::max(0, (GC.getHandicapInfo(GET_TEAM(eTeam).getHandicapType()).getNoTechTradeModifier() + 100));
    ...
    iTechTradeKnownPercent *=
        std::max(0, (GC.getHandicapInfo(GET_TEAM(eTeam).getHandicapType()).getTechTradeKnownModifier() + 100));
    iTechTradeKnownPercent /= 100;

    iTechTradeKnownPercent *= AI_getTechMonopolyValue(eTech, eTeam);
    iTechTradeKnownPercent /= 100;

    if ((iPossibleKnownCount > 0) ? (((iKnownCount * 100) / iPossibleKnownCount) < iTechTradeKnownPercent)
  ...
  for (iI = 0; iI < GC.getNumBuildingInfos(); iI++)
  {
    if (isTechRequiredForBuilding(eTech, ((BuildingTypes)iI)))
    {
      ...
      {
        if (getBuildingClassMaking((BuildingClassTypes)(GC.getBuildingInfo((BuildingTypes)iI).getBuildingClassType())) >
            0)
        {
          return DENIAL_MYSTERY;
        }
...
DenialTypes CvTeamAI::AI_surrenderTrade(TeamTypes eTeam, int iPowerMultiplier) const
{
	...
	*/
          ...
          if (kLoopTeam.AI_isLandTarget(getID()))
          {
            if (iLoopTeam != eTeam)
            {
              if (kLoopTeam.getPower(true) > getPower(true))
              {
                ...
                iAveragePower = (2 * iAveragePower * kLoopTeam.getPower(true)) /
                                std::max(1, kLoopTeam.getPower(true) + getPower(true));
              ...
              {
                iAveragePower =
                    (iAveragePower * (getPower(true) + GET_TEAM(eTeam).getPower(false))) / std::max(1, getPower(true));
              }
            }
          }
    ...
    for (int i = 0; i < GC.getNumVictoryInfos(); i++)
    {
      ...
      if (GC.getGameINLINE().getAdjustedPopulationPercent((VictoryTypes)i) > 0)
      {
        ...
        int iThreshold =
            GC.getGameINLINE().getTotalPopulation() * GC.getGameINLINE().getAdjustedPopulationPercent((VictoryTypes)i);
      ...
      if (GC.getGameINLINE().getAdjustedPopulationPercent((VictoryTypes)i) > 0 ||
          GC.getGameINLINE().getAdjustedLandPercent((VictoryTypes)i) > 0)
      {
  ...
  if (!isAtWar(eTeam))
  {
    ...
    AttitudeTypes eModifiedAttitude =
        CvPlayerAI::AI_getAttitudeFromValue(AI_getAttitudeVal(eTeam, false) + iAttitudeModifier);
         ...
         ++iter)
    {
      ...
      if (eModifiedAttitude <=
          GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getVassalRefuseAttitudeThreshold())
      {
        return DENIAL_ATTITUDE;
      }
    }
  ...
  {
    if (AI_getWarSuccess(eTeam) + 4 * GC.getDefineINT("WAR_SUCCESS_CITY_CAPTURING") >
...
DenialTypes CvTeamAI::AI_declareWarTrade(TeamTypes eWarTeam, TeamTypes eTeam, bool bConsiderPower) const
{
  ...
  {
    if (eAttitude <=
        GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getDeclareWarRefuseAttitudeThreshold())
    {
      return DENIAL_ATTITUDE;
    }
  }
  ...
  {
    if (eAttitudeThem >
        GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getDeclareWarThemRefuseAttitudeThreshold())
    {
      return DENIAL_ATTITUDE_THEM;
    }
  }
...
{
  // 3MiroPapal: OB with the Pope is less likely unless you share the Pope's religion
  if (GET_TEAM(eTeam).getLeaderID() == PAPAL_PLAYER)
  {
    if (GET_PLAYER(getLeaderID()).getStateReligion() == PAPAL_RELIGION)
    {
      return 2 * getNumCities();
    }
    else
    {
      return 0;
  ...
  if (getLeaderID() == PAPAL_PLAYER)
  {
    return 2 * GET_TEAM(eTeam).getNumCities();
  };
...
DenialTypes CvTeamAI::AI_openBordersTrade(TeamTypes eTeam) const
{
  ...
  {
    if (eAttitude <=
        GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getOpenBordersRefuseAttitudeThreshold())
    {
      return DENIAL_ATTITUDE;
    }
  }
...
int CvTeamAI::AI_defensivePactTradeVal(TeamTypes eTeam) const
{
  ...
  //if (GC.getGameINLINE().getGameTurn() > 380) { //the last 100 turns, starting from 1920
  //	modifier = 160;
  //}

  // 3Miro: AI less likely to trade defensive pact if from different religions
  if (GET_PLAYER(getLeaderID()).getStateReligion() != GET_PLAYER(GET_TEAM(eTeam).getLeaderID()).getStateReligion())
  {
    modifier += 280;
  };
...
DenialTypes CvTeamAI::AI_defensivePactTrade(TeamTypes eTeam) const
{
  ...
  if (iMaxEra >= 5 && iGameTurn > 390) //modern && year > 1940
    iCap = 4 + GC.getGameINLINE().countCivPlayersAlive() / 17 +
           (bTech ? 1 : 0);                 //16 Warlords, 13 vanilla (higher because of the addition of bTech)
  ...
  {
    if (eAttitude <=
        GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getDefensivePactRefuseAttitudeThreshold())
    {
      return DENIAL_ATTITUDE;
    }
  }
...
DenialTypes CvTeamAI::AI_permanentAllianceTrade(TeamTypes eTeam) const
{
  ...
  {
    if (eAttitude <=
        GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getPermanentAllianceRefuseAttitudeThreshold())
    {
      return DENIAL_ATTITUDE;
    }
  }
...
void CvTeamAI::AI_doWar()
{
  ...
  for (std::vector<PlayerTypes>::const_iterator iter = m_aePlayerMembers.begin(); iter != m_aePlayerMembers.end();
       ++iter)
  {
    int iUnitSpendingPercent =
        (GET_PLAYER(*iter).calculateUnitCost() * 100) / std::max(1, GET_PLAYER(*iter).calculatePreInflatedCosts());
    iHighUnitSpendingPercent += (std::max(0, iUnitSpendingPercent - 7) / 2);
    iLowUnitSpendingPercent += iUnitSpendingPercent;
  }
  // Sanguo Mod Performance, end
                ...
                if (isAtWar((TeamTypes)iI))
                {
                  if (AI_isChosenWar((TeamTypes)iI))
                  {
                    if (AI_getAtWarCounter((TeamTypes)iI) > 20)
                    {
                      ...
                      if (AI_getAtWarCounter((TeamTypes)iI) >
                          ((AI_getWarPlan((TeamTypes)iI) == WARPLAN_TOTAL) ? 40 : 30))
                      {
  ...
  else if (getAnyWarPlanCount(true) == 0)
  {
    ...
    if (iGetBetterUnitsCount * 3 < iNumMembers * 2)
    {
      if (bFinancialProWar || !bFinancesOpposeWar)
      {
        ...
        if (GC.getGameINLINE().getSorenRandNum(100, "AI Declare War 1") <
            GC.getHandicapInfo(GC.getGameINLINE().getHandicapType()).getAIDeclareWarProb())
        {
          // if non-agressive, random based on number of cities (the more cities, the less war)
          if ((bAggressive || bFinancialProWar) ? true
                                                : (GC.getGameINLINE().getSorenRandNum(4, "AI Declare War 2") != 0))
          {
            bMakeWarChecks = true;
          }
        }
                ...
                if (isHasMet((TeamTypes)iI))
                {
                  if (canDeclareWar((TeamTypes)iI))
                  {
                    if (iNoWarRoll >= AI_noWarAttitudeProb(AI_getAttitude((TeamTypes)iI)))
                    {
                      ...
                      if (iDefensivePower <
                          ((iOurPower * ((iPass > 1) ? AI_maxWarDistantPowerRatio() : AI_maxWarNearbyPowerRatio())) /
                           100))
                      {
                        // XXX make sure they share an area....

                        FAssertMsg(!(GET_TEAM((TeamTypes)iI).isBarbarian()),
                                   "Expected to not be declaring war on the barb civ");
                        ...
                        {
                          if ((iPass > 0) || (AI_calculateAdjacentLandPlots((TeamTypes)iI) >=
                ...
                if (canDeclareWar((TeamTypes)iI))
                {
                  if (iNoWarRoll >= AI_noWarAttitudeProb(AI_getAttitude((TeamTypes)iI)))
                  {
                    if (AI_isLandTarget((TeamTypes)iI) ||
                        (AI_isAnyCapitalAreaAlone() && GET_TEAM((TeamTypes)iI).AI_isAnyCapitalAreaAlone()))
                    {
                  ...
                  if (iNoWarRoll >= AI_noWarAttitudeProb(AI_getAttitude((TeamTypes)iI)))
                  {
                    if (GET_TEAM((TeamTypes)iI).getAtWarCount(true) > 0)
                    {
                      if (AI_isLandTarget((TeamTypes)iI))
                      {
                        ...
                        FAssert(GET_TEAM((TeamTypes)iI).getPower(true) == GET_TEAM((TeamTypes)iI).getDefensivePower() ||
    ...
    if (eLoopUnit != NO_UNIT)
    {
      if (isTechRequiredForUnit((eTech), eLoopUnit))
      {
        ...
        if (GC.getUnitInfo(eLoopUnit).getPrereqAndTech() == eTech)
        {
          ...
          int iCombatRatio =
              (GC.getUnitInfo(eLoopUnit).getCombat() * 100) / std::max(1, GC.getGameINLINE().getBestLandUnitCombat());
          if (iCombatRatio > 50)
          {
