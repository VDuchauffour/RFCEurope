...
//
//------------------------------------------------------------------------------------------------------
CvXMLLoadUtility::CvXMLLoadUtility() : m_iCurProgressStep(0), m_pCBFxn(NULL), m_pFXml(NULL)
{
...
void CvXMLLoadUtility::UpdateProgressCB(const char *szMessage)
{
  ...
  if (m_pCBFxn)
  {
    m_pCBFxn(++m_iCurProgressStep, GetNumProgressSteps(),
             CvString::format("Reading XML %s", szMessage ? szMessage : "").c_str());
  }
}
