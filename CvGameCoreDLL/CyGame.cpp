...
void CyGame::selectionListGameNetMessage(int eMessage, int iData2, int iData3, int iData4, int iFlags, bool bAlt,
                                         bool bShift)
{
  GC.getGameINLINE().selectionListGameNetMessage(eMessage, iData2, iData3, iData4, iFlags, bAlt, bShift);
}

void CyGame::selectedCitiesGameNetMessage(int eMessage, int iData2, int iData3, int iData4, bool bOption, bool bAlt,
                                          bool bShift, bool bCtrl)
{
...
bool CyGame::changePlayer(int playerIdx, int newCivType, int newLeader, int teamIdx, bool bIsHuman,
...
#ifdef _BUFFY
int CyGame::checkCRCs(std::string fileName_, std::string expectedModCRC_, std::string expectedDLLCRC_,
                      std::string expectedShaderCRC_, std::string expectedPythonCRC_, std::string expectedXMLCRC_) const
{
  return NULL != m_pGame ? m_pGame->checkCRCs(fileName_, expectedModCRC_, expectedDLLCRC_, expectedShaderCRC_,
                                              expectedPythonCRC_, expectedXMLCRC_)
                         : -1;
}
...
{
  return m_pGame->isTopCultureCity(x, y);
};

int CyGame::doesOwnCities(int iCiv, int BLx, int BLy, int TRx, int TRy)
{
  return m_pGame->doesOwnCities(iCiv, BLx, BLy, TRx, TRy);
};

int CyGame::doesOwnOrVassalCities(int iCiv, int BLx, int BLy, int TRx, int TRy)
{
  return m_pGame->doesOwnOrVassalCities(iCiv, BLx, BLy, TRx, TRy);
};

bool CyGame::doesHaveOtherReligion(int BLx, int BLy, int TRx, int TRy, int AllowR)
{
  return m_pGame->doesHaveOtherReligion(BLx, BLy, TRx, TRy, AllowR);
};

int CyGame::countOwnedCities(int iCiv, int BLx, int BLy, int TRx, int TRy)
{
  return m_pGame->countOwnedCities(iCiv, BLx, BLy, TRx, TRy);
};

bool CyGame::canSeeAllTerrain(int iCiv, int iTerrain)
{
  return m_pGame->canSeeAllTerrain(iCiv, iTerrain);
};

bool CyGame::controlMostTeritory(int iCiv, int BLx, int BLy, int TRx, int TRy)
{
  return m_pGame->controlMostTeritory(iCiv, BLx, BLy, TRx, TRy);
};

int CyGame::countCitiesLostTo(int iCiv, int iNewOwner)
{
  return m_pGame->countCitiesLostTo(iCiv, iNewOwner);
};

bool CyGame::safeMotherland(int iCiv)
{
...
int CyGame::cityStabilityPenalty( int iPlayer, int iAnger, int iHealth, int iReligion, int iLarge, int iHurry, int iNoMilitary, int iWarW, int iFReligion, int iFCulture, int iPerCityCap ){
	return m_pGame ->cityStabilityPenalty( iPlayer, iAnger, iHealth, iReligion, iLarge, iHurry, iNoMilitary, iWarW, iFReligion, iFCulture, iPerCityCap );
};

void CyGame::calcLastOwned(){
	m_pGame ->calcLastOwned();
};
