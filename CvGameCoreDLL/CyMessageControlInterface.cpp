...
void CyMessageControlInterface()
{
  python::class_<CyMessageControl>("CyMessageControl")
      ...
      .def("sendAdvancedStartAction", &CyMessageControl::sendAdvancedStartAction,
           "void (int /*AdvancedStartActionTypes*/ eAction, int /*PlayerTypes*/ ePlayer, int iX, int iY, int iData, "
           "bool bAdd)")
      .def("sendModNetMessage", &CyMessageControl::sendModNetMessage,
           "void (int iData1, int iData2, int iData3, int iData4, int iData5) - This is a NetMessage designed "
           "specifically for modders to use to make their mods Multiplayer friendly, eliminating Out-of-Sync errors. "
           "Check out 'onModNetMessage()' in CvEventManager for the callback")
