...
{
  return m_pPlayer ? m_pPlayer->isProductionMaxedBuildingClass((BuildingClassTypes)eBuildingClass, bAcquireCity)
                   : false;
}
...
int CyPlayer::getBuildingClassPrereqBuilding(int /*BuildingTypes*/ eBuilding,
                                             int /*BuildingClassTypes*/ ePrereqBuildingClass, int iExtra)
{
  return m_pPlayer ? m_pPlayer->getBuildingClassPrereqBuilding((BuildingTypes)eBuilding,
                                                               (BuildingClassTypes)ePrereqBuildingClass, iExtra)
                   : -1;
}
...
{
  return m_pPlayer
             ? m_pPlayer->getAdvancedStartUnitCost((UnitTypes)eUnit, bAdd, NULL != pPlot ? pPlot->getPlot() : NULL)
             : -1;
}
...
{
  return m_pPlayer ? m_pPlayer->getAdvancedStartImprovementCost((ImprovementTypes)eImprovement, bAdd,
                                                                NULL != pPlot ? pPlot->getPlot() : NULL)
                   : -1;
}
...
{
  return m_pPlayer
             ? m_pPlayer->getAdvancedStartRouteCost((RouteTypes)eRoute, bAdd, NULL != pPlot ? pPlot->getPlot() : NULL)
             : -1;
}
...
bool CyPlayer::canDoEspionageMission(int /*EspionageMissionTypes*/ eMission, int /*PlayerTypes*/ eTargetPlayer,
                                     CyPlot *pPlot, int iExtraData)
{
  return m_pPlayer ? m_pPlayer->canDoEspionageMission((EspionageMissionTypes)eMission, (PlayerTypes)eTargetPlayer,
                                                      NULL != pPlot ? pPlot->getPlot() : NULL, iExtraData, NULL)
                   : false;
}

int CyPlayer::getEspionageMissionCost(int /*EspionageMissionTypes*/ eMission, int /*PlayerTypes*/ eTargetPlayer,
                                      CyPlot *pPlot, int iExtraData)
{
  return m_pPlayer ? m_pPlayer->getEspionageMissionCost((EspionageMissionTypes)eMission, (PlayerTypes)eTargetPlayer,
                                                        NULL != pPlot ? pPlot->getPlot() : NULL, iExtraData)
                   : -1;
}

void CyPlayer::doEspionageMission(int /*EspionageMissionTypes*/ eMission, int /*PlayerTypes*/ eTargetPlayer,
                                  CyPlot *pPlot, int iExtraData, CyUnit *pUnit)
{
  if (m_pPlayer)
    m_pPlayer->doEspionageMission((EspionageMissionTypes)eMission, (PlayerTypes)eTargetPlayer,
                                  NULL != pPlot ? pPlot->getPlot() : NULL, iExtraData, pUnit->getUnit());
}
...
void CyPlayer::createGreatPeople(int eGreatPersonUnit, bool bIncrementThreshold, bool bIncrementExperience, int iX,
                                 int iY)
{
  if (m_pPlayer)
  {
...
EventTriggeredData *CyPlayer::initTriggeredData(int /*EventTriggerTypes*/ eEventTrigger, bool bFire, int iCityId,
                                                int iPlotX, int iPlotY, int /*PlayerTypes*/ eOtherPlayer,
                                                int iOtherPlayerCityId, int /*ReligionTypes*/ eReligion,
                                                int /*CorporationTypes*/ eCorporation, int iUnitId,
                                                int /*BuildingTypes*/ eBuilding)
{
  return m_pPlayer
             ? m_pPlayer->initTriggeredData((EventTriggerTypes)eEventTrigger, bFire, iCityId, iPlotX, iPlotY,
                                            (PlayerTypes)eOtherPlayer, iOtherPlayerCityId, (ReligionTypes)eReligion,
                                            (CorporationTypes)eCorporation, iUnitId, (BuildingTypes)eBuilding)
             : NULL;
}
...
{
  return m_pPlayer->getFinancialPower();
};

int CyPlayer::getVotingPower(int iReligion)
{
  return m_pPlayer->getVotingPower((ReligionTypes)iReligion);
};

void CyPlayer::setIsCrusader(bool bVal)
{
  m_pPlayer->setIsCrusader(bVal);
};

bool CyPlayer::getIsCrusader()
{
  return m_pPlayer->getIsCrusader();
};

int CyPlayer::getFaith()
{
  return m_pPlayer->getFaith();
};

void CyPlayer::setFaith(int iNewFaith)
{
  return m_pPlayer->setFaith(iNewFaith);
};

void CyPlayer::changeFaith(int iChange)
{
  return m_pPlayer->changeFaith(iChange);
};

int CyPlayer::getProsecutionCount()
{
  return m_pPlayer->getProsecutionCount();
};
void CyPlayer::setProsecutionCount(int iNewCount)
{
  m_pPlayer->setProsecutionCount(iNewCount);
};
void CyPlayer::changeProsecutionCount(int iChange)
{
  m_pPlayer->changeProsecutionCount(iChange);
};

/*int CyPlayer::getFaithStability(){
	return m_pPlayer ->getFaithStability();
};*/

int CyPlayer::getFaithBenefit(int iFaithPower)
{
  return m_pPlayer->getFaithBenefit(iFaithPower);
};
