...
void CyStructsPythonInterface1()
{
  ...
  python::class_<POINT>("POINT").def_readwrite("x", &POINT::x).def_readwrite("y", &POINT::y);
  ...
  python::class_<IDInfo>("IDInfo").def_readwrite("eOwner", &IDInfo::eOwner).def_readwrite("iID", &IDInfo::iID);
