...
INT_PTR CALLBACK AssertDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_INITDIALOG:
  {
    ...
    sprintf(g_AssertText,
            ...
            "\r\n"
            "----------------------------------------------------------\r\n",
            g_AssertInfo.szFileName, g_AssertInfo.line, g_AssertInfo.szExpression,
...
DWORD DisplayAssertDialog()
{
  CDialogTemplate dialogTemplate(
      _T("Assert Failed!"), DS_SETFONT | DS_CENTER | DS_MODALFRAME | DS_FIXEDSYS | WS_POPUP | WS_CAPTION | WS_SYSMENU,
      0, 0, 379, 166, _T("MS Shell Dlg"), 8);

  dialogTemplate.AddButton(_T("Ignore Always"), WS_VISIBLE, 0, 157, 145, 64, 14, IDC_IGNORE_ALWAYS);

  dialogTemplate.AddButton(_T("&Ignore Once"), WS_VISIBLE, 0, 82, 145, 64, 14, IDC_IGNORE_ONCE);

  dialogTemplate.AddButton(_T("&Debug"), WS_VISIBLE, 0, 307, 145, 64, 14, IDC_DEBUG);

  dialogTemplate.AddButton(_T("&Abort"), WS_VISIBLE, 0, 232, 145, 64, 14, IDC_ABORT);

  dialogTemplate.AddEditBox(
      _T(""), ES_MULTILINE | ES_AUTOVSCROLL | ES_AUTOHSCROLL | ES_READONLY | WS_VSCROLL | WS_HSCROLL | WS_VISIBLE,
      WS_EX_STATICEDGE, 7, 7, 365, 130, IDC_ASSERTION_TEXT);
