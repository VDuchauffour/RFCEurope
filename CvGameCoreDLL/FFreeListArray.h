...
template <class T> class FFreeListArray : public FFreeListArrayBase<T>
{
public:
  FFreeListArray();
...
template <class T> FFreeListArray<T>::FFreeListArray()
{
...
template <class T> void FFreeListArray<T>::init(int iNumSlots)
{
  int iI;
...
template <class T> void FFreeListArray<T>::uninit()
{
  if (m_pArray != NULL)
  {
    removeAll();
...
template <class T> void FFreeListArray<T>::insert(T data)
{
  int iIndex;
...
template <class T> void FFreeListArray<T>::insertAt(T data, int iIndex)
{
  int iTempIndex;
...
template <class T> T *FFreeListArray<T>::getAt(int iIndex)
{
...
template <class T> int FFreeListArray<T>::getIndex(T data)
{
  int iI;
...
template <class T> bool FFreeListArray<T>::removeAt(int iIndex)
{
  assert(m_pArray != NULL);
...
template <class T> void FFreeListArray<T>::removeAll()
{
  int iI;
...
template <class T> void FFreeListArray<T>::growArray()
{
...
//
template <class T> inline void FFreeListArray<T>::Read(FDataStreamBase *pStream)
{
  int iCount = 0;
  pStream->Read(&iCount);
  init(iCount);
...
template <class T> inline void FFreeListArray<T>::Write(FDataStreamBase *pStream)
{
  int iCount = getCount();
...
//
template <class T> inline void ReadStreamableFFreeListArray(FFreeListArray<T> &flist, FDataStreamBase *pStream)
{
  int iCount = 0;
  pStream->Read(&iCount);
  flist.init(iCount);
...
// use when list contains streamable types
//
template <class T> inline void WriteStreamableFFreeListArray(FFreeListArray<T> &flist, FDataStreamBase *pStream)
{
