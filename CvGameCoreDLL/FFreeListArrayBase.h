...
template <class T> class FFreeListArrayBase
{
public:
  FFreeListArrayBase();
  ...
  int getCount() const
  {
    return m_iLastIndex - m_iFreeListCount + 1;
  }
...
template <class T> FFreeListArrayBase<T>::FFreeListArrayBase()
{
...
template <class T> inline T *FFreeListArrayBase<T>::operator[](int iIndex) const
{
  return (getAt(iIndex));
}
...
// start at the beginning of the list and return the first item or NULL when done
template <class T> T *FFreeListArrayBase<T>::beginIter(int *pIterIdx) const
{
  *pIterIdx = 0;
...
// iterate from the current position and return the next item found or NULL when done
template <class T> T *FFreeListArrayBase<T>::nextIter(int *pIterIdx) const
{
...
// start at the end of the list and return the last item or NULL when done
template <class T> T *FFreeListArrayBase<T>::endIter(int *pIterIdx) const
{
...
// iterate from the current position and return the prev item found or NULL when done
template <class T> T *FFreeListArrayBase<T>::prevIter(int *pIterIdx) const
{
