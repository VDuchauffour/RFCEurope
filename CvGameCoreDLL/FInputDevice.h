...
private:
  ...
  struct compStruct
  {
    bool operator()(const InputType s1, const InputType s2) const
    {
      return (s1 < s2);
    }
  };
...
public:
  ...
  struct InputValue
  {
    ...
    InputValue(int iState = UNINIT, int px = -1, int py = -1) : m_iState(iState)
    {
      m_point.x = px;
      m_point.y = py;
    }
    bool operator==(const InputValue &in) const
    {
      return in.m_iState == m_iState && in.m_point.x == m_point.x && in.m_point.y == m_point.y;
    }
  };
  ...
  {
    return m_ulFlags;
  } // init flags should be passed in to Init()

  // access the map of buffered input events
  const InputEventsList &GetInputEvents() const
  {
    return m_InputEvents;
  } // return list of input events
  int GetNumInputEvents() const
  {
    return (int)m_InputEvents.size();
  }
  const InputEvent &GetInputEvent(int i)
  {
    return m_InputEvents[i];
  }
