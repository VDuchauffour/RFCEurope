...
class Stability:
	...
	def setup(self): # Sets starting stability
		...
		for iPlayer in range(iNumMajorPlayers):
			...
			for pCity in utils.getCityList(iPlayer):
				...
				if pCity.hasBuilding(utils.getUniqueBuilding(iPlayer, xml.iManorHouse)) and gc.getGame().getSorenRandNum(10, 'build stab chance') < 7:
					pPlayer.changeStabilityBase( iCathegoryEconomy, 1 )
				if pCity.hasBuilding(utils.getUniqueBuilding(iPlayer, xml.iCastle)) and gc.getGame().getSorenRandNum(10, 'build stab chance') < 7:
					pPlayer.changeStabilityBase( iCathegoryExpansion, 1 )
				if pCity.hasBuilding(utils.getUniqueBuilding(iPlayer, xml.iNightWatch)) and gc.getGame().getSorenRandNum(10, 'build stab chance') < 7:
					pPlayer.changeStabilityBase( iCathegoryCivics, 1 )
				if pCity.hasBuilding(utils.getUniqueBuilding(iPlayer, xml.iCourthouse)) and gc.getGame().getSorenRandNum(10, 'build stab chance') < 7:
	...
	def onCityAcquired(self, iOwner, playerType, city, bConquest, bTrade):
		...
		if iOwner < iNumPlayers and city.getX() == tCapitals[iOwner][0] and city.getY() == tCapitals[iOwner][1]:
			if iOwner == con.iScotland: #Scotland UP part 2
				pOwner.changeStabilityBase( iCathegoryExpansion, -5 )
				pOwner.setStabilitySwing( pOwner.getStabilitySwing() - 5 )
			elif gc.hasUP(iOwner,con.iUP_Emperor): # If Byzantium loses Constantinople, they should lose all non-core cities
	...
	def onTechAcquired(self, iTech, iPlayer):
		if iTech in [xml.iFeudalism, xml.iGuilds, xml.iGunpowder, xml.iProfessionalArmy, xml.iNationalism, xml.iCivilService, xml.iEconomics, xml.iMachinery, xml.iAristocracy]:
			gc.getPlayer(iPlayer).changeStabilityBase( iCathegoryEconomy, -1 )
		pass
						...
						if pPlayer.getNumCities() > 8:
							if iRand1 < 8: #80 chance for secession start
								if iRand2 < (-3 - iStability): #10% at -4, increasing by 10% with each point (100% with -13 or less)
									rnf.revoltCity( iPlayer, False )
							elif iRand3 < 1 and iGameTurn >= con.tBirth[iPlayer] + 20 and not utils.collapseImmune(iPlayer): #10 chance for collapse start
							...
							if iRand1 < 4: #40 chance for secession start
								if iRand2 < (-3 - iStability): #10% at -4, increasing by 10% with each point (100% with -13 or less)
									rnf.revoltCity( iPlayer, False )
							elif iRand3 < 4 and iGameTurn >= con.tBirth[iPlayer] + 20 and not utils.collapseImmune(iPlayer): #40 chance for collapse start
								if iRand2 < (-1.5 - (iStability/2)): #10% at -4, increasing by 10% with 2 points (100% with -22 or less)
									self.collapseCivilWar(iPlayer, iStability)
		...
		for pCity in utils.getCityList(iPlayer):
			...
			if iCivic4 != xml.iCivicFreeReligion: # Religious Tolerance negates stability penalties from non-state religions
				if not gc.hasUP( iPlayer, con.iUP_ReligiousTolerance ): # Polish UP
					...
					if pCity.getNumForeignReligions() > 1: # additional -1 stability for every further foreign religion
