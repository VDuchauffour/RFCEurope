...
#			szLabel = localText.changeTextColor(szLabel, gc.getInfoTypeForString("COLOR_YELLOW"))
	...
	def drawFaithInfo( self ):
		...
		if ( pPlayer.isFaithBenefit( con.iFP_Stability ) ):
			sString = localText.getText("TXT_KEY_FAITH_STABILITY",()) + (" +%i " %pPlayer.getFaithBenefit(con.iFP_Stability) )
			szFaithBenefits += u"<font=3>" + sString + u"</font>\n"
		if ( pPlayer.isFaithBenefit( con.iFP_Civic ) ):
			sString = localText.getText("TXT_KEY_FAITH_CIVIC",()) + (" -%i percent " %pPlayer.getFaithBenefit(con.iFP_Civic) )
			szFaithBenefits += u"<font=3>" + sString + u"</font>\n"
		if ( pPlayer.isFaithBenefit( con.iFP_Growth ) ):
			sString = localText.getText("TXT_KEY_FAITH_GROWTH",()) + (" +%i percent " %pPlayer.getFaithBenefit(con.iFP_Growth) )
			szFaithBenefits += u"<font=3>" + sString + u"</font>\n"
		if ( pPlayer.isFaithBenefit( con.iFP_Units ) ):
			sString = localText.getText("TXT_KEY_FAITH_UNITS",()) + (" -%i percent " %pPlayer.getFaithBenefit(con.iFP_Units) )
			szFaithBenefits += u"<font=3>" + sString + u"</font>\n"
		if ( pPlayer.isFaithBenefit( con.iFP_Science ) ):
			sString = localText.getText("TXT_KEY_FAITH_SCIENCE",()) + (" -%i percent " %pPlayer.getFaithBenefit(con.iFP_Science) )
			szFaithBenefits += u"<font=3>" + sString + u"</font>\n"
