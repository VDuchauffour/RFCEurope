	...
	if (isActiveCorporation(eCorporation) && !isDisorder())
	{
		for (int i = 0; i < GC.getNUM_CORPORATION_PREREQ_BONUSES(); ++i)
		{
			...
			if (NO_BONUS != eBonus && getNumBonuses(eBonus) > 0)
			{
				...
				iYield += (GC.getCorporationInfo(eCorporation).getYieldProduced(eIndex) * getNumBonuses(eBonus) * 2 * GC.getWorldInfo(GC.getMapINLINE().getWorldSize()).getCorporationMaintenancePercent()) / 100;
	...
	if (isActiveCorporation(eCorporation) && !isDisorder())
	{
		for (int i = 0; i < GC.getNUM_CORPORATION_PREREQ_BONUSES(); ++i)
		{
			...
			if (NO_BONUS != eBonus && getNumBonuses(eBonus) > 0)
			{
				...
				iCommerce += (GC.getCorporationInfo(eCorporation).getCommerceProduced(eIndex) * getNumBonuses(eBonus) * 2 * GC.getWorldInfo(GC.getMapINLINE().getWorldSize()).getCorporationMaintenancePercent()) / 100;
			...
			if (bFirst)
			{
				...
				if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
				{
					...
					if (isWorldWonderClass((BuildingClassTypes)(GC.getBuildingInfo(eIndex).getBuildingClassType())))
					{
						...
						for (iI = 0; iI < MAX_PLAYERS; iI++)
						{
							if ((GET_PLAYER((PlayerTypes)iI).isAlive()) && (GC.getGameINLINE().getGameTurn() >= startingTurn[GC.getGameINLINE().getActivePlayer()])) // Absinthe: no wonder built message during autoplay
							{
								if (isRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false))
								{
						...
						if (GET_PLAYER((PlayerTypes)iJ).isAlive())
						{
							for (pLoopCity = GET_PLAYER((PlayerTypes)iJ).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER((PlayerTypes)iJ).nextCity(&iLoop))
							{
								if (pLoopCity->isConnectedTo(this))
								{
									...
									int iSpreadFactor = civSpreadFactor[getOwnerINLINE() + NUM_ALL_PLAYERS_B * iI] * iBaseSpreadFactor / 100;
...
PlayerTypes CvCity::getLiberationPlayer(bool bConquest) const
{
	...
	for (int iPlayer = 0; iPlayer < NUM_MAJOR_PLAYERS; ++iPlayer) //Rhye
	{
		...
		if (kLoopPlayer.isAlive() && !kLoopPlayer.isBarbarian() && !kLoopPlayer.isMinorCiv()) // Absinthe: no liberation for non-major players
		{
